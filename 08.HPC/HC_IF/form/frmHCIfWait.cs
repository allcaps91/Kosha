...
/// <summary>
/// Class Name      : HC_IF
/// File Name       : frmHCIfWait.cs
/// Description     : 대기순번(1번) 통합
/// Author          : 이상훈
...
/// <history>  
/// </history>
/// <seealso cref= "Frm대기순번1.frm(Frm대기순번1)" />

namespace HC_IF
{
    public partial class frmHCIfWait : Form
    {
        ...
        void eFormLoad(object sender, EventArgs e)
        {
            ...
            strIpAddress = clsCompuInfo.gstrCOMIP + ":1";
            ...
            timer1.Enabled = true;
            timer2.Enabled = true;
            eTimerTick(timer2, new EventArgs());
        ...
        void eTimerTick(object sender, EventArgs e)
        {
            ...
            else if (sender == timer2)
            {
                ...
                strGubun = "BAS_표시장비자동종료시간";

                if (strYoil == "토요일")
                {
                    strStopTime = VB.Pstr(hicBcodeService.GetCodeNamebyGubunCode(strGubun, clsCompuInfo.gstrCOMIP), "{}", 2);
                }
                else
                {
                    strStopTime = VB.Pstr(hicBcodeService.GetCodeNamebyGubunCode(strGubun, clsCompuInfo.gstrCOMIP), "{}", 1);
                }

                //종료시간 설정값이 없으면
                if (strStopTime.IsNullOrEmpty())
                {
                    return;
                }
                ...
                date1 = Convert.ToDateTime(clsPublic.GstrSysDate);
                date2 = Convert.ToDateTime(strStopTime);
                if (date1 >= date2)
                {
                    timer1.Enabled = false;
                    timer2.Enabled = false;
                    //PC 강제종료
                    Process.Start("shutdown.exe", "-s -t 3"); //xx 초 후 종료
                }
            }
        ...
        void fn_Screen_Display_Hea()
        {
            ...
            for (int i = 0; i < nREAD; i++)
            {
                ...
                if (FstrRoom == "9")    //내시경실
                {
                    ...
                    //종검,건강검진 구분을 읽음
                    if (heaJepsuService.GetWrtNobySDatePtNo(list[i].WRTNO.ToString()) > 0)
                    {
                        ssList.ActiveSheet.Cells[i, 3].Text = "TO";

                    }
                    else
                    {
                        ssList.ActiveSheet.Cells[i, 3].Text = "HR";
                    }
                }
                nRow += 1;
            }
