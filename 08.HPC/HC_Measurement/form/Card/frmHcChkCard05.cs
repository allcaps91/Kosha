...
{
    public partial class frmHcChkCard05 :CommonForm
    {
        ...
        public frmHcChkCard05(long nWRTNO)
        {
            InitializeComponent();
            SetEvent();
            SetControl();
        ...
        private void eTxtKeyPress(object sender, KeyPressEventArgs e)
        {
            if (sender == txtChaAmt1)
            {
                if (e.KeyChar == 13)
                {
                    long nBillAmt = VB.Replace(lblBillAmt.Text, ",", "").To<long>(0);
        ...
        private void Display_Amt_Gesan(long nWRTNO)
        {
            ...
            {
                lblBaseAmt.Text = VB.Format(hCEST.BASEAMT, "###,###,##0");
                lblChargeAmt.Text = VB.Format(hCEST.CHARGEAMT, "###,###,##0");
                lblBillAmt.Text = VB.Format(hCEST.TOTAMT, "###,###,##0");
        ...
        private void DisPlay_Estimate(string argMark = "")
        {
            ...
            try
            {
                ...
                if (dto.GBEST.Equals("Y"))
                {
                    ...
                    ssPrt.SetCellText(nPrtRow, 3, "1");            //건수     
                    ssPrt.SetCellText(nPrtRow, 6, lblBaseAmt.Text);     //공급가액

                    nPrtRow += 1;
                    //분석수수료 표시
                    ssPrt.SetCellText(nPrtRow, 1, "분석수수료");         //항목  
                    ssPrt.SetCellText(nPrtRow, 6, lblChargeAmt.Text);    //공급가액    
        ...
        private void Process_Amt_Gesan()
        {
            ...
            lblBaseAmt.Text = VB.Format(nBaseAmt, "###,###,##0");
            lblChargeAmt.Text = VB.Format(nAMT, "###,###,##0");
            lblBillAmt.Text = VB.Format(nBaseAmt + nAMT, "###,###,##0");
        ...
        private void Save_Data()
        {
            try
            {
                ...
                {
                    TOTAMT = VB.Replace(lblBillAmt.Text, ",", "").To<long>(0),
                    SEQNO = nSeqno,
                    BASEAMT = VB.Replace(lblBaseAmt.Text, ",", "").To<long>(0),
                    CHARGEAMT = VB.Replace(lblChargeAmt.Text, ",", "").To<long>(0),
        ...
        private bool Check_Estimate_Amt()
        {
            ...
            if (lblBaseAmt.Text != VB.Format(nBaseAmt, "###,###,##0")) { rtnVal = false; }
            if (lblChargeAmt.Text != VB.Format(nAMT, "###,###,##0")) { rtnVal = false; }
            if (lblBillAmt.Text != VB.Format(nBaseAmt + nAMT, "###,###,##0")) { rtnVal = false; }
            ...
            if (lblBillAmt.Text != VB.Format(nTotAmt, "###,###,##0")) { rtnVal = false; }
        ...
        private void eFormload(object sender, EventArgs e)
        {
            ...
            Display_Estimate(FnWRTNO);

            ssEST.AddRows(1);
        }

        private void Display_Estimate(long fnWRTNO)
        {
            long nYear = 0;
            int nRow = 0;
            ...
            //최종작업내역
            HIC_CHUKMST_NEW hCMN = hicChukMstNewService.GetItemByWrtno(FnWRTNO);

            if (!hCMN.IsNullOrEmpty())
            {
                nYear = hCMN.CHKYEAR.To<long>(0);
            ...
            //견적서 내역
            List<HIC_CHUK_ESTIMATE> lstHCEST = hicChukEstimateService.GetItemByWrtno(FnWRTNO);
            ...
            else
            {
                ...
                List<HIC_CHUKDTL_PLAN_SUGA> lstSUGA = hicChukDtlPlanService.GetAccountByPlan(FnWRTNO, nYear);
                ...
                if (lstSUGA.Count > 0)
                {
                    ...
                    //기본관리료 계산 및 추가
                    ADD_BASIC_SUGA(lstHCE, hCE, ref nRow, FnWRTNO, hCMN);
