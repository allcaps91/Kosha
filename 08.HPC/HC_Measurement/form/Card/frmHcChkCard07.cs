...
{
    public partial class frmHcChkCard07 :CommonForm
    {
        ...
        public frmHcChkCard07(long nWRTNO)
        {
            InitializeComponent();
            SetEvent();
            SetControl();
        ...
        private void Data_Save()
        {
            try
            {
                ...
                if (lstNOISE.Count > 0)
                {
                    if (lstNOISE.Count > 0)
                    {
                        if (!hicChukDtlResultService.Save(lstNOISE, FnWRTNO, "2"))
                        {
