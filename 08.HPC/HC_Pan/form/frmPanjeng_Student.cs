...
/// <summary>
/// Class Name      : HC_Pan
/// File Name       : frmPanjeng_Student.cs
/// Description     : 학생검진판정
/// Author          : 이상훈
...
namespace HC_Pan
{
    public partial class frmPanjeng_Student : Form
    {
        ...
        //HC_PANJENG_PATLIST_SCHOOL FPatListItem = null;
        HC_PANJENG_PATLIST FPatListItem = null;
        ...
        public frmPanjeng_Student()
        {
            InitializeComponent();
            SetEvent();
        }

        //public frmPanjeng_Student(HC_PANJENG_PATLIST_SCHOOL PatListItem)
        //{
        //    InitializeComponent();
        //    FPatListItem = new HC_PANJENG_PATLIST_SCHOOL();
        //    FPatListItem = PatListItem;
        //    FnWRTNO = FPatListItem.WRTNO;

        //    SetEvent();
        //}

        public frmPanjeng_Student(HC_PANJENG_PATLIST PatListItem)
        {
            InitializeComponent();
            FPatListItem = new HC_PANJENG_PATLIST();
            FPatListItem = PatListItem;
            FnWRTNO = FPatListItem.WRTNO;
        ...
        void eTxtKeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            {
                if (txtPanJochi.Text.To<int>() > 300)
                {
        ...
        void eFormLoad(object sender, EventArgs e)
        {
            ...
            //    fn_Sangdam_Display(FnWRTNO);
            //    txtPanDr.Text = clsHcVariable.GnHicLicense.ToString();
        ...
        void fn_Sangdam_Display(long nWrtNo)
        {
            ...
            txtRemark.Text = hicSangdamNewService.GetRemarkbyWrtNo(FnWRTNO);
        }
        ...
        void eBtnClick(object sender, EventArgs e)
        {
            ...
            else if (sender == btnOK)
            {
                ...
                FstrROWID = hicSchoolNewService.GetRowIdbyWrtNo(FnWRTNO);
                ...
                if (!list.IsNullOrEmpty())
                {
                    ...
                    strJepDate = list.JEPDATE.To<string>();           //접수일자
                }
                else
                {
                ...
                result = hicJepsuService.UpdatePanjengDrNoPanDatebyWrtNo(FnWRTNO, nPPanDrno, strPanDate);
            ...
            else if (sender == btnXrayResult)
            {
                ...
                strSname = FPatListItem.SNAME;
                strJepDate = FPatListItem.JEPDATE;
        ...
        void fn_Exam_Result_Show()
        {
            ...
            //검사결과불러오기
            List<HIC_RESULT> list = hicResultService.GetExCodeResultbyOnlyWrtNo(FnWRTNO);
        ...
        void fn_Bp_AutoPanjeng()
        {
            ...
            else
            {
                ...
                {
                    if (FnHeight < SS2.ActiveSheet.Cells[nCurRow, i].Text.To<double>())
                    {
                        if (i == 1)
                        {
                            i += 1;
            ...
            for (int i = n1; i <= n2; i++)
            {
                if (SS3.ActiveSheet.Cells[nCurRow, i].Text.Trim() == strTGrade)
                {
                    ...
                    {
                        if (txtBH.Text.Trim().To<double>() < SS3.ActiveSheet.Cells[j, i].Text.To<double>())
                        {
                            if (j == nStart)
                            {
                        ...
                        else if (txtBH.Text.Trim().To<double>() == SS3.ActiveSheet.Cells[j, 1].Text.Trim().To<double>())
                        {
                            strHGrade = SS3.ActiveSheet.Cells[j, 1].Text.Trim();
                            break;
                        }
            ...
            for (int i = n1; i <= n2; i++)
            {
                if (SS4.ActiveSheet.Cells[0, i].Text.Trim() == strTGrade)
                {
                    ...
                    {
                        if (txtBL.Text.Trim().To<double>() < SS4.ActiveSheet.Cells[j, i].Text.Trim().To<double>())
                        {
                            if (j == nStart)
                            {
                        ...
                        else if (txtBL.Text.Trim().To<double>() == SS4.ActiveSheet.Cells[j, i].Text.Trim().To<double>())
                        {
            ...
            if (strHGrade.To<double>() >= 90 && strHGrade.To<double>() <= 95)
            {
                lblPan23.Text = "정상(경계)";
            }

            if (strHGrade.To<double>() >= 90 && strLGrade.To<double>() <= 95)
            {
                lblPan23.Text = "정상(경계)";
            }

            if (strHGrade.To<double>() > 95 && strLGrade.To<double>() > 95)
            {
        ...
        void fn_ERROR_CHECK()
        {
            ...
            if (hicResultService.GetExCodebyWrtNoExCode(FnWRTNO) > 0)
            {
