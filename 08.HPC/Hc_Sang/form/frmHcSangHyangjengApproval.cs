...
namespace Hc_Sang
{
    public partial class frmHcSangHyangjengApproval : Form
    {
        ...
        void eBtnClick(object sender, EventArgs e)
        {
            ...
            else if (sender == btnOK)
            {
                ...
                for (int i = 0; i < SS1.ActiveSheet.RowCount; i++)
                {
                    ...
                    if (SS1.ActiveSheet.Cells[i, 12].Text.Trim() == "")
                    {
                        nCNT += 1;
                    }
                }
            ...
            else if (sender == btnSearch)
            {
                ...
                {
                    strDeptCode = list[i].DEPTCODE.Trim();
                    strOK = "OK";
                    if (list[i].QTY == 0)
                    {
                        strOK = "";
                    ...
                    if (strOK == "OK")
                    {
                        ...
                        SS1.ActiveSheet.Cells[i, 1].Text = list[i].BDATE.ToString();
                        SS1.ActiveSheet.Cells[i, 2].Text = list[i].WRTNO.ToString();
                        SS1.ActiveSheet.Cells[i, 3].Text = list[i].SNAME.Trim();
                        SS1.ActiveSheet.Cells[i, 4].Text = list[i].AGE + "/" + list[i].SEX;
                        SS1.ActiveSheet.Cells[i, 5].Text = " " + strSuCode;
                        SS1.ActiveSheet.Cells[i, 6].Text = list[i].QTY.ToString("#0:0.00");
                        SS1.ActiveSheet.Cells[i, 8].Text = list[i].REALQTY.ToString("#0:0.00");
                        ...
                        if (strDrname == "")
                        {
                            strDrname = list[i].DRSABUN.Trim();
                        }
                        SS1.ActiveSheet.Cells[i, 13].Text = strDrname;
                        SS1.ActiveSheet.Cells[i, 14].Text = list[i].APPROVETIME.ToString();
                        SS1.ActiveSheet.Cells[i, 15].Text = list[i].ROWID;
            ...
            {
                SS1.ActiveSheet.Cells[0, 0, SS1.ActiveSheet.NonEmptyRowCount, 0].Text = "True";
            }
            else if (sender == btnMenuAllCancel)
            {
                SS1.ActiveSheet.Cells[0, 0, SS1.ActiveSheet.NonEmptyRowCount, 0].Text = "";
            }
        }
        ...
        void fn_HIC_HYANG_Approve(long argWrtNo)
        {
            ...
            for (int i = 0; i < SS1.ActiveSheet.RowCount; i++)
            {
                if (long.Parse(SS1.ActiveSheet.Cells[i, 2].Text) == argWrtNo)
                {
                    strSuCode = SS1.ActiveSheet.Cells[i, 5].Text.Trim();
                    strList += strSuCode + ",";
                }
            ...
            List<HIC_HYANG_APPROVE> list = hicHyangApproveService.GetItemListbyWrtNo(argWrtNo);
            ...
            for (int i = 0; i < nREAD; i++)
            {
                ...
                if (VB.InStr(strList, "," + strSuCode + ",") == 0)
                {
                    ...
                    result = hicHyangApproveService.UpdatebyRowId(list[i].ROWID);
                    ...
                    result = hicHyangService.UpdatebyRowId(list[i].ROWID, argWrtNo, strSuCode);
                    ...
                    {
                        clsDB.setRollbackTran(clsDB.DbCon);
                        MessageBox.Show("향정/마약 삭제 중 오류발생", "오류", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
            ...
            for (int i = 0; i < SS1.ActiveSheet.RowCount; i++)
            {
                nWRTNO = long.Parse(SS1.ActiveSheet.Cells[i, 2].Text);
                strSuCode = SS1.ActiveSheet.Cells[i, 2].Text.Trim();
                nQty = double.Parse(SS1.ActiveSheet.Cells[i, 2].Text);

                if (nWRTNO == argWrtNo && strSuCode != "")
                {
                    HIC_HYANG_APPROVE list3 = hicHyangApproveService.GetItembyWrtNoSucode(argWrtNo, strSuCode);

                    strROWID = list3.ROWID.Trim();
                    strPTNO =  list3.PTNO.Trim();
                    strSname = list3.SNAME.Trim();
                    strGbSite = list3.GBSITE.Trim();
                    strSex = list3.SEX.Trim();
                    nOldQty = list3.QTY;
                    nAge = list3.AGE;

                    //승인한 약품의 수량이 변경되었으면 History를 저장함
                    if (nQty != nOldQty && list3.APPROVETIME.ToString() != "")
                    {
                    ...
                    result = hicHyangApproveService.UpdateItembyRowId(nQty, nOldQty, clsType.User.IdNumber, strROWID, list3.APPROVETIME.ToString().Trim());
                   
                    if (result < 0)
                    {
                        clsDB.setRollbackTran(clsDB.DbCon);
                        MessageBox.Show("향정 삭제시 오류 발생!", "오류", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
                    ...
                    //HIC_HYANG Update
                    sRowId = hicHyangService.GetRowIdbyWrtNoSuCode(argWrtNo, strSuCode);
                    ...
                    //OCS_OORDER 전송
                    COMHPC list4 = comHpcLibBService.GetRowIdQtyOcsOrderbyPtNoBDateSuCode(strPTNO, strBDATE, strSuCode);
                    ...
                    {
                        result = comHpcLibBService.UpdateQtybyRowId(nQty, list3.ROWID);
                    }
                    ...
                    if (strGbSite == "2")
                    {
                        if (VB.Left(strSuCode, 2) == "N-")
                        {
                            ...
                            item.REMARK2 = "Pain";
                            item.JUSO = strJuso;
                            item.DRSABUN = long.Parse(clsType.User.IdNumber);
                            item.CERTNO = "";
                            item.ROWID = strROWID;
