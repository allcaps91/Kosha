...
/// <summary>
/// Description : 신용카드 수동등록(다우데이터) - 건강증진센터
/// Author : 김민철
/// Create Date : 2020.06.11
/// </summary>
/// <history>
/// </history>
/// <seealso cref="frm신용카드등록/Frm신용카드등록(센터))"/> 

namespace HC_Main
{
    public partial class frmHcMstCardDaou : Form
    {
        ...
        private void setEvent()
        {
            ...
            this.txtWrtno.KeyPress                  += new KeyPressEventHandler(eCtl_KeyPress);
        ...
        private void SAVE_PROCESS(PsmhDb pDbCon)
        {
            ...
            {
                ComFunc.MsgBox("접수번호를 입력하시기 바랍니다.", "확인");
                txtWrtno.Focus();
                return;
            }
            ...
            try
            {
                if (FstrRowID == "")
                {
                    ...
                    SQL = "";
                    SQL += ComNum.VBLF + " INSERT INTO " + ComNum.DB_PMPA + "CARD_APPROV_CENTER ";
                    SQL += ComNum.VBLF + "        (CARDSEQNO, ACTDATE, CDSEQNO, ";
                    SQL += ComNum.VBLF + "         PANO, HPANO, HWRTNO, JUMIN, SNAME, ";
                    SQL += ComNum.VBLF + "         GBIO, DEPTCODE, PCODE, ";
                    SQL += ComNum.VBLF + "         PART, SEQNO, TRANHEADER, ";
                    SQL += ComNum.VBLF + "         TEMINALID, TRANDATE, INPUTMETHOD,";
                    SQL += ComNum.VBLF + "         INSTPERIOD, TRADEAMT, TRXAMT, ";
                    ...
                    SQL += ComNum.VBLF + "         ORIGINNO, ORIGINDATE, CARDNO, ";
                    SQL += ComNum.VBLF + "         PERIOD, FULLCARDNO, REMARK,";
                    SQL += ComNum.VBLF + "         ENTSABUN, ENTERDATE, GUBUN, CENTERGBN ";
                    ...
                    SQL += ComNum.VBLF + "         " + txtHPano.Text.Trim() + ", ";
                    SQL += ComNum.VBLF + "         " + txtWrtno.Text.Trim() + ", ";
                    ...
                    SQL += ComNum.VBLF + "         '" + clsAES.AES(txtInCardNo.Text.Trim()) + "', ";
                    SQL += ComNum.VBLF + "         '" + clsAES.AES(txtInPer.Text.Trim()) + "', ";
                    SQL += ComNum.VBLF + "         '', ";
                    SQL += ComNum.VBLF + "         '', ";
                    SQL += ComNum.VBLF + "         '" + clsType.User.Sabun + "', ";
                    SQL += ComNum.VBLF + "         SYSDATE, ";
                    SQL += ComNum.VBLF + "         '" + strBun + "', ";
                    SQL += ComNum.VBLF + "         '" + cboInIO.Text.Trim() == "H" ? "'1" : "'2" + "') ";
                }
                else
                {
                    SQL = "";
                    SQL += ComNum.VBLF + " UPDATE " + ComNum.DB_PMPA + "CARD_APPROV_CENTER ";
        ...
        private void READ_PROCESS(PsmhDb pDbCon)
        {
            ...
            SQL += ComNum.VBLF + "        ROWID, HPANO, HWRTNO, ";
            ...
            for (int i = 0; i < Dt.Rows.Count; i++)
            {
                ...
                ssList_Sheet1.Cells[i, 6].Text = clsAES.DeAES(Dt.Rows[i]["CARDNO"].ToString().Trim());
                ssList_Sheet1.Cells[i, 7].Text = clsAES.DeAES(Dt.Rows[i]["PERIOD"].ToString().Trim());
                ...
                ssList_Sheet1.Cells[i, 15].Text = Dt.Rows[i]["HPANO"].ToString().Trim();
                ssList_Sheet1.Cells[i, 16].Text = Dt.Rows[i]["HWRTNO"].ToString().Trim();
            }
        ...
        private void eCtl_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if (sender == this.txtInPtno && e.KeyChar == (Char)13)
            {
                ...
                txtHPano.Text = hicPatientService.GetPanobyPtno(txtInPtno.Text).To<string>();
        ...
        private void eCtl_Selected(object sender, EventArgs e)
        {
            ...
            if (sender == this.cboInFiCode)
            {
                switch (cboInFiCode.Text.Trim())
                {
                    ...
                    case "03": txtInFiName.Text = "외환카드"; break;
                    case "04": txtInFiName.Text = "삼성카드"; break;
                    case "05": txtInFiName.Text = "엘지카드"; break;
                    case "08": txtInFiName.Text = "현대카드"; break;
                    case "09": txtInFiName.Text = "롯데아멕스"; break;
                    case "10": txtInFiName.Text = "신한은행"; break;
        ...
        private void ssList_CellDoubleClick(object sender, FarPoint.Win.Spread.CellClickEventArgs e)
        {
            ...
            txtHPano.Text = ssList_Sheet1.Cells[e.Row, 15].Text.Trim();
            txtWrtno.Text = ssList_Sheet1.Cells[e.Row, 16].Text.Trim();
        ...
        private void SELECT_CARD(string ArgPtno, string ArgDate)
        {
            ...
            SQL += ComNum.VBLF + "        DEPTCODE, ROWID,                                  --진료과목,산전지원승인금액,rowid";
            ...
            if (Dt.Rows.Count >= 1)
            {
                txtInCardNo.Text = clsAES.DeAES(Dt.Rows[0]["CardNo"].ToString().Trim());
                txtInPer.Text = clsAES.DeAES(Dt.Rows[0]["PERIOD"].ToString().Trim());
