...
/// <summary>
/// Class Name      : HC_Bill
/// File Name       : frmHcMisuNoHelp1.cs
/// Description     : 회사별 미수번호 찾기
/// Author          : 심명섭
/// Create Date     : 2021-06-17
/// Update History  : 
/// </summary>
/// <seealso cref= "hcmisu > FrmMisunoHelp1 (HcMisu02.frm)" />
/// 

namespace HC_Bill
{
    public partial class frmHcMisuNoHelp1 :BaseForm
    {
        ...
        private void SetControl()
        {
            ...
            // 회사코드 번호
            SS1.AddColumn("회사코드",         nameof(HIC_MISU_MST_SLIP.LTDCODE),     100,    FpSpreadCellType.TextCellType,     new SpreadCellTypeOption { IsVisivle = false });
            // 쿼리에서 함수 호출 / 프로퍼티 생성(LTDNAME) /  KOSMOS_PMPA.FC_HIC_LTDNAME(LTDCODE) LTDNAME
            SS1.AddColumn("회사명",           nameof(HIC_MISU_MST_SLIP.LTDNAME),     100,    FpSpreadCellType.TextCellType,     new SpreadCellTypeOption { IsEditble = false, Aligen = CellHorizontalAlignment.Center });
            SS1.AddColumn("검진종류",         nameof(HIC_MISU_MST_SLIP.GJONG),       100,    FpSpreadCellType.TextCellType,     new SpreadCellTypeOption { IsVisivle = false });
            ...
            SS1.AddColumn("입금금액",         nameof(HIC_MISU_MST_SLIP.IPGUMAMT),    100,    FpSpreadCellType.TextCellType,     new SpreadCellTypeOption { IsEditble = false, Aligen = CellHorizontalAlignment.Center });
            ...
            SS1.AddColumn("지로번호",         nameof(HIC_MISU_MST_SLIP.GIRONO),      100,    FpSpreadCellType.TextCellType,     new SpreadCellTypeOption { IsEditble = false, Aligen = CellHorizontalAlignment.Center });
            ...
            SS1.AddColumn("담당자",           nameof(HIC_MISU_MST_SLIP.DAMNAME),     100,    FpSpreadCellType.TextCellType,     new SpreadCellTypeOption { IsEditble = false, Aligen = CellHorizontalAlignment.Center });
        }
        ...
        private void DisPlay_Screen()
        {
            ...
            else if(CboYYMM.Text == "최근1년")
            {
                strYYMM     =   "";
                strFdate    =   CF.DATE_ADD(clsDB.DbCon, clsPublic.GstrSysDate, -370);
                strTdate    =   clsPublic.GstrSysDate;
            }
            ...
            List<HIC_MISU_MST_SLIP> item = hicMisuMstSlipService.Getitem(strFdate, strTdate, strJong, strDtlJong, strLtdCode, rdoJob1.Checked, rdoJob2.Checked, TxtMisuAmt.Text.To<long>(0), CboName.Text.Trim(), rdoSort1.Checked, strLtdCode.To<long>(0));
            ...
            //    SS1.ActiveSheet.Cells[i, 2].Text    =      cHB.READ_Ltd_Name(item[i].LTDCODE.To<string>("0").Trim());
            //    SS1.ActiveSheet.Cells[i, 3].Text    =      cHB.READ_GjJong_Name(item[i].GJONG.Trim());
            //    SS1.ActiveSheet.Cells[i, 4].Text    =      VB.Format(item[i].MISUAMT, "###,###,###,##0");
            //    SS1.ActiveSheet.Cells[i, 5].Text    =      VB.Format(item[i].IPGUMAMT, "###,###,###,##0");
            //    SS1.ActiveSheet.Cells[i, 6].Text    =      VB.Format(item[i].DAMT, "###,###,###,##0");
            //    SS1.ActiveSheet.Cells[i, 7].Text    =      VB.Format(item[i].JANAMT, "###,###,###,##0");
            //    SS1.ActiveSheet.Cells[i, 8].Text    =      item[i].GIRONO;
            //    SS1.ActiveSheet.Cells[i, 9].Text    =      item[i].REMARK;
            //    SS1.ActiveSheet.Cells[i, 10].Text   =      item[i].DAMNAME;
            //}
        ...
        private void eFormLoad(object sender, EventArgs e)
        {
            ...
            CboName.Clear();
            CboName.Items.Add(" ");
            CboName.Items.Add("이창우");
            CboName.Items.Add("장미선");
            CboName.Items.Add("권향미");
            CboName.Items.Add("이영숙");
            CboName.Items.Add("김진용");
            CboName.Items.Add("김태동");
            CboName.Items.Add("김찬준");
            CboName.Items.Add("이정희");
            CboName.Items.Add("이미옥");
            CboName.Items.Add("손정숙");
            CboName.Items.Add("박수민");

            TxtLtdCode.Text     =   ""      ;
            PanelLtdName.Text   =   "전체"  ;
