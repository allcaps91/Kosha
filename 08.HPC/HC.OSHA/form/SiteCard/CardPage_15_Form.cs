using ComBase;
using ComBase.Controls;
using ComBase.Mvc.Enums;
using ComBase.Mvc.Utils;
using ComHpcLibB.Model;
using FarPoint.Win;
using FarPoint.Win.Spread;
using HC.Core.Common.Interface;
using HC.Core.Common.Util;
using HC.Core.Dto;
using HC.Core.Model;
using HC.Core.Repository;
using HC.Core.Service;
using HC.OSHA.Dto;
using HC.OSHA.Service;
using HC_Core;
using System;
using System.Collections.Generic;
using System.Drawing;
using System.IO;
using System.Windows.Forms;

namespace HC_OSHA
{
    /// <summary>
    /// 19.위탁업무수행일지 1페이지,
    /// </summary>
    public partial class CardPage_15_Form : CommonForm, ISelectSite, ISelectEstimate, IPrint
    {
        private HcOshaCard19Service hcOshaCard19Service;
        HcUsersRepository hcUsersRepository;
        AutoCompleteMacro autoCompleteMacro;
        public CardPage_15_Form()
        {
            InitializeComponent();
            this.hcOshaCard19Service = new HcOshaCard19Service();
            hcUsersRepository = new HcUsersRepository();
        }

        public void Select(IEstimateModel estimateModel)
        {
            base.SelectedEstimate = estimateModel;
            Clear15();

            Search();
        }

        public void Select(ISiteModel siteModel)
        {
            base.SelectedSite = siteModel;
           
        }

        private void Clear()
        {
            SSList.ActiveSheet.RowCount = 0;
            pan19.Initialize();
        }

        public void Print()
        {
            Init();
            Search("PRINT");
            SpreadPrint print = new SpreadPrint(SSCard, PrintStyle.FORM, false);
            print.Execute();
        }

        public bool NewPrint()
        {
            Init();
            Search("PRINT");
            SpreadPrint print = new SpreadPrint(SSCard, PrintStyle.FORM, false);
            print.Execute();
            return true;
        }

        private void BtnPrint_Click(object sender, EventArgs e)
        {
            Print();
        }

        private void CardPage_15_Form_Load(object sender, EventArgs e)
        {
            Init();

            Clear15();

            SetUser();
        }
        private void Init()
        {
            RdoStatus_0.SetOptions(new RadioButtonOption { DataField = nameof(HC_OSHA_CARD19.STATUS), CheckValue = "0" });
            RdoStatus_1.SetOptions(new RadioButtonOption { DataField = nameof(HC_OSHA_CARD19.STATUS), CheckValue = "1" });
            RdoStatus_2.SetOptions(new RadioButtonOption { DataField = nameof(HC_OSHA_CARD19.STATUS), CheckValue = "2" });

            DtpRegDate.SetOptions(new DateTimePickerOption { DataField = nameof(HC_OSHA_CARD19.REGDATE), DisplayFormat = DateTimeType.YYYY_MM_DD, DataBaseFormat = DateTimeType.None });
           
            SSList.Initialize(new SpreadOption() { IsRowSelectColor = false , RowHeightAuto =true});
            SSList.AddColumnDateTime("방문일자", nameof(HC_OSHA_CARD19.REGDATE), 81, IsReadOnly.N, DateTimeType.YYYY_MM_DD, new SpreadCellTypeOption { IsSort = true, IsShowCalendarButton = false });
            SSList.AddColumnText("자격", nameof(HC_OSHA_CARD19.CERT), 54, IsReadOnly.N, new SpreadCellTypeOption { IsSort = false, IsMulti = true, WordWrap = true });
            SSList.AddColumnText("성명", nameof(HC_OSHA_CARD19.NAME), 50, IsReadOnly.N, new SpreadCellTypeOption { IsSort = false });
            SSList.AddColumnText("업무수행내용", nameof(HC_OSHA_CARD19.CONTENT), 233, IsReadOnly.Y, new SpreadCellTypeOption { IsSort = false, IsMulti = true, WordWrap = true });
            SSList.AddColumnText("문제점 및 개선지도사항", nameof(HC_OSHA_CARD19.OPINION), 247, IsReadOnly.Y, new SpreadCellTypeOption { IsSort = false, IsMulti = true, WordWrap = true });
            SSList.AddColumnText("서명여부", nameof(HC_OSHA_CARD19.ISSIGN), 40, IsReadOnly.N, new SpreadCellTypeOption { IsSort = false, IsMulti = false, WordWrap = false });

            ComFunc.ReadSysDate(clsDB.DbCon);

            int year = clsPublic.GstrSysDate.Left(4).To<int>();

            for(int i = 0; i<5; i++)
            {
                int y = year - i;
                CboStartYear1.Items.Add(y);
                CboStartYear2.Items.Add(y);

                CboEndYear1.Items.Add(y);
                CboEndYear2.Items.Add(y);
            }

            CboStartYear1.SelectedIndex = 1;
            CboStartYear2.SelectedIndex = 1;

            CboEndYear1.SelectedIndex = 0;
            CboEndYear2.SelectedIndex = 0;

        }

        private void Search(string gubun = "ALL")
        {
            if (base.SelectedSite == null)
            {
                return;
            }

            if (base.SelectedEstimate.CONTRACTENDDATE == null)
            {
                return;
            }

            List<HC_OSHA_CARD19> list = null;

            if(gubun.Equals("ALL") || gubun.Equals("LIST"))
            {
                //List<HC_OSHA_CARD19> list = hcOshaCard19Service.hcOshaCard19Repository.FindAll(base.SelectedSite.ID, base.SelectedEstimate.CONTRACTSTARTDATE, base.SelectedEstimate.CONTRACTENDDATE);
                list = hcOshaCard19Service.hcOshaCard19Repository.FindAll(base.SelectedSite.ID, CboStartYear2.Text, CboEndYear2.Text);
                if (list.Count > 0)
                {
                    for (int i = 0; i < list.Count; i++)
                    {
                        if (list[i].SITESIGN != null)
                        {
                            list[i].ISSIGN = "Y";
                        }
                        else
                        {
                            list[i].ISSIGN = "N";
                        }
                    }
                    SSList.SetDataSource(list);
                }
                else
                {
                    SSList.SetDataSource(new List<HC_OSHA_CARD19>());
                }
            }

            if(gubun.Equals("ALL") || gubun.Equals("PRINT"))
            {
                Clear15();

                list = hcOshaCard19Service.hcOshaCard19Repository.FindAll(base.SelectedSite.ID, CboStartYear1.Text, CboEndYear1.Text);
                int row = 0;
                SSCard.ActiveSheet.RowCount = list.Count + 3;

                for (int i = 0; i < list.Count; i++)
                {
                    row = i + 3;

                    string regDate = DateUtil.DateTimeToStrig(list[i].REGDATE, DateTimeType.YYYY_MM_DD);
                    // 방문일자
                    SSCard.ActiveSheet.Cells[row, 0].Value = regDate.Substring(5, 2) + "." + regDate.Substring(8, 2);

                    SSCard.ActiveSheet.Cells[row, 1].Text = list[i].CERT; //자격

                    SSCard.ActiveSheet.Cells[row, 2].Value = list[i].NAME;
                    SSCard.ActiveSheet.Cells[row, 3].Value = list[i].CONTENT;
                    SSCard.ActiveSheet.Cells[row, 4].Value = list[i].OPINION; //지도사항

                    SSCard.ActiveSheet.Cells[row, 5].Value = "";
                    SSCard.ActiveSheet.Cells[row, 6].Value = "";
                    SSCard.ActiveSheet.Cells[row, 7].Value = "";

                    if (list[i].STATUS == "0")
                    {
                        SSCard.ActiveSheet.Cells[row, 5].Value = "O";
                    }
                    else if (list[i].STATUS == "1")
                    {
                        SSCard.ActiveSheet.Cells[row, 6].Value = "O";
                    }
                    else if (list[i].STATUS == "2")
                    {
                        SSCard.ActiveSheet.Cells[row, 7].Value = "O";
                    }

                    if (list[i].SITESIGN != null)
                    {
                        string[] image = list[i].SITESIGN.Split(',');
                        Image signImage = Base64ToImage(image[1]);
                        FarPoint.Win.Spread.CellType.ImageCellType imageCellType = new FarPoint.Win.Spread.CellType.ImageCellType();
                        imageCellType.Style = FarPoint.Win.RenderStyle.StretchAndScale;
                        SSCard.ActiveSheet.Cells[row, 8].CellType = imageCellType;
                        SSCard.ActiveSheet.Cells[row, 8].Value = signImage;
                        SSCard.ActiveSheet.Cells[row, 8].VerticalAlignment = CellVerticalAlignment.Center;
                    }

                    SSCard.ActiveSheet.Rows[row].Height = SSCard.ActiveSheet.Rows[row].GetPreferredHeight();
                }//for
            }
        }

        private void Clear15()
        {
            SignImage.Image = null;

            int row = 0;
            SSCard.ActiveSheet.RowCount = 30;

            for (int i = 3; i < SSCard.ActiveSheet.RowCount; i++)
            {
                row = i;
                SSCard.ActiveSheet.Cells[row, 0].Value = ""; // 방문일자
                SSCard.ActiveSheet.Cells[row, 1].Value = "";
                SSCard.ActiveSheet.Cells[row, 2].Value = "";
                SSCard.ActiveSheet.Cells[row, 3].Value = "";
                SSCard.ActiveSheet.Cells[row, 4].Value = "";
                SSCard.ActiveSheet.Cells[row, 5].Value = "";
                SSCard.ActiveSheet.Cells[row, 6].Value = "";
                SSCard.ActiveSheet.Cells[row, 7].Value = "";
            }
        }
      

        private void SSList_CellDoubleClick(object sender, CellClickEventArgs e)
        {
            Clear15();

            HC_OSHA_CARD19 dto = SSList.GetRowData(e.Row) as HC_OSHA_CARD19;
            pan19.SetData(dto);
            if (dto.SITESIGN != null)
            {
                SignImage.Image = null;
                string[] image = dto.SITESIGN.Split(',');
                Image signImage = Base64ToImage(image[1]);

                SignImage.Image = signImage;
            }
        }

        private void BtnNew_Click(object sender, EventArgs e)
        {
            pan19.Initialize();
            SignImage.Image = null;

            SetUser();

        }
        private void SetUser()
        {
            HC_USER user = hcUsersRepository.FindOne(CommonService.Instance.Session.UserId);
            Role role = (Role)Enum.Parse(typeof(Role), user.Role);
            if (role == Role.DOCTOR)
            {
                TxtWorkerCount.Text = "의사";
            }
            else if (role == Role.NURSE)
            {
                TxtWorkerCount.Text = "간호사";
            }
            else
            {
                TxtWorkerCount.Text = "산업위생기사";
            }

            TxtTarget.Text = user.Name;

        }
        private void BtnSave_Click(object sender, EventArgs e)
        {
            if (base.SelectedEstimate == null)
            {
                MessageUtil.Info("사업장 계약 정보를 선택하세요");
            }
            else
            {
                if (pan19.Validate<HC_OSHA_CARD19>())
                {
                    HC_OSHA_CARD19 dto = pan19.GetData<HC_OSHA_CARD19>();
                    dto.ESTIMATE_ID = base.SelectedEstimate.ID;
                    dto.SITE_ID = base.SelectedSite.ID;
                    if (SignImage.Tag != null)
                    {
                        dto.SITESIGN = SignImage.Tag.ToString();
                    }
                    
                    HC_OSHA_CARD19 saved = hcOshaCard19Service.Save(dto);

                    pan19.SetData(saved);
                    pan19.Initialize();

                    Search();
                }
            }
        }

        private void BtnDelete_Click(object sender, EventArgs e)
        {
            HC_OSHA_CARD19 dto = pan19.GetData<HC_OSHA_CARD19>();

            if (dto.ID > 0)
            {
                if (MessageUtil.Confirm("삭제하시겠습니까?") == DialogResult.Yes)
                {
                    hcOshaCard19Service.Delete(dto);
                    pan19.Initialize();

                    Search();
                }
            }
        }

        private void button1_Click(object sender, EventArgs e)
        {
          int row =   SSCard.AddRows();

            
        }

        private void BtnSign_Click(object sender, EventArgs e)
        {
            SignPadForm form = new SignPadForm(false);
            if (form.ShowDialog() == DialogResult.OK)
            {
                //image = "";

                HC_OSHA_CARD19 dto = pan19.GetData<HC_OSHA_CARD19>();
                dto.ESTIMATE_ID = base.SelectedEstimate.ID;
                dto.SITE_ID = base.SelectedSite.ID;
                dto.SITESIGN = form.Base64Image;
                dto.CERT = "서명";
                dto.CONTENT = "서명";
                dto.OPINION = "서명";
                hcOshaCard19Service.Save(dto);

                Search();

                //SetSign(form.Base64Image);

            }
        }
     
        public Image Base64ToImage(string base64String)
        {
            // Convert base 64 string to byte[]
            byte[] imageBytes = Convert.FromBase64String(base64String);
            // Convert byte[] to Image
            using (var ms = new MemoryStream(imageBytes))
            {
                Image image = Image.FromStream(ms);
                return image;
            }
        }

        private void BtnSiteSign_Click(object sender, EventArgs e)
        {
            HC_OSHA_CARD19 dto = pan19.GetData<HC_OSHA_CARD19>();
            if (dto.ID > 0)
            {
                SignPadForm form = new SignPadForm(false);
                if (form.ShowDialog() == DialogResult.OK)
                {
                    //image = "";

                    SignImage.Image = null;
                    string[] image = form.Base64Image.Split(',');
                    Image signImage = Base64ToImage(image[1]);

                    SignImage.Image = signImage;
                    SignImage.Tag = form.Base64Image;

                    BtnSave.PerformClick();
                }
            }
            else
            {
                MessageUtil.Alert("저장된 일지를 선택하세요");
            }
        
        }

        private void BtnTargetMacro_Click(object sender, EventArgs e)
        {
            MacrowordForm form = new MacrowordForm("CardPage_15_Form", TxtTargetCOunt, TxtRemark);
            form.Show();
        }

        private void BtnSearch_Click(object sender, EventArgs e)
        {
            Search("LIST");
        }

        private void BtnPrintSearch_Click(object sender, EventArgs e)
        {
            Search("PRINT");
        }
    }


}
