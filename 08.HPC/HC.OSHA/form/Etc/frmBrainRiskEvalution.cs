...
namespace HC_OSHA.form.Etc
{
    ...
    public partial class frmBrainRiskEvalution : CommonForm, ISelectSite
    {
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            string xx =     DtpStartDate.GetValue().Substring(4, 6) ;
            DtpStartDate.SetValue(CboYear.Text + xx);
            Search();
        }

        private void frmBrainRiskEvalution_Load(object sender, EventArgs e)
        {
            TxtSiteIdOrName.SetExecuteButton(BtnSearchSite);
            DateTime sysDate = codeService.CurrentDate;

            DtpStartDate.SetValue(sysDate.AddMonths(-4));
            DtpEndDate.SetValue(sysDate);

            DateTime dtm = sysDate;
            for (int i = 0; i < 10; i++)
            {
                CboYear.Items.Add(dtm.ToString("yyyy"));
                dtm = dtm.AddYears(-1);
            }
            ...
            SSList.AddColumnText("주민등록", nameof(BranRiskEvalutionModel.COL57), 63, IsReadOnly.Y, new SpreadCellTypeOption { IsSort = false });
        ...
        private void Search()
        {
            try
            {
                ...
                if (SelectedSite != null)
                {
                    ...
                    string startDate = DtpStartDate.GetValue();
                    string endDate = DtpEndDate.GetValue();
                    bool isMore = ChkMore.Checked;
