...
namespace HC_OSHA.form.Visit
{
    ...
    public partial class VIsitDocumentBatchForm : CommonForm
    {
            ...
            for(int i=0; i<list.Count; i++)
            {
                if (nOldSiteId == -1 || nOldSiteId != list[i].SiteId)
                {
                    ...
                    if (bOK == true)
                    {
                        ...
                        for (int j = 1; j <= 5; j++)
                        {
                            if (VB.Pstr(strList, ",", j) != "")
                            {
                                k = Int32.Parse(VB.Pstr(strList, ",", j));
                                if (!list[k].visitPlace.IsNullOrEmpty())
                                {
                                    strVisit += list[k].VisitReserveDate.Substring(5) + " " + list[k].visitPlace.Trim() + ",";
                                }
                            }
                        }
        ...
        {
            bool bOk = SSList.SaveExcel("c:\\temp\\방문일정_공문발송.xls", FarPoint.Excel.ExcelSaveFlags.SaveCustomColumnHeaders);
            {
                if (bOk == true)
