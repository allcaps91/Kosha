...
namespace HC_OSHA
{
    public partial class FrmExcelUpload4 : Form
    {
        private int[] FnCol = new int[1000];

        public FrmExcelUpload4()
        {
            InitializeComponent();
            Screen_Set();
        }
        ...
        private void btnJob5_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < SSConv_Sheet1.RowCount; i++)
            {
                ...
                for (int j = 0; j < SSExcel_Sheet1.ColumnCount; j++)
                {
                    ...
                    if (bOK == false && strHead == "작업명")
                    {
                        if (strData == "지급대상 작업명") bOK = true;
                        if (strData == "지급대상작업명") bOK = true;
                        if (strData == "공정명") bOK = true;
                    }
                    if (bOK == false && strHead == "근로자수")
                    {
                        if (strData == "지급대상근로자수") bOK = true;
                        if (strData == "지급대상 근로자수") bOK = true;
                        if (strData == "지급대상") bOK = true;
                    }
                    if (bOK == false && strHead == "변동사항")
                    {
                        if (strData == "참고사항") bOK = true;
                    }
        ...
        private void btnJob3_Click(object sender, EventArgs e)
        {
            ...
            //기존의 자료를 삭제함
            SQL = "DELETE FROM HIC_OSHA_CARD13 ";
            ...
            {
                strName = SS1_Sheet1.Cells[i, 0].Text.ToString().Trim();
                strTARGET = SS1_Sheet1.Cells[i, 1].Text.ToString().Trim();
                strWORKERCOUNT = SS1_Sheet1.Cells[i, 2].Text.ToString().Trim();
                strTARGETCOUNT = SS1_Sheet1.Cells[i, 3].Text.ToString().Trim();
                strPASSNUMBER = SS1_Sheet1.Cells[i, 4].Text.ToString().Trim();
                strREMARK = SS1_Sheet1.Cells[i, 5].Text.ToString().Trim();

                if (strName != "")
                {
                    nID = GET_SeqNextVal("HC_OSHA_CARD_ID_SEQ");

                    SQL = "INSERT INTO HIC_OSHA_CARD13 (ID,SITE_ID,ESTIMATE_ID,YEAR,NAME,";
                    SQL = SQL + " TARGET,WORKERCOUNT,TARGETCOUNT,PASSNUMBER,";
                    SQL = SQL + " REMARK,MODIFIED,MODIFIEDUSER,CREATED, ";
                    SQL = SQL + " CREATEDUSER,SWLICENSE) VALUES (";
                    SQL = SQL +  nID + "," + nLtdCode + "," + nEstimate + ",'";
                    SQL = SQL + strYear + "','" + strName + "','" + strTARGET + "','";
                    SQL = SQL + strWORKERCOUNT + "','" + strTARGETCOUNT + "','";
                    SQL = SQL + strPASSNUMBER + "','" + strREMARK + "',SYSDATE,'";
