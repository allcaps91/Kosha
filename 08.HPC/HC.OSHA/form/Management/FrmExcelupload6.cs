...
namespace HC_OSHA
{
    public partial class FrmExcelupload6 : Form
    {
        private int[] FnCol = new int[1000];

        public FrmExcelupload6()
        {
            InitializeComponent();
            Screen_Set();
        }
            ...
            for (int i = 0; i < SSConv_Sheet1.RowCount; i++)
            {
                ...
                for (int j = 0; j < SSExcel_Sheet1.ColumnCount; j++)
                {
                    ...
                    if (bOK == false && strHead == "공정")
                    {
                        ...
                        if (VB.InStr(strData, "부서") > 0) bOK = true;
                    }
                    if (bOK == false && strHead == "유해인자")
                    {
                        if (VB.InStr(strData, "유해") > 0) bOK = true;
                    }
                    if (bOK == true)
                    {
        ...
        private void btnJob3_Click(object sender, EventArgs e)
        {
            ...
            //기존의 자료를 삭제함
            SQL = "DELETE FROM HIC_OSHA_CARD4_2 ";
            ...
            {
                nRank = i;
                strTASK = SS1_Sheet1.Cells[i, 0].Text.ToString().Trim();
                strTASKUNIT = SS1_Sheet1.Cells[i, 1].Text.ToString().Trim();
                strMSDS = SS1_Sheet1.Cells[i, 2].Text.ToString().Trim();
                strWORKERCOUNT = SS1_Sheet1.Cells[i, 3].Text.ToString().Trim();
                strWORKDESC = SS1_Sheet1.Cells[i, 4].Text.ToString().Trim();
                ...
                if (strMSDS != "")
                {
                    ...
                    SQL = "INSERT INTO HIC_OSHA_CARD4_2 (ID, SITE_ID,ESTIMATE_ID,YEAR,RANK,";
                    SQL = SQL + " TASK,TASKUNIT,MSDS,WORKERCOUNT,WORKDESC,REMARK,";
                    SQL = SQL + " MODIFIED,MODIFIEDUSER,CREATED,CREATEDUSER,SWLICENSE) VALUES (";
                    SQL = SQL + nID + "," + nLtdCode + "," + nEstimate + ",'";
                    SQL = SQL + strYear + "'," + nRank + ",'" + strTASK + "','" + strTASKUNIT + "','";
                    SQL = SQL + strMSDS + "','" + strWORKERCOUNT + "','" + strWORKDESC + "','" + strREMARK + "',";
