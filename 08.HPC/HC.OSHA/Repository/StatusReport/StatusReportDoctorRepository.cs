...
namespace HC.OSHA.Repository.StatusReport
{
    public class StatusReportDoctorRepository : BaseRepository
    {
        ...
        public StatusReportDoctorDto FindLast(long siteId)
        {
            MParameter parameter = CreateParameter();
            parameter.AppendSql("SELECT * FROM HIC_OSHA_REPORT_DOCTOR ");
            parameter.AppendSql("WHERE SITE_ID = :SITE_ID AND VISITDATE = (SELECT MAX(VISITDATE) FROM HIC_OSHA_REPORT_DOCTOR    ");
        ...
        public List<VisitDateModel> FindAll(long siteId)
        {
            ...
            parameter.AppendSql("SELECT A.ID, To_DATE(A.VISITDATE, 'YYYY-MM-DD') AS VISITDATE, B.NAME, SUBSTR(A.VISITDATE,0,4)  AS VISITYEAR FROM HIC_OSHA_REPORT_DOCTOR A   ");
        ...
        public List<VisitDateModel> FindVisitDate(long siteId, string year)
        {
            ...
            parameter.AppendSql("SELECT VISITDATE, ID FROM HIC_OSHA_REPORT_DOCTOR                      ");
