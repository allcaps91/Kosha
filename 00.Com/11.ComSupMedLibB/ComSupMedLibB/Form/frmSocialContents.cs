...
{
    public partial class frmSocialContents : Form
    {
        public frmSocialContents()
        {
            InitializeComponent();
        }

        private void frmSocialContents_Load(object sender, EventArgs e)
        {
            GetData();
                ...
                for (i = 0; i < ssView_Sheet1.RowCount; i++)
                {
                    ...
                    if (strFlag == "True")
                    {
                        if (strRowid != "")
                        {
                            ...
                            SQL = " DELETE KOSMOS_ADM.SOCIAL_CONTENTS                                                  \r\n";
                    ...
                    else
                    {
                        ...
                        {
                            SQL = "";
                            SQL = "INSERT INTO KOSMOS_ADM.SOCIAL_CONTENTS";
                            SQL = SQL + ComNum.VBLF + "(";
                            SQL = SQL + ComNum.VBLF + "     SEQNO, REQDAY, REQUESTER, PTNAME, SEX, AGE, DISNAME, DRNAME, PHONENO, REQSAYU, CONTENTS, GUBUN, REGDAY, SABUN";
                            SQL = SQL + ComNum.VBLF + ")";
                        ...
                        {
                            SQL = "";
                            SQL = "UPDATE KOSMOS_ADM.SOCIAL_CONTENTS SET";
                            SQL = SQL + ComNum.VBLF + "  SEQNO =   '" + ssView_Sheet1.Cells[i, 1].Text + "',";
                            SQL = SQL + ComNum.VBLF + "  REQDAY =   '" + ssView_Sheet1.Cells[i, 2].Text + "',";
                            SQL = SQL + ComNum.VBLF + "  REQUESTER =   '" + ssView_Sheet1.Cells[i, 3].Text + "',";
                            ...
                            SQL = SQL + ComNum.VBLF + "  DISNAME =   '" + ssView_Sheet1.Cells[i, 7].Text + "',";
                            SQL = SQL + ComNum.VBLF + "  DRNAME =   '" + ssView_Sheet1.Cells[i, 8].Text + "',";
                            SQL = SQL + ComNum.VBLF + "  PHONENO =   '" + ssView_Sheet1.Cells[i, 9].Text + "',";
                            SQL = SQL + ComNum.VBLF + "  REQSAYU =  '" + ssView_Sheet1.Cells[i, 10].Text + "',";
                            SQL = SQL + ComNum.VBLF + "  CONTENTS =   '" + ssView_Sheet1.Cells[i, 11].Text + "',";
                            SQL = SQL + ComNum.VBLF + "  GUBUN =   '" + ssView_Sheet1.Cells[i, 12].Text + "',";
        ...
        private void GetData()
        {
            ...
            {
                SQL = "";
                SQL = "SELECT SEQNO, REQDAY, REQUESTER, PTNAME, SEX, AGE, DISNAME, DRNAME, PHONENO, REQSAYU, CONTENTS, GUBUN, REGDAY, SABUN, ROWID";
                SQL = SQL + ComNum.VBLF + "  FROM KOSMOS_ADM.SOCIAL_CONTENTS";
                SQL = SQL + ComNum.VBLF + " WHERE (REQDAY >= '" + dtpSdate.Value.ToString("yyyy-MM-dd") + "'";
                SQL = SQL + ComNum.VBLF + "      AND REQDAY <=  TO_DATE('" + dtpEdate.Value.ToString("yyyy-MM-dd") + " 23:59','YYYY-MM-DD HH24:MI'))";
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    for (i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        ssView_Sheet1.Cells[i, 2].Text = dt.Rows[i]["REQDAY"].ToString().Trim();
                        ssView_Sheet1.Cells[i, 3].Text = dt.Rows[i]["REQUESTER"].ToString().Trim();
                        ...
                        ssView_Sheet1.Cells[i, 7].Text = dt.Rows[i]["DISNAME"].ToString().Trim();
                        ssView_Sheet1.Cells[i, 8].Text = dt.Rows[i]["DRNAME"].ToString().Trim();
                        ssView_Sheet1.Cells[i, 9].Text = dt.Rows[i]["PHONENO"].ToString().Trim();
                        ssView_Sheet1.Cells[i, 10].Text = dt.Rows[i]["REQSAYU"].ToString().Trim();
                        ssView_Sheet1.Cells[i, 11].Text = dt.Rows[i]["CONTENTS"].ToString().Trim();
                        ssView_Sheet1.Cells[i, 12].Text = dt.Rows[i]["GUBUN"].ToString().Trim();
