...
namespace ComPmpaLibB
{
    ...
    /// File Name       : frmPmpaViewForeinTong.cs
    /// Description     : 입원 / 외래 진료비 수납 내역 조회
    /// Author          : 안정수
    /// Create Date     : 2017-10-06
    /// Update History  : 
    /// <history>       
    /// d:\psmh\OPD\jengsan\Frm진료비수납내역서_2012.frm(Frm진료비수납내역서_2012) => frmPmpaViewForeinTong.cs 으로 변경함
    /// </history>
    /// <seealso>
    /// d:\psmh\OPD\jengsan\Frm진료비수납내역서_2012.frm(Frm진료비수납내역서_2012)
    /// </seealso>
    /// </summary>
    public partial class frmPmpaViewForeinTong : Form, MainFormMessage
    {
        ...
        public frmPmpaViewForeinTong()
        {
            InitializeComponent();
            setEvent();
        }

        public frmPmpaViewForeinTong(MainFormMessage pform)
        {
            InitializeComponent();
        ...
        public frmPmpaViewForeinTong(int GnJobSabun)
        {
            InitializeComponent();
            setEvent();
        ...
        public frmPmpaViewForeinTong(string GstrJobName)
        {
            InitializeComponent();
            setEvent();
        ...
        void Set_Combo()
        {
            ...
            SQL += ComNum.VBLF + "      AND DEPTCODE NOT IN ('HD','ER','II','R6','TO','HR','PT','MD')   ";
        ...
        void eGetData()
        {
            ...
            else
            {
                ...
                if (optIO1.Checked == true)
                {
                    ...
                    for (i = 0; i < nREAD; i++)
                    {
                        ...
                        if (chkView.Checked == true)
                        {
                            SQL = "";
                            SQL += ComNum.VBLF + "SELECT                                                                                    ";
                            SQL += ComNum.VBLF + "  PANO,DEPTCODE,BI,TO_CHAR(ACTDATE,'YYYY-MM-DD') ACTDATE,TO_CHAR(bDATE,'YYYY-MM-DD') bDATE,PART                       ";
                            SQL += ComNum.VBLF + "FROM " + ComNum.DB_PMPA + "OPD_SLIP                                                       ";
                            SQL += ComNum.VBLF + "WHERE 1=1                                                                                 ";
                            SQL += ComNum.VBLF + "      AND PANO ='" + txtPano.Text + "'                                                    ";
                            ...
                            SQL += ComNum.VBLF + "      AND BI = '" + strBi + "'";
                            SQL += ComNum.VBLF + "      AND DEPTCODE = '" + strDept + "'";
                            SQL += ComNum.VBLF + "      AND TRIM(SUNEXT) NOT IN ( SELECT CODE FROM KOSMOS_PMPA.BAS_BCODE WHERE GUBUN ='원무영수제외코드')"; //저가약제 제외코드 2011-04-01                            
                        ...
                        if (strBDATE == clsPmpaPb.GstrSysDate && nREAD2 > 0)
                        {
                            ...
                            if (dt1.Rows.Count > 0)
                            {
                                ...
                                {
                                    data[13] = (VB.Val(data[13]) + Convert.ToInt32(VB.Val(dt1.Rows[0]["AMT1"].ToString().Trim())).ToString());
                                    data[45] = (VB.Val(data[45]) + Convert.ToInt32(VB.Val(dt1.Rows[0]["AMT1"].ToString().Trim())).ToString());
                        ...
                        //CT
                        ssList1_Sheet1.Cells[i, 42].Text = clsPmpaType.TRPG.TotAmt6[11].ToString();
                        ssList1_Sheet1.Cells[i, 43].Text = clsPmpaType.TRPG.TotAmt5[11].ToString();
                        ssList1_Sheet1.Cells[i, 44].Text = (clsPmpaType.TRPG.TotAmt2[11] + clsPmpaType.TRPG.TotAmt4[11]).ToString();
                ...
                else
                {
                    ...
                    SQL += ComNum.VBLF + "ORDER BY M.PANO, M.BI, M.DEPTCODE, M.ACTDATE";
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            ssList1_Sheet1.Cells[i, 77].Text = (DRG.GnGs80Amt_J + DRG.GnGs50Amt_J + clsPmpaType.TRPG.TotAmt8[13]).ToString();   //선별 공단 GnGs100Amt
                            ssList1_Sheet1.Cells[i, 78].Text = (DRG.GnGs80Amt_B + DRG.GnGs50Amt_B + clsPmpaType.TRPG.TotAmt9[13]).ToString();   //선별 본인

                            //DRG
                            if (dt.Rows[i]["GBDRG"].ToString().Trim() == "D")
                            {
                                nDrgBonAmt = DRG.GnDrgFoodAmt[0] + DRG.GnDrgFoodAmt[2];
                                nDrgBonAmt += DRG.GnDrgRoomAmt[0] + DRG.GnDrgRoomAmt[2];
        ...
        void eGetData1()
        {
            ...
            if (String.Compare(dtpFDate.Text, Convert.ToDateTime(CurrentDate).AddDays(-1830).ToShortDateString()) <= 0)
            {
                ComFunc.MsgBox("외래 진료비 수납 내역서는 5년까지만 출력이 가능합니다.!!!");
                if (CF.JIN_AMT_PRINT_CHK(clsDB.DbCon, mnJobSabun.ToString().Trim()) == "OK")
                {
                    btnPrint.Enabled = true;
                }
            }
            ...
            #region //사용안함
            if (chkManual.Checked == true) //사용안함
            {
                #region 입원수동데이타, READ_IPD_SU_SLIP(GoSub)
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    for (i = 0; i < nREAD; i++)
                    {
                        ...
                        //Ipd_Tewon_PrtAmt_Gesan_Manual(nTRSNo);
            ...
            if (optIO1.Checked == true)
            {
                ...
                else
                {
                    SQL = "";
                    SQL += ComNum.VBLF + "SELECT";
                    SQL += ComNum.VBLF + "  X.BDATE, X.BI, X.DEPTCODE FROM (";
                    SQL += ComNum.VBLF + "                                  SELECT /*+index(A INDEX_OPDSL4)*/BDATE, BI, DEPTCODE";
                    SQL += ComNum.VBLF + "                                  FROM " + ComNum.DB_PMPA + "OPD_SLIP";
                    SQL += ComNum.VBLF + "                                  WHERE BDATE >= TO_DATE('" + dtpFDate.Text + "','YYYY-MM-DD')";
                    SQL += ComNum.VBLF + "                                  AND BDATE <= TO_DATE('" + dtpTDate.Text + "','YYYY-MM-DD') ";
                    ...
                    SQL += ComNum.VBLF + "                                  GROUP BY BDATE, BI, DEPTCODE";
                    SQL += ComNum.VBLF + "                                  UNION ALL";
                    SQL += ComNum.VBLF + "                                  SELECT BIBDATE, BI, DEPTCODE";
                    SQL += ComNum.VBLF + "                                  FROM " + ComNum.DB_PMPA + "OPD_MASTER";
                    SQL += ComNum.VBLF + "                                  WHERE BDATE >= TO_DATE('" + dtpFDate.Text + "','YYYY-MM-DD')";
                    SQL += ComNum.VBLF + "                                  AND BDATE <= TO_DATE('" + dtpTDate.Text + "','YYYY-MM-DD') ";
                    SQL += ComNum.VBLF + "                                  AND PANO = '" + txtPano.Text + "'";
                ...
                for (i = 0; i < nREAD; i++)
                {
                    ...
                    else
                    {
                        ...
                        SQL += ComNum.VBLF + "      AND BI ='" + strBi + "'";
                        SQL += ComNum.VBLF + "      AND DEPTCODE ='" + strDept + "'";
                        SQL += ComNum.VBLF + "GROUP BY PANO,DEPTCODE,BI,TO_CHAR(ACTDATE,'YYYY-MM-DD'),SEQNO,PART";
                    ...
                    if (dt1.Rows.Count > 0)
                    {
                        for (j = 0; j < dt1.Rows.Count; j++)
                        {
                            strBun = dt1.Rows[i]["BUN"].ToString().Trim();
                            strSelf = dt1.Rows[i]["GBSELF"].ToString().Trim();
                    ...
                    if (strBDATE == clsPmpaPb.GstrSysDate && nREAD2 > 0 && strDept_New != strDept_Old)
                    {
                        ...
                        if (dt1.Rows.Count > 0)
                        {
                            ...
                            //선택진료
                            data[54] = (VB.Val(data[54]) + Convert.ToInt32(dt1.Rows[0]["AMT2"].ToString().Trim())).ToString();

                            data[48] = (VB.Val(data[48]) + Convert.ToInt32(VB.Val(dt1.Rows[0]["AMT1"].ToString().Trim())).ToString());  //진료비총액
                            data[47] = (VB.Val(data[47]) + Convert.ToInt32(VB.Val(dt1.Rows[0]["AMT4"].ToString().Trim())).ToString());  //조합부담금
                            data[50] = (VB.Val(data[50]) + Convert.ToInt32(VB.Val(dt1.Rows[0]["AMT5"].ToString().Trim())).ToString());  //감액
                            data[51] = (VB.Val(data[51]) + Convert.ToInt32(VB.Val(dt1.Rows[0]["AMT6"].ToString().Trim())).ToString());  //미수
                            data[52] = (VB.Val(data[52]) + Convert.ToInt32(VB.Val(dt1.Rows[0]["AMT7"].ToString().Trim())).ToString());  //본인부담액
                            ...
                            if (READ_MISU_GYEPANO(txtPano.Text.Trim(), dtpTDate.Text, strDept, strBi) == "OK")
                            {
                                if (data[52] == "0")
                                {
                                    data[52] = (VB.Val(data[52]) + Convert.ToInt32(VB.Val(dt1.Rows[0]["AMT6"].ToString().Trim())).ToString());
                                }
                            }

                            if (strBi != "43")
                            {
                                data[12] = (VB.Val(data[12]) + Convert.ToInt32(VB.Val(dt1.Rows[0]["AMT1"].ToString().Trim())).ToString());

                                data[44] = (VB.Val(data[44]) + Convert.ToInt32(VB.Val(dt1.Rows[0]["AMT1"].ToString().Trim())).ToString());
                    ...
                    if (dt1.Rows.Count > 0)
                    {
                        ...
                        {
                            //본소스 data(4) = AdoGetString(rs1, "BDATE", 0) & " " & AdoGetString(rs1, "BDATE", RowIndicator - 1)
                            data[4] = dt1.Rows[0]["BDATE"].ToString().Trim() + " " + dt1.Rows[dt1.Rows.Count]["BDATE"].ToString().Trim();
                            data[5] = dt1.Rows.Count.ToString();
                        }
            ...
            else
            {
                ...
                SQL += ComNum.VBLF + "      TO_CHAR(T.actDATE,'YYYY.MM.DD') actDATE, T.ILSU, T.BI, M.ILLCODE1, ";
                SQL += ComNum.VBLF + "      M.ILLCODE2, M.ILLCODE3, M.ILLCODE4, TO_CHAR(M.ACTDATE,'YYYY-MM-DD') ACTDATE,";
                SQL += ComNum.VBLF + "      T.AMT50,T.AMT52, T.AMT53, T.AMT53, T.AMT56, T.AMT57, T.AMT54,T.AMT64,T.AMT44,T.GBDRG,T.DRGCODE ";
                SQL += ComNum.VBLF + "FROM " + ComNum.DB_PMPA + "IPD_NEW_MASTER M, " + ComNum.DB_PMPA + "IPD_TRANS T";
                SQL += ComNum.VBLF + "WHERE 1=1";
                SQL += ComNum.VBLF + "      AND T.ACTDATE >= TO_DATE('" + dtpFDate.Text + "','YYYY-MM-DD')";
                SQL += ComNum.VBLF + "      AND T.ACTDATE <= TO_DATE('" + dtpTDate.Text + "','YYYY-MM-DD')";
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    strDept = dt.Rows[i]["DEPTCODE"].ToString().Trim();
                    nTRSNo = VB.Val(dt.Rows[i]["Trsno"].ToString().Trim());
                    nIPDNO = VB.Val(dt.Rows[i]["IPDNO"].ToString().Trim());
                    strDrgCode = dt.Rows[i]["DRGCODE"].ToString().Trim();
                    ...
                    CI.Ipd_Trans_PrtAmt_Read(clsDB.DbCon, Convert.ToInt64(nTRSNo), "");

                    //TODO : Ipd_Tewon_PrtAmt_Gesan 구현 필요
                    //Ipd_Tewon_PrtAmt_Gesan(nTRSNo, "");
                    ...
                    if (dt.Rows[i]["GBDRG"].ToString().Trim() == "D")
                    {
                        ...
                        //TODO : READ_DRG_AMT_MASTER, IPD_TRANS_PRTAmt_READ_DRG구현 필요
                        //READ_DRG_AMT_MASTER(strDrgCode, clsPmpaType.TIT.Pano, nIPDNO, nTRSNo, strNgt, strInDate, strOutDate);
                        //IPD_TRANS_PRTAmt_READ_DRG(nTRSNo, "");
                ...
                //투약료
                ssList1_Sheet1.Cells[i, 18].Text = (clsPmpaType.TRPG.TotAmt6[4] + clsPmpaType.TRPG.TotAmt6[4]).ToString();
                ssList1_Sheet1.Cells[i, 19].Text = (clsPmpaType.TRPG.TotAmt5[4] + clsPmpaType.TRPG.TotAmt5[4]).ToString();
                ssList1_Sheet1.Cells[i, 20].Text = (clsPmpaType.TRPG.TotAmt2[4] + clsPmpaType.TRPG.TotAmt4[4] + clsPmpaType.TRPG.TotAmt2[5] + clsPmpaType.TRPG.TotAmt4[5]).ToString();
                ...
                //기타
                ssList1_Sheet1.Cells[i, 51].Text = (clsPmpaType.TRPG.TotAmt6[49] + clsPmpaType.TRPG.TotAmt6[13] + clsPmpaType.TRPG.TotAmt6[16]).ToString();
                ssList1_Sheet1.Cells[i, 52].Text = (clsPmpaType.TRPG.TotAmt5[49] + clsPmpaType.TRPG.TotAmt5[13] + clsPmpaType.TRPG.TotAmt5[16]).ToString();
                ssList1_Sheet1.Cells[i, 53].Text = (clsPmpaType.TRPG.TotAmt2[49] + clsPmpaType.TRPG.TotAmt4[49] + clsPmpaType.TRPG.TotAmt2[13] + clsPmpaType.TRPG.TotAmt4[13] + clsPmpaType.TRPG.TotAmt2[16] + clsPmpaType.TRPG.TotAmt4[16]).ToString();
            ...
            if (chkTerm.Checked == true && optIO1.Checked == true)
            {
                SQL = "";
                SQL += ComNum.VBLF + "SELECT";
                SQL += ComNum.VBLF + "  MIN(BDATE) MinBDATE,MAX(BDATE) MaxBDATE";
                SQL += ComNum.VBLF + "FROM " + ComNum.DB_PMPA + "OPD_SLIP";
                SQL += ComNum.VBLF + "WHERE 1=1";
                SQL += ComNum.VBLF + "      AND BDATE >= TO_DATE('" + dtpFDate.Text + "','YYYY-MM-DD')";
                SQL += ComNum.VBLF + "      AND BDATE <= TO_DATE('" + dtpTDate.Text + "','YYYY-MM-DD')";
            ...
            ssList1_Sheet1.Cells[i, 6].Text = strJumin;
            ssList1_Sheet1.Cells[i, 7].Text = strBoHum;

            ssList1_Sheet1.Cells[i, 12].Text = njin[0].ToString();
            ssList1_Sheet1.Cells[i, 13].Text = njin[1].ToString();
            ssList1_Sheet1.Cells[i, 14].Text = njin[2].ToString();
            ssList1_Sheet1.Cells[i, 15].Text = njin[3].ToString();
            ssList1_Sheet1.Cells[i, 16].Text = njin[4].ToString();
            ssList1_Sheet1.Cells[i, 17].Text = njin[5].ToString();
            ssList1_Sheet1.Cells[i, 18].Text = njin[6].ToString();
            ssList1_Sheet1.Cells[i, 19].Text = njin[7].ToString();
            ssList1_Sheet1.Cells[i, 20].Text = njin[8].ToString();

            ssList1_Sheet1.Cells[i, 21].Text = njin[9].ToString();
            ssList1_Sheet1.Cells[i, 22].Text = njin[10].ToString();
            ssList1_Sheet1.Cells[i, 23].Text = njin[11].ToString();
            ssList1_Sheet1.Cells[i, 24].Text = njin[12].ToString();
            ssList1_Sheet1.Cells[i, 25].Text = njin[13].ToString();
            ssList1_Sheet1.Cells[i, 26].Text = njin[14].ToString();
            ssList1_Sheet1.Cells[i, 27].Text = njin[15].ToString();
            ssList1_Sheet1.Cells[i, 28].Text = njin[16].ToString();
            ssList1_Sheet1.Cells[i, 29].Text = njin[17].ToString();
            ssList1_Sheet1.Cells[i, 30].Text = njin[18].ToString();

            ssList1_Sheet1.Cells[i, 31].Text = njin[19].ToString();
            ssList1_Sheet1.Cells[i, 32].Text = njin[20].ToString();
            ssList1_Sheet1.Cells[i, 33].Text = njin[21].ToString();
            ssList1_Sheet1.Cells[i, 34].Text = njin[22].ToString();
            ssList1_Sheet1.Cells[i, 35].Text = njin[23].ToString();
            ssList1_Sheet1.Cells[i, 36].Text = njin[24].ToString();
            ssList1_Sheet1.Cells[i, 37].Text = njin[25].ToString();
            ssList1_Sheet1.Cells[i, 38].Text = njin[26].ToString();
            ssList1_Sheet1.Cells[i, 39].Text = njin[27].ToString();
            ssList1_Sheet1.Cells[i, 40].Text = njin[28].ToString();

            ssList1_Sheet1.Cells[i, 41].Text = njin[29].ToString();
            ssList1_Sheet1.Cells[i, 42].Text = njin[30].ToString();
            ssList1_Sheet1.Cells[i, 43].Text = njin[31].ToString();
            ssList1_Sheet1.Cells[i, 44].Text = njin[32].ToString();
            ssList1_Sheet1.Cells[i, 45].Text = njin[33].ToString();
            ssList1_Sheet1.Cells[i, 46].Text = njin[34].ToString();
            ssList1_Sheet1.Cells[i, 47].Text = njin[35].ToString();
            ssList1_Sheet1.Cells[i, 48].Text = njin[36].ToString();
            ssList1_Sheet1.Cells[i, 49].Text = njin[37].ToString();
            ssList1_Sheet1.Cells[i, 50].Text = njin[38].ToString();

            ssList1_Sheet1.Cells[i, 51].Text = njin[39].ToString();
            ssList1_Sheet1.Cells[i, 52].Text = njin[40].ToString();
            ssList1_Sheet1.Cells[i, 53].Text = njin[41].ToString();
            ssList1_Sheet1.Cells[i, 54].Text = njin[42].ToString();
            ssList1_Sheet1.Cells[i, 55].Text = njin[43].ToString();
            ssList1_Sheet1.Cells[i, 56].Text = njin[44].ToString();
            ssList1_Sheet1.Cells[i, 57].Text = njin[45].ToString();
            ssList1_Sheet1.Cells[i, 58].Text = njin[46].ToString();
            ssList1_Sheet1.Cells[i, 59].Text = njin[47].ToString();
            ssList1_Sheet1.Cells[i, 60].Text = njin[48].ToString();

            ssList1_Sheet1.Cells[i, 61].Text = njin[49].ToString();
            ssList1_Sheet1.Cells[i, 62].Text = njin[50].ToString();
            ssList1_Sheet1.Cells[i, 63].Text = njin[51].ToString();
            ssList1_Sheet1.Cells[i, 64].Text = njin[52].ToString();
            ssList1_Sheet1.Cells[i, 65].Text = njin[53].ToString();
            ssList1_Sheet1.Cells[i, 66].Text = njin[54].ToString();
            ssList1_Sheet1.Cells[i, 67].Text = njin[55].ToString();
            ssList1_Sheet1.Cells[i, 68].Text = njin[56].ToString();
            ssList1_Sheet1.Cells[i, 69].Text = njin[57].ToString();
            ssList1_Sheet1.Cells[i, 70].Text = njin[58].ToString();
        ...
        private void optIO_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (optIO0.Checked == true)
                {
                    ...
                    SQL = "";
                    SQL = "     SELECT to_char(a.jobdate,'yyyy-mm-dd') jobdate , a.pano ,a.sname  , jumin1,jumin3  ";
                    SQL = SQL + ComNum.VBLF + " from TONG_PATIENT a ,bas_patient b where a.pano=b.pano and paclass  ='2'  ";
                    ...
                    SQL = SQL + ComNum.VBLF + " AND substr(jumin2,1,1) > '4' ";
                    SQL = SQL + ComNum.VBLF + " ORDER BY jobdate,pano  ";
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        for (i = 0; i < dt.Rows.Count; i++)
                        {
                            ssList3_Sheet1.Cells[i, 0].Text = dt.Rows[i]["JOBDATE"].ToString().Trim();
                            ssList3_Sheet1.Cells[i, 1].Text = dt.Rows[i]["PANO"].ToString().Trim();
                            ssList3_Sheet1.Cells[i, 2].Text = dt.Rows[i]["SNAME"].ToString().Trim();
                            ssList3_Sheet1.Cells[i, 3].Text = dt.Rows[i]["JUMIN1"].ToString().Trim() + clsAES.DeAES(dt.Rows[i]["JUMIN3"].ToString().Trim());
                        }
                    }
                ...
                if (optIO1.Checked == true)
                {
                    ...
                    SQL = "";
                    SQL = "     SELECT to_char(a.jobdate,'yyyy-mm-dd') jobdate , a.pano ,a.sname  , jumin1,jumin3  ";
                    SQL = SQL + ComNum.VBLF + " from TONG_PATIENT a ,bas_patient b where a.pano=b.pano and paclass  ='1'  ";
                    SQL = SQL + ComNum.VBLF + " AND jobdate >= TO_DATE('" + dtpFDate.Value.ToString("yyyy-MM-dd") + "','YYYY-MM-DD') ";
                    SQL = SQL + ComNum.VBLF + " AND jobdate <= TO_DATE('" + dtpTDate.Value.ToString("yyyy-MM-dd") + "','YYYY-MM-DD') ";
                    SQL = SQL + ComNum.VBLF + " AND substr(jumin2,1,1) > '4' ";
                    SQL = SQL + ComNum.VBLF + " ORDER BY jobdate,pano  ";
        ...
        private void ssList3_CellDoubleClick(object sender, CellClickEventArgs e)
        {
            if (ssList3_Sheet1.RowCount == 0)
                return;
            if (e.ColumnHeader == true)
                return;
            string strPTNO = ssList3_Sheet1.Cells[e.Row, 1].Text.Trim();
            string strPTNAME = ssList3_Sheet1.Cells[e.Row, 2].Text.Trim();

            txtPano.Text = strPTNO;
            txtSName.Text = strPTNAME;
