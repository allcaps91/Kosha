...
namespace ComPmpaLibB
{
    ...
    /// File Name       : frmPmpaMisuilsuNEW2014STS.cs
    /// Description     : 청구 미수금 미수 일수 분석 (2014이후)
    /// Author          : 김효성
    ...
    /// 
    /// </history>
    /// <seealso cref= "psmh\misu\misubs\misubs.vbp(청구미수금분석2014.FRM) >> frmPmpaMisuilsuNEW2014STS.cs 폼이름 재정의" />	
    public partial class frmPmpaMisuilsuNEW2014STS : Form
    {
        string FstrFYYMM = "";
        //string FstrTYYMM = "";
        ...
        public frmPmpaMisuilsuNEW2014STS(string strFYYMM, string strTYYMM, string strSQL, string strDate, string strHelpCode, string strRet)
        {
        ...
        public frmPmpaMisuilsuNEW2014STS()
        {
            InitializeComponent();
        }

        private void frmPmpaMisuilsuNEW2014STS_Load(object sender, EventArgs e)
        {
        ...
        private void btnView_Click(object sender, EventArgs e)
        {
            ...
            {
                ComFunc.MsgBox("2014년 이전은 <공단검진> => <공무원, 사업장, 암검진> 분리 되어집니다.", "2013년 이전은 다른 메뉴를 선택 해주세요");
                return;
            }
        ...
        private void ssView_CellDoubleClick(object sender, FarPoint.Win.Spread.CellClickEventArgs e)
        {
            ...
            {
                case 2: //31 ~ 60 일
                    strFDate = VB.Left((Convert.ToDateTime(strTdate).AddDays(-30)).ToString(), 10);
                    //strTdate = strTdate;
                    break;
                case 3: //61 ~ 90 일
                    strFDate = VB.Left((Convert.ToDateTime(strTdate).AddDays(-60)).ToString(), 10);
                    strTdate = VB.Left((Convert.ToDateTime(strTdate).AddDays(-31)).ToString(), 10);
                    break;
                case 4: //91 ~ 180 일
                    strFDate = VB.Left((Convert.ToDateTime(strTdate).AddDays(-90)).ToString(), 10);
                    strTdate = VB.Left((Convert.ToDateTime(strTdate).AddDays(-61)).ToString(), 10);
                    break;
                case 5: //91 ~ 180 일
                    strFDate = VB.Left((Convert.ToDateTime(strTdate).AddDays(-180)).ToString(), 10);
                    strTdate = VB.Left((Convert.ToDateTime(strTdate).AddDays(-91)).ToString(), 10);
                    break;
                case 6: //91 ~ 180 일
                    strFDate = VB.Left((Convert.ToDateTime(strTdate).AddDays(-365)).ToString(), 10);
                    strTdate = VB.Left((Convert.ToDateTime(strTdate).AddDays(-181)).ToString(), 10);
                    break;
                case 7: //91 ~ 180 일
                    strFDate = VB.Left((Convert.ToDateTime(strTdate).AddDays(-5000)).ToString(), 10);
                    strTdate = VB.Left((Convert.ToDateTime(strTdate).AddDays(-366)).ToString(), 10);
                    break;
            }
            ...
            try
            {
                ...
                else if (e.Row == 6)
                {
                    ...
                    SQL = SQL + ComNum.VBLF + "   AND a.JanAmt <> 0 ";
                    SQL = SQL + ComNum.VBLF + "   AND b.GJONG <> '83'    ";
                    SQL = SQL + ComNum.VBLF + "   AND c.MISUJONG IN ( '1','2','3')    ";      //'성인병
                    SQL = SQL + ComNum.VBLF + "  AND B.BDATE >=TO_DATE('" + strFDate + "','YYYY-MM-DD')";
                    SQL = SQL + ComNum.VBLF + "  AND B.BDATE <= TO_DATE('" + strTdate + "','YYYY-MM-DD')";
                    SQL = SQL + ComNum.VBLF + "ORDER BY a.wrtno,b.BDate ";
                }
                ...
                if (SQL != "")
                {
                    GstrRet = e.Row+1.ToString();
                    frmPmpaViewGiganDetailA frm = new frmPmpaViewGiganDetailA(SQL, GstrRet);
                    frm.Show();
                }
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            strHead1 = "청구미수금 미수일수 분석 " +  cboYYYY.Text + "   ";
