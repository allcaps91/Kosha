...
{
    public partial class frmPmpaSetDeptTel : Form
    {
        clsQuery CQ = new clsQuery();
        ComFunc CF = new ComFunc();

        public frmPmpaSetDeptTel()
        {
            InitializeComponent();
        }

        private void frmPmpaSetDeptTel_Load(object sender, EventArgs e)
        {
            if (ComQuery.isFormAuth(clsDB.DbCon, this) == false) { this.Close(); }       //Form 권한조회
            ComFunc.SetFormInit(clsDB.DbCon, this, "Y", "Y", "Y");                       //Form 기본값 셋팅

            frmPmpaSetDeptTel frm = new frmPmpaSetDeptTel();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < ssList_Sheet1.RowCount; i++)
            {
                ...
                strGubun = ssList_Sheet1.Cells[i, 3].Text.Trim();

                if (strDel == "" && strCode != "")
                {
                    if (strName == "")
                    {
                        ComFunc.MsgBox("전화번호란이 공란입니다..", "확인");
                        return;
                    }
                    if (strGubun == "")
                    {
        ...
        {
            if (e.Column == 0) { ssList_Sheet1.Cells[e.Row, 3].Text = "Y"; }
        }
    }
}
