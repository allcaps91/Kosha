...
/// <summary>
/// Description : 예약검사부도자 환불관리
/// Author : 박병규
/// Create Date : 2018.04.05
/// </summary>
/// <history>
/// </history>
/// <seealso cref="frm예약검사부도관리.frm"/> 

namespace ComPmpaLibB
{
    public partial class frmPmpaEntryExamResFail : Form
    {
        clsSpread CS = null;
        ...
        public frmPmpaEntryExamResFail()
        {
            InitializeComponent();
            setParam();
        }

        public frmPmpaEntryExamResFail(string ArgPtno = "")
        {
            InitializeComponent();
            FstrPtno = ArgPtno;
            setParam();
        ...
        private void Seve_Process(PsmhDb pDbCon)
        {
            ...
            {
                SQL = "";
                SQL += ComNum.VBLF + " UPDATE " + ComNum.DB_PMPA + "OPD_REFUND_EXAM ";
                ...
                SQL = "";
                SQL += ComNum.VBLF + " INSERT INTO " + ComNum.DB_PMPA + "OPD_REFUND_EXAM_HIS";
        ...
        private void eForm_Load(object sender, EventArgs e)
        {
            ...
            frmPmpaEntryExamResFail frm = new frmPmpaEntryExamResFail();
        ...
        private void eForm_Clear()
        {
            ...
            ssList_Sheet1.Columns[2].Visible = false;
        ...
        private void Search_Process(PsmhDb pDbCon)
        {
            ...
            SQL = "";
            SQL += ComNum.VBLF + " SELECT TO_CHAR(ACTDATE,'YYYY-MM-DD') ACTDATE, PANO , DEPTCODE, ";
            SQL += ComNum.VBLF + "        BI, TO_CHAR(YDATE1,'YYYY-MM-DD') YDATE1, TO_CHAR(YDATE3,'YYYY-MM-DD HH24:MI') YDATE3, ";
            SQL += ComNum.VBLF + "        NVL(AMT7, 0) AMT7, NVL(CAMT, 0) CAMT, CSABUN, ";
            SQL += ComNum.VBLF + "        TO_CHAR(RDATE,'YYYY-MM-DD') RDATE, NVL(RAMT, 0) RAMT, RSABUN, ";
            SQL += ComNum.VBLF + "        RREMARK, CARDSEQNO, ROWID";
            SQL += ComNum.VBLF + "   FROM " + ComNum.DB_PMPA + "OPD_REFUND_EXAM ";
            SQL += ComNum.VBLF + "  WHERE 1     = 1 ";
            ...
            {
                ssList_Sheet1.Cells[i, 0].Text = Dt.Rows[i]["ACTDATE"].ToString().Trim();   //부도일자
                ssList_Sheet1.Cells[i, 1].Text = Dt.Rows[i]["DEPTCODE"].ToString().Trim();  //진료과
                ssList_Sheet1.Cells[i, 2].Text = Dt.Rows[i]["YDATE3"].ToString().Trim();    //예약일자
                ssList_Sheet1.Cells[i, 3].Text = string.Format("{0:#,##0}", Convert.ToInt64(Dt.Rows[i]["AMT7"]));      //예약금액
                ssList_Sheet1.Cells[i, 4].Text = string.Format("{0:#,##0}", Convert.ToInt64(Dt.Rows[i]["CAMT"]));      //예약 보관금액
                ssList_Sheet1.Cells[i, 5].Text = Dt.Rows[i]["CSABUN"].ToString().Trim();    //예약 보관사번
                ssList_Sheet1.Cells[i, 6].Text = Dt.Rows[i]["RDATE"].ToString().Trim();     //보관금액 반환일자
                ssList_Sheet1.Cells[i, 7].Text = string.Format("{0:#,##0}", Convert.ToInt64(Dt.Rows[i]["RAMT"]));      //보관금액 반환금액
                ssList_Sheet1.Cells[i, 8].Text = Dt.Rows[i]["RSABUN"].ToString().Trim();    //보관금액 반환사번
                ssList_Sheet1.Cells[i, 9].Text = Dt.Rows[i]["RREMARK"].ToString().Trim();  //보관금액 반환사유
                ssList_Sheet1.Cells[i, 10].Text = Dt.Rows[i]["CARDSEQNO"].ToString().Trim();
        ...
        private void ssList_CellClick(object sender, FarPoint.Win.Spread.CellClickEventArgs e)
        {
            ...
            ssFind_Sheet1.Cells[0, 0].Text = ssList_Sheet1.Cells[e.Row, 2].Text;    //예약일자
            ssFind_Sheet1.Cells[1, 0].Text = ssList_Sheet1.Cells[e.Row, 0].Text;    //부도일자
            ssFind_Sheet1.Cells[2, 0].Text = ssList_Sheet1.Cells[e.Row, 3].Text;    //예약금액
            ssFind_Sheet1.Cells[3, 0].Text = ssList_Sheet1.Cells[e.Row, 4].Text;    //예약 보관금액
            ssFind_Sheet1.Cells[4, 0].Text = ssList_Sheet1.Cells[e.Row, 5].Text;    //예약 보관사번
            ssFind_Sheet1.Cells[5, 0].Text = ssList_Sheet1.Cells[e.Row, 6].Text;    //보관금액 반환일자
            ssFind_Sheet1.Cells[6, 0].Text = ssList_Sheet1.Cells[e.Row, 7].Text;    //보관금액 반환금액
            ssFind_Sheet1.Cells[7, 0].Text = ssList_Sheet1.Cells[e.Row, 8].Text;    //보관금액 반환사번
            ssFind_Sheet1.Cells[8, 0].Text = ssList_Sheet1.Cells[e.Row, 9].Text;   //보관금액 반환사유

            FnRAmt = Convert.ToInt32(VB.Replace(ssList_Sheet1.Cells[e.Row, 7].Text, ",",""));
            FnCardSeqNo = Convert.ToInt32(VB.Replace(ssList_Sheet1.Cells[e.Row, 10].Text, ",", ""));
            FstrRowID = ssList_Sheet1.Cells[e.Row, 11].Text;
            txtRemark.Text = ssList_Sheet1.Cells[e.Row, 9].Text;
