...
using RestSharp;
/// <summary>
/// Description : 신용카드 / 현금영수 승인
/// Author : 김민철
/// Create Date : 2018.02.07
/// </summary>
/// <history>
/// </history>
/// <seealso cref="Frm신용카드승인_다우.frm / Frm현금영수승인_다우.frm "/> 

//TODO : 고유거래번호로 신용카드 승인 취소가 가능하니 구현해줄것.
namespace ComPmpaLibB
{
    public partial class frmPmpaEntryCardDaou : Form
    {
        ...
        private const string MP_Hid = "HSPTL-0001";
        private const string MP_Server = "http://221.157.239.4:8888";
        private const string MP_URL_CANCEL = "/MPService/inside/cancelPay";
        private const string MP_URL_UNREG = "/MPService/inside/unregistMedical";

        private const int OP_TYPE_CANCEL = 1;   // 결제 취소
        private const int OP_TYPE_UNREG = 2;    // 이지라이프 삭제

        private JObject SendHttpReq(int _type, string _hid, string _seq)
        {
            string url = MP_Server;
            switch (_type)
            {
                case OP_TYPE_CANCEL:
                    url += MP_URL_CANCEL;
            ...
            json.Add("h_id", _hid);
            json.Add("app_no", _seq);
            //json.Add("rtn_cd", "0000");
            //json.Add("rtn_msg", "성공");

            request.AddParameter("text/json", json.ToString(), ParameterType.RequestBody);
        ...
        public frmPmpaEntryCardDaou(string strJob, string strIO)
        {
            InitializeComponent();
            SetEvent();
            SetControl();
        ...
        public frmPmpaEntryCardDaou(string ArgPano, string ArgSname, string ArgDept, string IO, long nAmt, string strJob, string strBDate)
        {
            InitializeComponent();
        ...
        void SetEvent()
        {
            ...
            //this.btnRSign.Click                 += new EventHandler(eBtnClick);
            this.btnRSign2.Click                += new EventHandler(eBtnClick);
            //this.btnPead.Click                  += new EventHandler(eBtnClick);
            ...
            //this.timer1.Tick                    += new EventHandler(Display_Sub_Form);
            ...
            //this.chkCash.CheckedChanged         += new EventHandler(eChkChecked);     // 2018-08-02
        ...
        void eFormLoad(object sender, EventArgs e)
        {
            ...
            frmPmpaEntryCardDaou frm = new frmPmpaEntryCardDaou();
            ComFunc.Form_Center(frm);

            cPmpaSpd.sSpd_enmCardApprov(SS1, cPb.sSpdCardApprov, cPb.nSpdCardApprov, 10, 0);
            cSpd.setColStyle(SS1, -1, (int)clsPmpaPb.enmCardApprov.SEL, clsSpread.enmSpdType.Hide);
        ...
        void SetHphone(PsmhDb pDbCon)
        {
            ...
            try
            {
             ...
             //   SQL += "    AND CARDNO <> '0100001234'                      \r\n";
                ...
                if (Dt.Rows.Count > 0)
                {
                    FstrHPhone = Dt.Rows[0]["CARDNO"].ToString().Trim();
                    txtHphone.Text = Dt.Rows[0]["CARDNO"].ToString().Trim();
                }
        ...
        void eTabSelCtrl(object sender, EventArgs e)
        {
            ...
            if (tabCtrl.SelectedTab == tabPage1)
            {
                ...
                cSpd.setColStyle(SS1, -1, (int)clsPmpaPb.enmCardApprov.SEL,         clsSpread.enmSpdType.Hide);
                cSpd.setColStyle(SS1, -1, (int)clsPmpaPb.enmCardApprov.CARDNO,      clsSpread.enmSpdType.Hide);
                cSpd.setColStyle(SS1, -1, (int)clsPmpaPb.enmCardApprov.ORIGINNO2,   clsSpread.enmSpdType.Hide);
                cSpd.setColStyle(SS1, -1, (int)clsPmpaPb.enmCardApprov.FINAME,      clsSpread.enmSpdType.View);
                cSpd.setColStyle(SS1, -1, (int)clsPmpaPb.enmCardApprov.HPAY,        clsSpread.enmSpdType.View);
            ...
            else if (tabCtrl.SelectedTab == tabPage2)
            {
                ...
                cSpd.setColStyle(SS1, -1, (int)clsPmpaPb.enmCardApprov.SEL,         clsSpread.enmSpdType.View);
                cSpd.setColStyle(SS1, -1, (int)clsPmpaPb.enmCardApprov.CARDNO,      clsSpread.enmSpdType.View);
                cSpd.setColStyle(SS1, -1, (int)clsPmpaPb.enmCardApprov.ORIGINNO2,   clsSpread.enmSpdType.View);
                cSpd.setColStyle(SS1, -1, (int)clsPmpaPb.enmCardApprov.FINAME,      clsSpread.enmSpdType.Hide);
                cSpd.setColStyle(SS1, -1, (int)clsPmpaPb.enmCardApprov.HPAY,        clsSpread.enmSpdType.Hide);
        ...
        void eSpdDblClick(object sender, CellClickEventArgs e)
        {
            ...
            if (SS1.ActiveSheet.Cells[e.Row, (int)clsPmpaPb.enmCardApprov.HPAY].Text == "Y" && FstrJob == "CARD")
            {
                chkPay.Checked = true;
            }
            else
            ...
            if (cboDept.FindString(SS1.ActiveSheet.Cells[e.Row, (int)clsPmpaPb.enmCardApprov.DEPTCODE].Text, -1) > 0)
            {
                cboDept.SelectedIndex = cboDept.FindString(SS1.ActiveSheet.Cells[e.Row, (int)clsPmpaPb.enmCardApprov.DEPTCODE].Text, -1);
            }
            else
            {
                cboDept.SelectedIndex = 0;
            ...
            if (cboIO.FindString(SS1.ActiveSheet.Cells[e.Row, (int)clsPmpaPb.enmCardApprov.GBIO].Text, -1) > 0)
            {
                cboIO.SelectedIndex = cboIO.FindString(SS1.ActiveSheet.Cells[e.Row, (int)clsPmpaPb.enmCardApprov.GBIO].Text, -1);
            }
            else
            {
                cboIO.SelectedIndex = 0;
            ...
            if (FstrJob == "CASH")
            {
                txtHphone.Text      = SS1.ActiveSheet.Cells[e.Row, (int)clsPmpaPb.enmCardApprov.CARDNO].Text;
                txtOrNo2.Text       = SS1.ActiveSheet.Cells[e.Row, (int)clsPmpaPb.enmCardApprov.ORIGINNO].Text;
                txtOrDate2.Text     = SS1.ActiveSheet.Cells[e.Row, (int)clsPmpaPb.enmCardApprov.TRANDATE].Text;
                txtCashAmt.Text     = SS1.ActiveSheet.Cells[e.Row, (int)clsPmpaPb.enmCardApprov.TRADEAMT].Text;
            }
            else
            {
                txtOrNo.Text        = SS1.ActiveSheet.Cells[e.Row, (int)clsPmpaPb.enmCardApprov.ORIGINNO].Text;
                txtOrDate.Text      = SS1.ActiveSheet.Cells[e.Row, (int)clsPmpaPb.enmCardApprov.TRANDATE].Text;
                txtCardAmt.Text     = SS1.ActiveSheet.Cells[e.Row, (int)clsPmpaPb.enmCardApprov.TRADEAMT].Text;
                txtDivideTerm.Text  = SS1.ActiveSheet.Cells[e.Row, (int)clsPmpaPb.enmCardApprov.DIV].Text;
            }
            
            FstrRowid = SS1.ActiveSheet.Cells[e.Row, (int)clsPmpaPb.enmCardApprov.ROWID].Text;
            if (FstrJob == "CASH")
            {
            ...
            //이미 취소된건은 취소불가
            if (SS1.ActiveSheet.Cells[e.Row, (int)clsPmpaPb.enmCardApprov.ORIGINNO2].Text == "")
            {
                if (FstrJob == "CARD")
                { 
                ...
                else if (FstrJob == "CASH")
                {
                    ...
                    txtOrNo2.Text = SS1.ActiveSheet.Cells[e.Row, (int)clsPmpaPb.enmCardApprov.ORIGINNO].Text;
                    FstrRowid = SS1.ActiveSheet.Cells[e.Row, (int)clsPmpaPb.enmCardApprov.ROWID].Text;
                    if (SS1.ActiveSheet.Cells[e.Row, (int)clsPmpaPb.enmCardApprov.INPUTMETHOD].Text == "1")
                    {
                        rdoGbn1.Checked = true;
                    }
                    else if (SS1.ActiveSheet.Cells[e.Row, (int)clsPmpaPb.enmCardApprov.INPUTMETHOD].Text == "2")
                    {
                        rdoGbn2.Checked = true;
                    }
                    else if(SS1.ActiveSheet.Cells[e.Row, (int)clsPmpaPb.enmCardApprov.INPUTMETHOD].Text == "3")
                    {
                        rdoGbn3.Checked = true;
                    }
            ...
            {
                if (SS1.ActiveSheet.Cells[e.Row, (int)clsPmpaPb.enmCardApprov.ORIGINNO].Text != SS1.ActiveSheet.Cells[e.Row, (int)clsPmpaPb.enmCardApprov.ORIGINNO2].Text)
                {
        ...
        void DisPlay_Card_Approv(PsmhDb pDbCon, FpSpread Spd, string ArgPano, string ArgDept, string ArgIO, string ArgGb, bool chkMenual)
        {
            ...
            try
            {
                ...
                SQL += ComNum.VBLF + "        DECODE(HPay,'1','Y','') HPay,                                     ";
                SQL += ComNum.VBLF + "        DECODE(SEL,'0','Y','','Y','') SEL,                                ";
                SQL += ComNum.VBLF + "        DECODE(PtGubun,'3','','4','모바일','코세스') PtGubun, IC, ROWID,  ";
                SQL += ComNum.VBLF + "        INSTPERIOD  ,INPUTMETHOD                                          "; 
                ...
                if (nRead > 0)
                {
                    for (i = 0; i < nRead; i++)
                    {
                        ...
                        Spd.ActiveSheet.Cells[nRow - 1, (int)clsPmpaPb.enmCardApprov.TRANDATE].Text     = Dt.Rows[i]["TranDate"].ToString().Trim();
                        Spd.ActiveSheet.Cells[nRow - 1, (int)clsPmpaPb.enmCardApprov.TRADEAMT].Text     = VB.Val(Dt.Rows[i]["TradeAmt"].ToString()).ToString("###,###,###");
                        Spd.ActiveSheet.Cells[nRow - 1, (int)clsPmpaPb.enmCardApprov.DEPTCODE].Text     = Dt.Rows[i]["DeptCode"].ToString().Trim();
                        Spd.ActiveSheet.Cells[nRow - 1, (int)clsPmpaPb.enmCardApprov.GBIO].Text         = Dt.Rows[i]["GbIO"].ToString().Trim();
                        Spd.ActiveSheet.Cells[nRow - 1, (int)clsPmpaPb.enmCardApprov.FINAME].Text       = Dt.Rows[i]["FINAME"].ToString().Trim();
                        Spd.ActiveSheet.Cells[nRow - 1, (int)clsPmpaPb.enmCardApprov.CARDNO].Text       = Dt.Rows[i]["CARDNO"].ToString().Trim();
                        Spd.ActiveSheet.Cells[nRow - 1, (int)clsPmpaPb.enmCardApprov.ORIGINNO].Text     = Dt.Rows[i]["OriginNo"].ToString().Trim();
                        Spd.ActiveSheet.Cells[nRow - 1, (int)clsPmpaPb.enmCardApprov.ORIGINNO2].Text    = Dt.Rows[i]["OriginNo2"].ToString().Trim();
                        Spd.ActiveSheet.Cells[nRow - 1, (int)clsPmpaPb.enmCardApprov.PTGUBUN].Text      = Dt.Rows[i]["PtGubun"].ToString().Trim();
                        Spd.ActiveSheet.Cells[nRow - 1, (int)clsPmpaPb.enmCardApprov.SEL].Text          = Dt.Rows[i]["SEL"].ToString().Trim();
                        Spd.ActiveSheet.Cells[nRow - 1, (int)clsPmpaPb.enmCardApprov.TRANHEADER].Text   = Dt.Rows[i]["TranHeader"].ToString().Trim();
                        Spd.ActiveSheet.Cells[nRow - 1, (int)clsPmpaPb.enmCardApprov.HPAY].Text         = Dt.Rows[i]["HPAY"].ToString().Trim();
                        Spd.ActiveSheet.Cells[nRow - 1, (int)clsPmpaPb.enmCardApprov.IC].Text           = Dt.Rows[i]["IC"].ToString().Trim();
                        Spd.ActiveSheet.Cells[nRow - 1, (int)clsPmpaPb.enmCardApprov.ROWID].Text        = Dt.Rows[i]["ROWID"].ToString().Trim();
                        Spd.ActiveSheet.Cells[nRow - 1, (int)clsPmpaPb.enmCardApprov.DIV].Text          = Dt.Rows[i]["INSTPERIOD"].ToString().Trim();
                        Spd.ActiveSheet.Cells[nRow - 1, (int)clsPmpaPb.enmCardApprov.INPUTMETHOD].Text  = Dt.Rows[i]["INPUTMETHOD"].ToString().Trim();

                        if (Spd.ActiveSheet.Cells[nRow - 1, (int)clsPmpaPb.enmCardApprov.ORIGINNO2].Text != "")
                        {
                            if (Spd.ActiveSheet.Cells[nRow - 1, (int)clsPmpaPb.enmCardApprov.ORIGINNO].Text != Spd.ActiveSheet.Cells[nRow - 1, (int)clsPmpaPb.enmCardApprov.ORIGINNO2].Text)
                            {
                                cSpd.setSpdForeColor(Spd, nRow - 1, 0, nRow - 1, Spd.ActiveSheet.ColumnCount - 1, Color.Red);
                            }
                        }

                        if (Spd.ActiveSheet.Cells[nRow - 1, (int)clsPmpaPb.enmCardApprov.PTGUBUN].Text == "모바일")
                        {

                            cSpd.setSpdForeColor(Spd, nRow - 1, 0, nRow - 1, Spd.ActiveSheet.ColumnCount - 1, Color.Blue);
                ...
                //    nColCnt = (int)clsPmpaPb.enmCardApprov.ORIGINNO;
                //    strFind = Spd.ActiveSheet.Cells[i, (int)clsPmpaPb.enmCardApprov.ORIGINNO2].Text;

                //    Spd.Search(0, strFind, false, false, false, false, true, false, false, 0, nColCnt, ref nRowCnt, ref nColCnt);

                //    if (nRowCnt != Spd.ActiveSheet.RowCount - 1)
                //    {
                //        cSpd.setSpdForeColor(Spd, i, 0, i, Spd.ActiveSheet.ColumnCount - 1, Color.Red);
                //    }
                //}
            }
            catch (Exception ex)
        ...
        void Approval_Data(PsmhDb pDbCon)
        {
            ...
            if (VB.Left(cboIO.Text, 1) != "I" && VB.Left(cboIO.Text, 1) != "O" && VB.Left(cboIO.Text, 1) != "*")
            {
                ComFunc.MsgBox("입원외래 구분오류 .", "확인");
                return;
            }
            ...
            {
                ComFunc.MsgBox("외래/입원 구분이 없습니다.", "확인");
                return;

            }
            if (FstrJob == "CARD" && chkOG.Checked)
            {
                if (txtDivideTerm.Text.Trim() !="38")
                {   
                    if (ComFunc.MsgBoxQ("산전승인은 할부개월 38 로 세팅해야함..이대로 하시겠습니까??", "확인", MessageBoxDefaultButton.Button2) == DialogResult.No)
                    {
                        return;
                    }
                }
            ...
            if (isConnected == true)
            {
                ...
                if (FstrJob == "CARD")
                {
                    ...
                    clsPmpaType.RD.HPay = chkPay.Checked == true ? "1" : "0";                           //모바일 페이
                }
                else if (FstrJob == "CASH")
                {
                    clsPmpaType.RSD.Gubun = "2";                                                        //현금영수증
                    clsPmpaType.RSD.CardData = txtHphone.Text;                                          //현금영수증 번호
        ...
        void Card_Approv_Data_Set(string strRD)
        {
            ...
            try
            {
                if (CARD.CardApprov_Insert(clsDB.DbCon, FstrJob) == false)
                {
                    clsDB.setRollbackTran(clsDB.DbCon);
                    Cursor.Current = Cursors.Default;
                    return;
                ...
                if (m_cardApproveGbn == "2")
                {
                    if (CARD.UpDate_Card_Refund(clsDB.DbCon, FstrRowid) == false)
                    {
                        clsDB.setRollbackTran(clsDB.DbCon);
                        Cursor.Current = Cursors.Default;
                        return;
        ...
        void Read_Card_Approv_Data(PsmhDb pDbCon, string argPano, string argOrigin)
        {
            ...
            Dt = CARD.sel_CardApprov_Data(pDbCon, argPano, argOrigin);
            ...
            if (Dt.Rows.Count > 0)
            {
                ...
                Card.CVar.gstrCdSName   = Dt.Rows[0]["SNAME"].ToString().Trim();
                Card.CVar.gstrCdPtno    = Dt.Rows[0]["PANO"].ToString().Trim();
                clsPmpaType.RSD.TotAmt      = Convert.ToInt64(Dt.Rows[0]["TRADEAMT"].ToString());   //거래금액
                clsPmpaType.RD.BankName     = Dt.Rows[0]["AccepterName"].ToString().Trim();         //매입사명
                clsPmpaType.RD.PublishBank  = Dt.Rows[0]["FINAME"].ToString().Trim();               //발급사명
                if (Dt.Rows[0]["INSTPERIOD"].ToString().Trim() != "")
                {
                    clsPmpaType.RSD.CardDivide = Convert.ToInt32(Dt.Rows[0]["INSTPERIOD"].ToString()); //할부개월수
                }
                clsPmpaType.RD.ApprovalNo   = Dt.Rows[0]["ORIGINNO"].ToString().Trim();             //승인번호
                clsPmpaType.RD.MDate        = Dt.Rows[0]["APPROVDATE"].ToString().Trim();           //거래일시
                if (Dt.Rows[0]["TRANHEADER"].ToString().Trim() == "2")
                {
                    FstrGubun = "카드취소";
                }
                else
                ...
                if (FstrJob == "CASH")
                {
                    clsPmpaType.RSD.CardData = Dt.Rows[0]["CardNo"].ToString().Trim();              //휴대폰번호
                }
            }
        ...
        void frmPmpaEntryCardDaou_FormClosing(object sender, FormClosingEventArgs e)
        {
            DisConnect_Port();
        }
        ...
        void Card_Print()
        {
            ...
            if (clsPrint.gGetPrinterFind("신용카드") == false)
            {
            ...
            pd.PrinterSettings.PrinterName = "신용카드";
        ...
        //2018.06.14 박병규 : 산전승인 체크시 할부개월 38 Defaulut 설정
        private void chkOG_Click(object sender, EventArgs e)
        {
            if (chkOG.Checked == true)
                txtDivideTerm.Text = "38";
            else
                txtDivideTerm.Text = "00";
