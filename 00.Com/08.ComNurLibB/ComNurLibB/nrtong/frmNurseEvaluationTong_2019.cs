...
namespace ComNurLibB
{
    ...
    /// //2019-08-01 2019년 기준으로 변경함
    /// </history>
    /// <seealso cref= "\nurse\nrtong\Frm간호간병통합평가표통계.frm(Frm간호간병통합평가표통계) >> frmNurseEvaluationTong.cs 폼이름 재정의" />
    public partial class frmNurseEvaluationTong_2019 : Form, MainFormMessage
    {
        #region 클래스 선언 및 etc....
        ...
        public frmNurseEvaluationTong_2019(MainFormMessage pform)
        {
            InitializeComponent();
        ...
        public frmNurseEvaluationTong_2019()
        {
            InitializeComponent();
            setEvent();
        }
        ...
        void eBtnClick(object sender, EventArgs e)
        {
            ...
            else if (sender == this.btnGraph)
            {
                ...
                double[] nVal1 = new double[19];
                double[] nVal2 = new double[19];
                double[] nVal3 = new double[19];
                ...
                frmNurseEvaluationGraph_2019 frmNurseEvaluationGraphX = new frmNurseEvaluationGraph_2019(nVal1, nVal2, nVal3, strTitle);
        ...
        void Set_Init()
        {
            ...
            if (clsType.User.Sabun != "48265" && clsType.User.JobGroup != "JOB013053" && clsType.User.Sabun != "22231" && clsType.User.Sabun  != "33629")
            {
        ...
        void eBtnCheck()
        {
            ...
            SQL = " INSERT INTO KOSMOS_PMPA.NUR_TOTALCARE_NOTE2_DEL ";
            SQL += ComNum.VBLF + " SELECT A.* ";
            SQL += ComNum.VBLF + "  FROM KOSMOS_PMPA.NUR_TOTALCARE_NOTE2 A, (SELECT MAX(ENTDATE) ENTDATE, PANO, JOBTIME ";
            SQL += ComNum.VBLF + "                                           FROM KOSMOS_PMPA.NUR_TOTALCARE_NOTE2 ";
            ...
            SQL = " DELETE KOSMOS_PMPA.NUR_TOTALCARE_NOTE2 ";
        ...
        void GetCount()
        {
            ...
            SQL += ComNum.VBLF + "   FROM NUR_MASTER A, KOSMOS_PMPA.NUR_TOTALCARE_NOTE2 B, BAS_PATIENT C ,IPD_NEW_MASTER D";//;, TONG_PATIENT E";
        ...
        void eGetData()
        {
            ...
            SQL += ComNum.VBLF + "decode(b.wardcode,'40','C002','60','C001','73','C003','75','C004','C001') wardcode,b.wardcode||'병동' wardname, ";
            ...
            SQL += ComNum.VBLF + " nvl(B.BUN01,0) BUN01, nvl(B.BUN02,0) BUN02, nvl(decode(B.BUN03,'2','1',B.BUN03),0) BUN03, nvl(B.BUN04,0) BUN04, ";
            SQL += ComNum.VBLF + " nvl(B.BUN05,0) BUN05, nvl(B.BUN06,0) BUN06, nvl(B.BUN07,0) BUN07, nvl(B.BUN08,0) BUN08, ";
            SQL += ComNum.VBLF + " nvl(B.BUN09,0) BUN09, nvl(decode(B.BUN10,'2','1',B.BUN10),0) BUN10, NVL(B.BUN11,0) BUN11, nvl(decode(B.BUN12,'2','1',B.BUN12),0) BUN12, ";
            SQL += ComNum.VBLF + " nvl(decode(B.BUN13,'2','1',B.BUN13),0) BUN13, NVL(B.BUN14,0) BUN14, nvl(decode(B.BUN15,'2','1',B.BUN15),0) BUN15, NVL(B.BUN16,0) BUN16, ";
            SQL += ComNum.VBLF + " NVL(B.BUN17,0) BUN17, NVL(B.BUN18,0) BUN18, NVL(B.BUN19,0) BUN19, ";
            SQL += ComNum.VBLF + " VALUE_OP, VALUE_ROOMLV , NVL(VALUE_FALL,'0') VALUE_FALL , NVL(VALUE_BEDSORE,'0') VALUE_BEDSORE ,";
            SQL += ComNum.VBLF + " B.JOBTIME, B.ENTSABUN,TO_CHAR(B.ENTDATE,'YYYY-MM-DD HH24:MM') ENTDATE,A.IPDNO,";
            SQL += ComNum.VBLF + "  decode(substr(a.BI,1,1),'1','1','2','2',decode(a.BI,'31','3','52','4','5')) bi ,";
            //SQL += ComNum.VBLF + " FC_TONG_PATIENT_HS(D.PANO,B.wardcode,to_char(E.JOBDATE,'yyyy-mm-dd')) gubun, B.ROWID ROWID2 ";
            SQL += ComNum.VBLF + " NVL(B.INOUT, FC_TONG_PATIENT_HS(D.PANO,B.wardcode,to_char(B.JOBTIME,'yyyy-mm-dd'))) gubun, B.ROWID ROWID2 ";
            //가퇴원 누락분 발생으로 TONG_PATIENT 조건 뺴줌
            //SQL += ComNum.VBLF + "  FROM NUR_MASTER A, KOSMOS_PMPA.NUR_TOTALCARE_NOTE2 B, BAS_PATIENT C ,IPD_NEW_MASTER D , TONG_PATIENT E";
            SQL += ComNum.VBLF + "  FROM NUR_MASTER A, KOSMOS_PMPA.NUR_TOTALCARE_NOTE2 B, BAS_PATIENT C ,IPD_NEW_MASTER D ";
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                for (i = 0; i < nREAD; i++)
                {
                    ...
                    ssList.ActiveSheet.Cells[nRow - 1, 0].Value = nRow;
                    ssList.ActiveSheet.Cells[nRow - 1, 1].Text = dt.Rows[i]["Hspcode"].ToString().Trim();
                    ...
                    ssList.ActiveSheet.Cells[nRow - 1, 4].Text = dt.Rows[i]["Pano"].ToString().Trim();
                    ssList.ActiveSheet.Cells[nRow - 1, 5].Text = dt.Rows[i]["Bi"].ToString().Trim();
                    ssList.ActiveSheet.Cells[nRow - 1, 6].Text = dt.Rows[i]["Gubun"].ToString().Trim();
                    if (VB.Mid(dt.Rows[i]["JUMIN2"].ToString().Trim(), 1, 1) == "3" || VB.Mid(dt.Rows[i]["JUMIN2"].ToString().Trim(), 1, 1) == "4")
                    {
                        ssList.ActiveSheet.Cells[nRow - 1, 7].Text = "20" + dt.Rows[i]["JUMIN1"].ToString().Trim();
                    }
                    else
                    {
                        ssList.ActiveSheet.Cells[nRow - 1, 7].Text = "19" + dt.Rows[i]["JUMIN1"].ToString().Trim();
                    }
                    ssList.ActiveSheet.Cells[nRow - 1, 8].Text = dt.Rows[i]["SEX"].ToString().Trim();
                    ssList.ActiveSheet.Cells[nRow - 1, 9].Text = dt.Rows[i]["DIAGNOSIS"].ToString().Trim();
                    ssList.ActiveSheet.Cells[nRow - 1, 10].Text = dt.Rows[i]["VALUE_OP"].ToString().Trim();
                    ssList.ActiveSheet.Cells[nRow - 1, 11].Text = dt.Rows[i]["BUN01"].ToString().Trim();
                    ssList.ActiveSheet.Cells[nRow - 1, 12].Text = dt.Rows[i]["BUN02"].ToString().Trim();
                    ssList.ActiveSheet.Cells[nRow - 1, 13].Text = dt.Rows[i]["BUN03"].ToString().Trim();
                    ssList.ActiveSheet.Cells[nRow - 1, 14].Text = dt.Rows[i]["BUN04"].ToString().Trim();
                    ssList.ActiveSheet.Cells[nRow - 1, 15].Text = dt.Rows[i]["BUN05"].ToString().Trim();
                    ssList.ActiveSheet.Cells[nRow - 1, 16].Text = dt.Rows[i]["BUN06"].ToString().Trim();
                    ssList.ActiveSheet.Cells[nRow - 1, 17].Text = dt.Rows[i]["BUN07"].ToString().Trim();
                    ssList.ActiveSheet.Cells[nRow - 1, 18].Text = dt.Rows[i]["BUN08"].ToString().Trim();
                    ssList.ActiveSheet.Cells[nRow - 1, 19].Text = dt.Rows[i]["BUN09"].ToString().Trim();
                    ssList.ActiveSheet.Cells[nRow - 1, 20].Text = dt.Rows[i]["BUN10"].ToString().Trim();
                    ssList.ActiveSheet.Cells[nRow - 1, 21].Text = dt.Rows[i]["BUN11"].ToString().Trim();
                    ssList.ActiveSheet.Cells[nRow - 1, 22].Text = dt.Rows[i]["BUN12"].ToString().Trim();
                    ssList.ActiveSheet.Cells[nRow - 1, 23].Text = dt.Rows[i]["BUN13"].ToString().Trim();
                    ssList.ActiveSheet.Cells[nRow - 1, 24].Text = dt.Rows[i]["BUN14"].ToString().Trim();
                    ssList.ActiveSheet.Cells[nRow - 1, 25].Text = dt.Rows[i]["BUN15"].ToString().Trim();
                    ssList.ActiveSheet.Cells[nRow - 1, 26].Text = dt.Rows[i]["BUN16"].ToString().Trim();
                    ssList.ActiveSheet.Cells[nRow - 1, 27].Text = dt.Rows[i]["BUN17"].ToString().Trim();
                    ssList.ActiveSheet.Cells[nRow - 1, 28].Text = dt.Rows[i]["BUN18"].ToString().Trim();
                    ssList.ActiveSheet.Cells[nRow - 1, 29].Text = dt.Rows[i]["BUN19"].ToString().Trim();
                    ssList.ActiveSheet.Cells[nRow - 1, 30].Text = dt.Rows[i]["VALUE_FALL"].ToString().Trim();
                    ssList.ActiveSheet.Cells[nRow - 1, 31].Text = dt.Rows[i]["VALUE_BEDSORE"].ToString().Trim();
                    ssList.ActiveSheet.Cells[nRow - 1, 32].Text = dt.Rows[i]["ROWID2"].ToString().Trim();
                }
            }

            TextCellType pgTextCellType = new TextCellType();
            pgTextCellType.CharacterCasing = CharacterCasing.Upper;
            ssList_Sheet1.Cells[0, 9, ssList_Sheet1.RowCount - 1, 9].CellType = pgTextCellType;
            ssList_Sheet1.Cells[0, 6, ssList_Sheet1.RowCount - 1, 6].CellType = pgTextCellType;
        ...
        private void ssList_Change(object sender, FarPoint.Win.Spread.ChangeEventArgs e)
        {
            ...
            if (e.Column != 9 && e.Column != 6)
            { return; }

            ssList_Sheet1.Cells[e.Row, 33].Text = "Y";
        ...
        private void eBtnSave()
        {
            ...
            {
                strROWID = ssList.ActiveSheet.Cells[i, 32].Text.Trim();
                strCHANGE = ssList.ActiveSheet.Cells[i, 33].Text.Trim();
                strILL = ssList.ActiveSheet.Cells[i, 9].Text.Trim();
                strINOUT = ssList.ActiveSheet.Cells[i, 6].Text.Trim();

                if (strROWID != "" && strCHANGE == "Y")
                {
                    SQL = " UPDATE KOSMOS_PMPA.NUR_TOTALCARE_NOTE2 SET ";
                    SQL += ComNum.VBLF + "  DIAGNOSIS = '" + strILL + "', ";
                    SQL += ComNum.VBLF + "  INOUT = '" + strINOUT + "' ";
