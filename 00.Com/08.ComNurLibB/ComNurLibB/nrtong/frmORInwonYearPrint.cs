...
namespace ComNurLibB
{
    ...
    public partial class frmORInwonYearPrint : Form, MainFormMessage
    {
        #region 클래스 선언 및 etc....
        ...
        public frmORInwonYearPrint(MainFormMessage pform)
        {
            InitializeComponent();
        ...
        void ePrint()
        {
            ...
            setOption = new clsSpread.SpdPrint_Option(PrintOrientation.Landscape, PrintType.All, 0, 0, true, true, true, true, true, false, false, (float)0.9);
        ...
        void eGetData()
        {
            ...
            SQL += ComNum.VBLF + "      AND WARDCODE IN ('NR','ND','32','40','50','53','55','63','65','70','73','75','80','DS')";
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                for(i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (VB.Left(dt.Rows[i]["YYMM"].ToString().Trim(), 4) == VB.Left(strBYYMM, 4))
                    {
                        nDATA[1, 13, nCol] = nDATA[1, 13, nCol] + VB.Val(dt.Rows[i]["IpInwon"].ToString().Trim());
                        nDATA[2, 13, nCol] = nDATA[2, 13, nCol] + VB.Val(dt.Rows[i]["Jewon"].ToString().Trim());
                    }
                    ...
                    else
                    {
                        ...
                        nDATA[1, 14, nCol] = nDATA[1, 14, nCol] + VB.Val(dt.Rows[i]["IpInwon"].ToString().Trim());
                        nDATA[2, 14, nCol] = nDATA[2, 14, nCol] + VB.Val(dt.Rows[i]["Jewon"].ToString().Trim());
            ...
            for(i = 1; i <= 14;i++)
            {
                ...
                if(i == 2)
                {
                    ...
                    nDATA[1, 14, 23] = nDATA[1, 14, 23] + nDATA[1, i, 23] - Convert.ToDouble(nDDCount[7]);
                }

                else
                {
                    ssList.ActiveSheet.Cells[(i * 2) - 1, 24].Text = nDATA[1, i, 23].ToString();
                    nDATA[1, 14, 23] = nDATA[1, 14, 23] + nDATA[1, i, 23];
                }

                if(i == 2)
                {
                    ssList.ActiveSheet.Cells[(i * 2) - 1, 25].Text = (nDATA[1, i, 24] - Convert.ToDouble(nDDCount[9])).ToString();
                    nDATA[1, 14, 24] = nDATA[1, 14, 24] + nDATA[1, i, 24] - Convert.ToDouble(nDDCount[9]);
                }

                else
                {
                    ssList.ActiveSheet.Cells[(i * 2) - 1, 25].Text = nDATA[1, i, 24].ToString();
                    nDATA[1, 14, 24] = nDATA[1, 14, 24] + nDATA[1, i, 24];
                }
                ...
                if(nDATA[0, i, 24] != 0)
                {
                    ssList.ActiveSheet.Cells[(i * 2) - 1, 28].Text = String.Format("{0:##0.0}", ((nDATA[2, i, 24] / nIlsu) / nDATA[0, i, 24]) * 100);
                }

                if(i == 11)
                {
                    ssList.ActiveSheet.Cells[i * 2, 24].Text = (nDATA[2, i, 23] - Convert.ToDouble(nDDCount[8])).ToString();
                    nDATA[2, 14, 23] = nDATA[2, 14, 23] + nDATA[2, i, 23] - Convert.ToDouble(nDDCount[8]);
                }

                else
                {
                    ssList.ActiveSheet.Cells[i * 2, 24].Text = nDATA[2, i, 23].ToString();
                    nDATA[2, 14, 23] = nDATA[2, 14, 23] + nDATA[2, i, 23];
                }

                if(i == 2)
                {
                    ssList.ActiveSheet.Cells[i * 2, 25].Text = (nDATA[2, i, 24] - Convert.ToDouble(nDDCount[10])).ToString();
                    nDATA[2, 14, 24] = nDATA[2, 14, 24] + nDATA[2, i, 24] - Convert.ToDouble(nDDCount[10]);
                }

                else
                {
                    ssList.ActiveSheet.Cells[i * 2, 25].Text = nDATA[2, i, 24].ToString();
                    nDATA[2, 14, 24] = nDATA[2, 14, 24] + nDATA[2, i, 24];
                }
            ...
            for(i = 1; i <= 22; i++)
            {
                ssList.ActiveSheet.Cells[29, i + 1].Text = nDATA[1, 14, i].ToString();
                ssList.ActiveSheet.Cells[30, i + 1].Text = nDATA[2, 14, i].ToString();

                if(nDATA[1, 13, i] != 0)
                {
                    ssList.ActiveSheet.Cells[31, i + 1].Text = String.Format("{0:##0.0}", (nDATA[1, 14, i] / nDATA[1, 13, i]) * 100 - 100);
                }

                if (nDATA[2, 13, i] != 0)
                {
                    ssList.ActiveSheet.Cells[32, i + 1].Text = String.Format("{0:##0.0}", (nDATA[2, 14, i] / nDATA[2, 13, i]) * 100 - 100);
                }

                //ssList의 MaxRowCount 수 보다 많으므로 주석처리함
                //ssList.ActiveSheet.Cells[33, i + 1].Text = String.Format("{0:##0.0}", nDATA[1, 14, i] / (double)nIlsu);
                //ssList.ActiveSheet.Cells[34, i + 1].Text = String.Format("{0:##0.0}", nDATA[2, 14, i] / (double)nIlsu);
            }
            ...
            for(j = 3; j <= 28; j++)
            {
                ...
                ssList.ActiveSheet.Cells[29, j - 1].Text = String.Format("{0:##,###,##0.0}", nSum1);
                ssList.ActiveSheet.Cells[30, j - 1].Text = String.Format("{0:##,###,##0.0}", nSum2);
            }
            ...
            if(nCnt1 > 0)
            {
                ssList.ActiveSheet.Cells[29, 28].Text = String.Format("{0:##,###,##0.0}", nSum1 / (double) nCnt1);
            }

            else
            {
                ssList.ActiveSheet.Cells[29, 28].Text = "0.0";
            }
