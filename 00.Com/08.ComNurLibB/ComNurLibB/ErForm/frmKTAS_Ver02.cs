...
namespace ComNurLibB
{
    ...
    public partial class frmKTAS_Ver02 : Form
    {
        ...
        //private string FstrOutDATE = "";

        
        public frmKTAS_Ver02()
        {
            InitializeComponent();
        }
        ...
        public frmKTAS_Ver02(string strIDNO, string strINDT, string strINTM, string strKTID, string strAge, string strROWID, string strJDATE)
        {
            //2020-05-21
            //public frmKTAS_Ver02(string strIDNO, string strINDT, string strINTM, string strKTID, string strAge, string strROWID, string strJDATE, string strOutTime = "")
            //FstrOutDATE = strOutTime;
            InitializeComponent();
            FstrIDNO = strIDNO;
            FstrINDT = strINDT;
            FstrINTM = strINTM;
        ...
        private void frmKTAS_Ver02_Load(object sender, EventArgs e)
        {
            ...
            txtInDate.Text = VB.Left(FstrINDT, 4) + "-" + VB.Mid(FstrINDT, 5, 2) + "-" + VB.Right(FstrINDT, 2);
            txtInTime.Text = VB.Left(FstrINTM, 2) + ":" + VB.Right(FstrINTM, 2);
        ...
        private bool SAVE_DATA()
        {
            ...
            if (chkDate.Checked == true)
            {
                ...
                strKTASTIME = txtKtdt.Text.Trim() + " " + txtKttm.Text.Trim();
            }
            else
            {
                ...
                strKTASTIME = clsPublic.GstrSysDate + " " + clsPublic.GstrSysTime;
            }
            ...
            //if (FstrOutDATE != "")
            //{
            //    if(Convert.ToDateTime(FstrOutDATE) < Convert.ToDateTime(strKTASTIME))
            //    {
            //        ComFunc.MsgBox("응급실 퇴실시간이 KTAS 분류시간보다 빠릅니다." + ComNum.VBLF + ComNum.VBLF + "퇴실시간이전으로 해주세요.");
            //        return rtVal;
            //    }
            //}
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            //사번체크 (2020-12-07)
            if ( cf.Read_SabunName(clsDB.DbCon, txtSabun.Text.Trim()) == "")
            {
                ComFunc.MsgBox("사번 입력 오류 입니다. 사번을 확인하시기 바랍니다.");
                return;
            }
        ...
        private string READ_KTAS_NAME(string arg)
        {
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    else if (VB.Right(arg, 1) == "1")
                    {
                        rtnVal += "비말/공기감염)";
                    }
                    else if (VB.Right(arg, 1) == "2")
                    {
                        rtnVal += "접촉감염)";
                    }
                    else if (VB.Right(arg, 1) == "3")
                    {
                        rtnVal += "특수상황)";
                    }
                    else if (VB.Right(arg, 1) == "9")
                    {
                        rtnVal += "미상)";
                    }
                }
        ...
        private bool SaveXML(PsmhDb pDbCon, string strPTMIIDNO, string strPTMIINDT, string strPTMIKPR, string strPTMIPTS, string strPTMIKTDT, string strPTMIKTTM, string strKTAS)
        {
            ...
            else
            {
                ...
                if (clsEmrQuery.SaveNurChartFlowEx(clsDB.DbCon, txtSabun.Text.Trim(), this, pAcp, pForm, strPTMIINDT, strPTMIIDNO, strDrCd, strPTMIKTDT, strPTMIKTTM + "00", strDataNew) == 0)
                {
                    ComFunc.MsgBoxEx(this, "중증도 분류내역 저장 도중 오류가 발생했습니다.");
                    return rtVal;
                }    
                #endregion
            }
