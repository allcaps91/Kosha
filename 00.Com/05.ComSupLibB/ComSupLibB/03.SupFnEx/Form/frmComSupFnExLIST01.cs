...
namespace ComSupLibB.SupFnEx
{
    ...
    /// File Name       : frmComSupFnExLIST01.cs
    /// Description     : 기능검사 심장초음파 시디복사 신청명단리스트
    /// Author          : 윤조연
    ...
    /// 
    /// </history>
    /// <seealso cref= "\Ocs\ekg\FrmCUSCOPY.frm(FrmCUSCOPY) >> frmComSupFnExLIST01.cs 폼이름 재정의" />
    public partial class frmComSupFnExLIST01 : Form
    {
        #region 클래스 선언 및 etc....
        ...
        void ePrint()
        {
            ...
            strTitle = "심장초음파 CD COPY 대장 " + "(" + dtpFDate.Text.Trim() + " ~ " + dtpTDate.Text.Trim() + ")";
        ...
        void GetData2(PsmhDb pDbCon, FarPoint.Win.Spread.FpSpread Spd, string argSDate, string argTDate)
        {
            ...
            cXrayDetail.XCode = "CUSCOPY";

            dt = xraySql.sel_XrayDetail_order(pDbCon, cXrayDetail);
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                for (i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    Spd.ActiveSheet.Cells[i, (int)clsComSupFnExSpd.enmCdList2.Remark].Text = "심초음파CD";
