...
namespace ComEmrBase
{
    ...
    /// 19-07-23
    /// </summary>
    public partial class frmPatientState : Form
    {
        private class GV
        {
        ...
        public frmPatientState()
        {
            InitializeComponent();
        }

        public frmPatientState(EmrPatient emr)
        {
            pAcp = emr;
            InitializeComponent();
        }
        ...
        bool GetBMedDept(ref DataTable dataTable, string strMedDeptCd = "", string strDeptKorName = "")
        {
            ...
            try
            {
                ...
                SQL = SQL + ComNum.VBLF + "    FROM ADMIN.VIEWBMEDDEPT ";
        ...
        void Read_Patient_List()
        {
            ...
            try
            {
                ...
                SQL = SQL + ComNum.VBLF + " (SELECT DRNAME FROM ADMIN.BAS_DOCTOR WHERE DRCODE = A.DRCODE) DRNAME, ";
        ...
        void CellClick(int Row)
        {
            ...
            READ_DATA(strPtno, strInDate, strIPDNO, strAge, strWARDCODE);
        ...
        void READ_VITAL(string argPTNO, string argSDATE, string argEDATE)
        {
            string SQL = string.Empty;
            DataTable dt = null;
            ...
            try
            {
                ...
                if (pForm.FmOLDGB != 1)
                {
                    ...
                    SQL = SQL + ComNum.VBLF + "            B5.ITEMVALUE AS IT8,   B8.ITEMVALUE AS IT10, B7.ITEMVALUE AS IT12,   ";
                    SQL = SQL + ComNum.VBLF + "            B6.ITEMVALUE AS IT14,  B9.ITEMVALUE AS IT274";
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        SSVITAL_Sheet1.Cells[8, i + 1].Text = dt.Rows[i]["IT12"].ToString().Trim();
                        SSVITAL_Sheet1.Cells[9, i + 1].Text = dt.Rows[i]["IT274"].ToString().Trim();
                        ...
                        SSVITAL_Sheet1.Rows[9].Height = SSVITAL_Sheet1.Rows[9].GetPreferredHeight() + 5;
        ...
        /// <param name="argSDATE"></param>
        /// <param name="argEDATE"></param>
        void READ_IO(string argPTNO, string argSDATE, string argEDATE)
        {
            string SQL = string.Empty;
            DataTable dt = null;

            SSIO_Sheet1.ColumnCount = 3;
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    SSIO_Sheet1.ColumnCount = dt.Rows.Count + 3;
                    nCol = 2;
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                            ...
                            strOLDCHARTDATE.Length > 0 && strCHARTDATE.Length > 0)
                        {
                            ...
                            SSIO_Sheet1.Cells[2, nCol].Text = nItotal.ToString();

                            SSIO_Sheet1.AddSpanCell(5, nCol, 5, 1);
                            SSIO_Sheet1.Cells[5, nCol].Text = nOtotal.ToString();
                        ...
                        SSIO_Sheet1.Cells[0, nCol].Text = VB.Left(VB.Right(strCHARTDATE, 4), 2) + "/" + VB.Right(strCHARTDATE, 2) + ComNum.VBLF +
                            VB.Left(strCHARTTIME, 2) + ":" + VB.Right(strCHARTTIME, 2);

                        SSIO_Sheet1.Cells[8, nCol].Text = dt.Rows[i]["DUTY"].ToString().Trim();

                        SSIO_Sheet1.Cells[2, nCol].Text = (VB.Val(dt.Rows[i]["구강1"].ToString().Trim()) +
                            VB.Val(dt.Rows[i]["구강2"].ToString().Trim()) +
                            VB.Val(dt.Rows[i]["구강3"].ToString().Trim())).ToString();

                        nItotal += (int)VB.Val(SSIO_Sheet1.Cells[2, nCol].Text.Trim());
                        ...
                        SSIO_Sheet1.Cells[3, nCol].Text = (VB.Val(dt.Rows[i]["수액1"].ToString().Trim()) +
                        ...
                        nItotal += (int)VB.Val(SSIO_Sheet1.Cells[3, nCol].Text.Trim());

                        SSIO_Sheet1.Cells[4, nCol].Text = (VB.Val(dt.Rows[i]["혈액1"].ToString().Trim()) +
                            VB.Val(dt.Rows[i]["혈액2"].ToString().Trim()) + VB.Val(dt.Rows[i]["혈액3"].ToString().Trim()) +
                            VB.Val(dt.Rows[i]["혈액4"].ToString().Trim())).ToString();
                        nItotal += (int)VB.Val(SSIO_Sheet1.Cells[4, nCol].Text.Trim());

                        SSIO_Sheet1.Cells[5, nCol].Text = dt.Rows[i]["URINE"].ToString().Trim();
                        nOtotal += (int)VB.Val(SSIO_Sheet1.Cells[5, nCol].Text.Trim());

                        SSIO_Sheet1.Cells[6, nCol].Text = dt.Rows[i]["STOOL"].ToString().Trim();
                        nOtotal += (int)VB.Val(SSIO_Sheet1.Cells[6, nCol].Text.Trim());


                        SSIO_Sheet1.Cells[7, nCol].Text = (VB.Val(dt.Rows[i]["기타배설1"].ToString().Trim()) +
                        ...
                        nOtotal += (int)VB.Val(SSIO_Sheet1.Cells[7, nCol].Text.Trim());


                        if (dt.Rows[i]["gbn"].ToString().Trim() == "ICU")
                        {
                            SSIO_Sheet1.Columns[nCol].ForeColor = System.Drawing.Color.FromArgb(0, 0, 255);
                        }
                        ...
                        SSIO_Sheet1.Cells[0, nCol].HorizontalAlignment = FarPoint.Win.Spread.CellHorizontalAlignment.Center;
                        SSIO_Sheet1.Cells[0, nCol].VerticalAlignment = FarPoint.Win.Spread.CellVerticalAlignment.Center;

                        //SSIO_Sheet1.Columns[nCol].Width = 50;
                        SSIO_Sheet1.Columns[nCol].Width = SSIO_Sheet1.Columns[nCol].GetPreferredWidth() + 5;
                    }

                    SSIO_Sheet1.Columns[3, SSIO_Sheet1.ColumnCount - 1].Border = border;
                    SSIO_Sheet1.Columns[3, SSIO_Sheet1.ColumnCount - 1].HorizontalAlignment = FarPoint.Win.Spread.CellHorizontalAlignment.Center;
                    SSIO_Sheet1.Columns[3, SSIO_Sheet1.ColumnCount - 1].VerticalAlignment = FarPoint.Win.Spread.CellVerticalAlignment.Center;
                ...
                SQL = SQL + ComNum.VBLF + ")";
                SQL = SQL + ComNum.VBLF + "  WHERE TOTAL > 0";
                SQL = SQL + ComNum.VBLF + "  ORDER BY CDATE, DECODE(DUTY, 'DAY', 0, 'EVE', 1, 'NIG', 2)";
                ...
                if (dt.Rows.Count > 0)
                {
                    SSIO_Sheet1.Columns.Add(3, 1);
                    nCol = 3;

                    DateTime dtpOldChartDate = Convert.ToDateTime(dt.Rows[0]["CDATE"].ToString().Trim());
                    DateTime dtpSumChartDate = Convert.ToDateTime(dt.Rows[0]["CDATE"].ToString().Trim());
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        if (
                            //나이트 ~ 데이
                            ((dtpChartDate.Date - dtpSumChartDate.Date).TotalDays == 0 && 
                              (dtpChartDate.Hour < 5 || dtpChartDate.Hour == 5 && dtpChartDate.Minute == 0) &&
                              !strDuty.Equals(strOldDuty))
                            ||
                            ((dtpChartDate.Date - dtpSumChartDate.Date).TotalDays == 1 && dtpChartDate.Hour > 5)
                            ||
                             ((dtpChartDate.Date - dtpSumChartDate.Date).TotalDays > 1)
                           )
                        {
                            SSIO_Sheet1.Columns.Add(3, 1);
                            nCol = 3;
                            ...
                            dtpSumChartDate = Convert.ToDateTime(dt.Rows[i]["CDATE"].ToString().Trim());
                        ...
                        SSIO_Sheet1.Cells[0, nCol].Text = dtpChartDate.ToString("MM") + "/" + dtpChartDate.ToString("dd");
                        SSIO_Sheet1.Cells[0, nCol].Text += ComNum.VBLF + dtpChartDate.ToString("HH:mm");
                        ...
                        switch (dt.Rows[i]["ITEM"].ToString().Trim())
                        {
                            ...
                            case "혈액":
                            case "경구":
                                intIn += (int)VB.Val(dt.Rows[i]["TOTAL"].ToString().Trim());
                                break;
                            case "Stool":
                            case "Urine":
                            case "배액관":
                                intOut += (int)VB.Val(dt.Rows[i]["TOTAL"].ToString().Trim());
                                break;
                        }
        ...
        private void Set_IOSUM(ref int nCol, ref int nItotal, ref int nOtotal)
        {
            SSIO_Sheet1.ColumnCount += 1;
            ...
            SSIO_Sheet1.AddSpanCell(5, nCol, 3, 1);
            SSIO_Sheet1.Cells[5, nCol].Text = nOtotal.ToString();
        ...
        /// <param name="argSDATE"></param>
        /// <param name="argEDATE"></param>
        void READ_BST(string argPTNO, string argSDATE, string argEDATE)
        {
            string SQL = string.Empty;
            DataTable dt = null;
        ...
        void READ_DETAIL_PAIN(string argIPDNO, string ArgPano)
        {
            ...
            try
            {
                ...
                SQL = SQL + ComNum.VBLF + "  FROM ADMIN.NUR_PAIN_SCALE";
                SQL = SQL + ComNum.VBLF + " WHERE IPDNO = " + argIPDNO;
