...
namespace ComEmrBase
{
    ...
    /// 욕창예방간호활동기록지
    /// FORMNO = 2638
    /// \mtsEmrf\frmTextEmr_ICUBundle_중심도관_k.frm
    /// </summary>
    public partial class frmTextEmr_ICUBundle : Form, EmrChartForm
    {
        #region // TopMenu관련 선언
        ...
        #region //생성자
        public frmTextEmr_ICUBundle()
        {
            InitializeComponent();
        }

        public frmTextEmr_ICUBundle(string pMake, int pItemCnt, int pHeadCnt, FormFlowSheet[] pFormFlowSheet, FormFlowSheetHead[,] pFormFlowSheetHead)
        {
            InitializeComponent();
            mFLOWGB = pMake;
        ...
        public frmTextEmr_ICUBundle(string strFormNo, string strUpdateNo, string strEmrNo, int pCallFormGb)
        {
            InitializeComponent();
        ...
        public frmTextEmr_ICUBundle(string strFormNo, string strUpdateNo, EmrPatient po, string strEmrNo, string strMode, string strOrderDate, FormEmrMessage pEmrCallForm) //절대 수정하시 마시요(서식생성기 에러남)
        {
            mstrFormNo = strFormNo;
            mstrUpdateNo = strUpdateNo;
            pAcp = po;
        ...
        public frmTextEmr_ICUBundle(string strFormNo, string strUpdateNo, EmrPatient po, string strEmrNo, string strMode, FormEmrMessage pEmrCallForm) //절대 수정하시 마시요(서식생성기 에러남)
        {
            mstrFormNo = strFormNo;
            mstrUpdateNo = strUpdateNo;
            pAcp = po;
        ...
        private void frmTextEmr_NrActYokPrevention_Load(object sender, EventArgs e)
        {
            ...
            if (pAcp.ptNo.Length > 0)
            {
                ...
                DesignFunc.DefaultDuty(ssUserChartIO);
        ...
        void GetLastValue(int lngCol)
        {
            ...
            string strEmrNo = ssUserChartHisIO_Sheet1.Cells[19, lngCol].Text.Trim();
            int i = 0;

            for (int xCol1 = 0; xCol1 < ssUserChartHisIO_Sheet1.ColumnCount; xCol1++)
            {
                if (strEmrNo == ssUserChartHisIO_Sheet1.Cells[19, xCol1].Text.Trim())
                {
                    i += 1;
                    for (int xRow = 3; xRow < 18; xRow++)
                    {
        ...
        void ConVIoData()
        {
            ...
            for (int xRow1 = 0; xRow1 < ssUserChartHis_Sheet1.RowCount; xRow1++)
            {
                ...
                //작성자
                ssConV_Sheet1.Cells[18, 0].Text = string.Format("{0} / {1}", ssUserChartHis_Sheet1.Cells[xRow1, 32].Text.Trim(), strTime);

                //EMRNO
                ssConV_Sheet1.Cells[19, 0].Text = ssUserChartHis_Sheet1.Cells[xRow1, 11].Text.Trim();

                //USEID
                ssConV_Sheet1.Cells[20, 0].Text = ssUserChartHis_Sheet1.Cells[xRow1, 12].Text.Trim();
        ...
        private void SsUserChartHisIO_CellDoubleClick(object sender, FarPoint.Win.Spread.CellClickEventArgs e)
        {
            ...
            string strEmrNo = ssUserChartHisIO_Sheet1.Cells[19, e.Column].Text.Trim();
            mstrEmrNo = strEmrNo;

            mEmrUseId = ssUserChartHisIO_Sheet1.Cells[20, e.Column].Text.Trim();
            ...
            for (int xCol1 = 0; xCol1 < ssUserChartIO_Sheet1.ColumnCount; xCol1++)
            {
                if (strEmrNo == ssUserChartHisIO_Sheet1.Cells[19, xCol1].Text.Trim())
                {
                    for (int xRow1 = 3; xRow1 < 18; xRow1++)
                    {
