...
namespace ComEmrBase
{
    ...
    /// 신생아 낙상예방 활동 점검표
    /// FORMNO = 2500
    /// \mtsEmrf\frmTextEmr_NrActFallBaby.frm
    /// </summary>
    public partial class frmTextEmr_NrActFallBaby : Form, EmrChartForm
    {
        #region // TopMenu관련 선언
        ...
        private void pPrintForm()
        {
            using (frmEmrPrintOption frmEmrPrintOptionX = new frmEmrPrintOption())
            {
                ...
                clsSpreadPrint.PrintEmrSpread(clsDB.DbCon, mstrFormNo, mstrUpdateNo, pAcp, true, dtpOptSDate.Value.ToString("yyyyMMdd"), dtpOptEDate.Value.ToString("yyyyMMdd"),
                                             ssUserChartHisIO, "P", 30, 20, 30, 20, false, FarPoint.Win.Spread.PrintOrientation.Landscape, mFLOWGB, -1, 20, mFLOWHEADCNT, "V");
            }
        }
        ...
        #region //생성자
        public frmTextEmr_NrActFallBaby()
        {
            InitializeComponent();
        }

        public frmTextEmr_NrActFallBaby(string pMake, int pItemCnt, int pHeadCnt, FormFlowSheet[] pFormFlowSheet, FormFlowSheetHead[,] pFormFlowSheetHead)
        {
            InitializeComponent();
            mFLOWGB = pMake;
        ...
        public frmTextEmr_NrActFallBaby(string strFormNo, string strUpdateNo, string strEmrNo, int pCallFormGb)
        {
            InitializeComponent();
        ...
        public frmTextEmr_NrActFallBaby(string strFormNo, string strUpdateNo, EmrPatient po, string strEmrNo, string strMode, string strOrderDate, FormEmrMessage pEmrCallForm) //절대 수정하시 마시요(서식생성기 에러남)
        {
            mstrFormNo = strFormNo;
            mstrUpdateNo = strUpdateNo;
            pAcp = po;
        ...
        public frmTextEmr_NrActFallBaby(string strFormNo, string strUpdateNo, EmrPatient po, string strEmrNo, string strMode, FormEmrMessage pEmrCallForm) //절대 수정하시 마시요(서식생성기 에러남)
        {
            mstrFormNo = strFormNo;
            mstrUpdateNo = strUpdateNo;
            pAcp = po;
        ...
        private void frmTextEmr_NrActFallBaby_Load(object sender, EventArgs e)
        {
            ...
            dtpOptSDate.Value = Convert.ToDateTime(clsEmrQueryPohangS.ReadOptionDate(clsEmrPublic.gUserGrade, pAcp.inOutCls, pAcp.medDeptCd, clsType.User.IdNumber,
                        Convert.ToDateTime(ComQuery.CurrentDateTime(clsDB.DbCon, "S")).AddDays(-3).ToString("yyyy-MM-dd")));
            ...
            if (pAcp.ptNo.Length > 0)
            {
                ...
                if ((mEmrCallForm as Form).Name.Equals("frmEmrLibViewerNr") == false)
                {
                    dtpOptSDate.Value = Convert.ToDateTime(clsEmrQueryPohangS.ReadOptionDate(clsEmrPublic.gUserGrade, pAcp.inOutCls, pAcp.medDeptCd, clsType.User.IdNumber,
        ...
        void ConVIoData()
        {
            ...
            for (int xRow1 = 0; xRow1 < ssUserChartHis_Sheet1.RowCount; xRow1++)
            {
                ...
                //작성자
                ssConV_Sheet1.Cells[19, 0].Text = string.Format("{0} / {1}", ssUserChartHis_Sheet1.Cells[xRow1, 33].Text.Trim(), strTime);

                //EMRNO
                ssConV_Sheet1.Cells[20, 0].Text = ssUserChartHis_Sheet1.Cells[xRow1, 11].Text.Trim();

                //USEID
                ssConV_Sheet1.Cells[21, 0].Text = ssUserChartHis_Sheet1.Cells[xRow1, 12].Text.Trim();
                SetIoHis();
            }
        ...
        private void SsUserChartHisIO_CellDoubleClick(object sender, FarPoint.Win.Spread.CellClickEventArgs e)
        {
            ...
            mEmrUseId = ssUserChartHisIO_Sheet1.Cells[21, e.Column].Text.Trim();

            string strEmrNo = ssUserChartHisIO_Sheet1.Cells[20, e.Column].Text.Trim();
            mstrEmrNo = strEmrNo;
