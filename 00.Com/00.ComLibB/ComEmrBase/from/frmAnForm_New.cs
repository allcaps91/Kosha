...
{
    public partial class frmAnForm_New : Form, EmrChartForm
    {
        class GV
        {
            public string Code = "";
        ...
        /// 항목
        /// </summary>
        private List<MedcationInfo> Medcations = new List<MedcationInfo>();
        /// <summary>
        /// 그래프 차트
        /// </summary>
        ...
        public frmAnForm_New(string strPtNo, string strInoutCls, string strMedFrDate, string strDeptCd)
        {
            InitializeComponent();
        ...
        public frmAnForm_New(string strFormNo, string strUpdateNo, EmrPatient po, string strEmrNo, string strMode)
        {
            InitializeComponent();
        ...
        public frmAnForm_New(string strFormNo, string strUpdateNo, EmrPatient po, string strEmrNo, string strMode, FormEmrMessage pEmrCallForm)
        {
            InitializeComponent();
        ...
        {
            ssView.ActiveSheet.RowHeader.ColumnCount = 1;
            ssView.ActiveSheet.RowHeader.Columns[0].Width = 100;
            ssView.ActiveSheet.RowHeader.Visible = true;
            ...
            chartVital.Visible = false;
            chartVital.Dock = DockStyle.Fill;
        ...
        private void chkOpStart_Click(object sender, EventArgs e)
        {
            ...
            if (chkOpStart.Checked)
            {
                ...
                //  View Spread 항목설정
                List<MedcationInfo> list = Medcations.ToList();
                ViewItemCount = list.Count;

                for (int i = 0; i < list.Count; i++)
                {
                    ssView.ActiveSheet.RowHeader.Cells[i + 2, 0].Text = list[i].GroupName;
        ...
        private void SetViewRowHead()
        {
            //ssView.ActiveSheet.RowHeader.Cells[0, 0, ssView.ActiveSheet.RowCount - 1, 0].Text = " ";
            //ssView.ActiveSheet.RowHeader.Cells[5, 0].Text = "V/S";
            //ssView.ActiveSheet.RowHeader.Cells[5, 0].RowSpan = 5;
            //ssView.ActiveSheet.RowHeader.Cells[10, 0].Text = "Monitor";
            //ssView.ActiveSheet.RowHeader.Cells[10, 0].RowSpan = 4;
            //ssView.ActiveSheet.RowHeader.Cells[14, 0].Text = "Agent";
            //ssView.ActiveSheet.RowHeader.Cells[14, 0].RowSpan = 6;
            ssView.ActiveSheet.SetRowHeight(-1, 23);
            //ssView.ActiveSheet.Columns[0, ssView.ActiveSheet.ColumnCount - 1].Resizable = false;
            //ssView.ActiveSheet.RowHeader.Columns[0].Resizable = false;
            //ssView.ActiveSheet.RowHeader.Columns[0].MergePolicy = MergePolicy.Always;
            ssView.ActiveSheet.RowHeader.Cells[0, 0, ssView.ActiveSheet.RowCount - 1, 0].Border = new ComplexBorder(new ComplexBorderSide(ComplexBorderSideStyle.None), new ComplexBorderSide(ComplexBorderSideStyle.None), new ComplexBorderSide(ComplexBorderSideStyle.None), new ComplexBorderSide(ComplexBorderSideStyle.MediumLine), new ComplexBorderSide(ComplexBorderSideStyle.None), false, false);
            ssView.ActiveSheet.Cells[0, 0, ssView.ActiveSheet.RowCount - 1, 1].Border = new ComplexBorder(new ComplexBorderSide(ComplexBorderSideStyle.None), new ComplexBorderSide(ComplexBorderSideStyle.None), new ComplexBorderSide(ComplexBorderSideStyle.None), new ComplexBorderSide(ComplexBorderSideStyle.MediumLine), new ComplexBorderSide(ComplexBorderSideStyle.None), false, false);

            int Span = 0;

            for (int i = 0; i < ssView.RowCount() - 1; i++)
            {
                ssView.ActiveSheet.RowHeader.Cells[i, 0].RowSpan = 1;

                if (ssView.ActiveSheet.RowHeader.Cells[i, 0].Text != "")
                {

                }

                if (ssView.ActiveSheet.RowHeader.Cells[i, 0].Text.Empty() && Span == 0)
                {
                    continue;
                }

                if (ssView.ActiveSheet.RowHeader.Cells[i, 0].Text == ssView.ActiveSheet.RowHeader.Cells[i + 1, 0].Text)
                {
                    Span++;
                }
                else
                {
                    if (Span > 0)
                    {
                        ssView.ActiveSheet.RowHeader.Cells[i - Span, 0].RowSpan = Span + 1;
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < ssWrite.ActiveSheet.Rows.Count; i++)
            {
                if (ssWrite.ActiveSheet.Rows[i].Tag != null)
                {
                    ...
                    double dVal = value.To<double>(0);

                    if (itemName.Equals("HR")) //맥박
                    {
                        if (dVal < 51 || dVal > 100)
                        {
                            ssView.ActiveSheet.Cells[row, col].ForeColor = Color.Red;
                        }
                    }

                    if (itemName.Equals("SBP"))
                    {
            ...
            ssView.SetViewportLeftColumn(0, col - 5); ;
        }
        ...
        private void SetAgentItems(List<MedcationInfo> AgentItems)
        {

            List<MedcationInfo> BakMedcations = new List<MedcationInfo>();

            BakMedcations = Medcations.ToList();

            if (AgentItems.Count == 0)
            {
                ComFunc.MsgBoxEx(this, "항목을 하나이상 선택 해주세요!");
                return;
            }

            //frmEmrAnItem
            Medcations = new List<MedcationInfo>();
            Medcations = AgentItems.ToList();
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                if (list[i].ItemNo.Equals("I0000030240") || list[i].ItemNo.Equals("I0000034072"))
                {
                    ssWrite.ActiveSheet.Cells[i, 1].Text = "Keep";
                }
            ...
            if (chkOpStart.Checked == true)
            {
                ...
                ssView.ActiveSheet.Cells[2, 0, ssView.RowCount() - 1, 1].Text = "";
                ssView.ActiveSheet.RowHeader.Cells[2, 0, ssView.RowCount() - 1, 0].Text = " ";
                ...
                for (i = 0; i < vlist.Count; i++)
                {
                    ...
                    MedcationInfo item = BakMedcations.Where(d => d.ItemNo == vlist[i].ItemNo).FirstOrDefault();

                    if (BakMedcations.Where(d => d.ItemNo == vlist[i].ItemNo).Any())
                    {
                        if (item.Row + h != vlist[i].Row)
                        {
                            ssView.ActiveSheet.MoveRange(item.Row + h, 0, vlist[i].Row, 0, 1, ssView.ActiveSheet.ColumnCount, true);
                        }
                    }
                    else
                    {
                        ssView.ActiveSheet.AddRows(vlist[i].Row, 1);
                        h++;
                    }

                    ssView.ActiveSheet.RowHeader.Cells[vlist[i].Row, 0].RowSpan = 1;
                    ssView.ActiveSheet.RowHeader.Cells[vlist[i].Row, 0].Text = (vlist[i].GroupName.NotEmpty() ? vlist[i].GroupName : " ");
                    ssView.ActiveSheet.Cells[vlist[i].Row, 0].Text = vlist[i].Name;
                    ssView.ActiveSheet.Cells[vlist[i].Row, 1].Text = vlist[i].Uint;

                    ssView.ActiveSheet.Cells[vlist[i].Row, 0].HorizontalAlignment = CellHorizontalAlignment.Left;
                    ssView.ActiveSheet.Cells[vlist[i].Row, 1].HorizontalAlignment = CellHorizontalAlignment.Center;

                    ssView.ActiveSheet.Cells[vlist[i].Row, 0].Locked = true;
                    ssView.ActiveSheet.Cells[vlist[i].Row, 1].Locked = true;

                }

                if (BakMedcations.Count > vlist.Count)
                {
                    ssView.ActiveSheet.Cells[vlist.Count + 2, 0, BakMedcations.Count + 1, ssView.ActiveSheet.ColumnCount - 1].Text = "";
                }
        ...
        private int PrintAnForm(string strPrintType = "P")
        {
                ...
                for (int p = 0; p <= 2 ; p++)
                {
                    if (p == 0)
                    {
                        ...
                        rtnVal = clsSpreadPrint.PrintSpdAnForm(clsDB.DbCon, mstrFormNo, mstrUpdateNo, AcpEmr, true, ref ssPrint1, strPrintType, 30, 20, 10, 0, false,
                                    FarPoint.Win.Spread.PrintOrientation.Portrait, strCurDateTime, 1, 1 + 2, 0.8f);
                        //ssPrint1_Sheet1.PrintInfo.Preview = true;
                    ...
                    else if (p == 1)
                    {
                        ...
                        rtnVal = clsSpreadPrint.PrintSpdAnForm(clsDB.DbCon, mstrFormNo, mstrUpdateNo, AcpEmr, true, ref ssPrint2, strPrintType, 30, 20, 10, 0, false,
                                    FarPoint.Win.Spread.PrintOrientation.Portrait, strCurDateTime, 2, 1 + 2, 0.8f);
                    ...
                    else
                    {
                        ...
                        rtnVal = clsSpreadPrint.PrintSpdAnForm(clsDB.DbCon, mstrFormNo, mstrUpdateNo, AcpEmr, true, ref ssPrint2, strPrintType, 30, 20, 10, 0, false,
                                    FarPoint.Win.Spread.PrintOrientation.Portrait, strCurDateTime, p + 1, 1 + 2, 0.75f);
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    AcpEmr.writeName = dt.Rows[0]["NAME"].ToString();
                    AcpEmr.writeDate = dt.Rows[0]["CHARTDATE"].ToString();
                    AcpEmr.opdate = dtpOpDateFr.Value.ToString("yyyy-MM-dd") + "~" + dtpOpDateEnd.Value.ToString("yyyy-MM-dd");
                }
        ...
        private void ssView_CellClick(object sender, CellClickEventArgs e)
        {
            if (e.RowHeader == true)
            {
                SsAgent.ActiveSheet.RowCount = 0;
                SsAgent.ActiveSheet.ColumnCount = 0;
                SsAgent.ActiveSheet.ColumnCount = 5;

                SsAgent.ActiveSheet.RowCount = BasicMedcationInfo.Count;

                SsAgent.ActiveSheet.Columns[0].CellType = new TextCellType();
                SsAgent.ActiveSheet.Columns[1].CellType = new TextCellType();
                SsAgent.ActiveSheet.Columns[2].CellType = new CheckBoxCellType();
                SsAgent.ActiveSheet.Columns[3].CellType = new TextCellType();
                SsAgent.ActiveSheet.Columns[4].CellType = new TextCellType();

                SsAgent.ActiveSheet.Columns[0].MergePolicy = MergePolicy.Always;

                SsAgent.ActiveSheet.Columns[0, 4].VerticalAlignment = CellVerticalAlignment.Center;
                SsAgent.ActiveSheet.Columns[0, 4].HorizontalAlignment = CellHorizontalAlignment.Left;
                SsAgent.ActiveSheet.Columns[0, 4].Locked = true;
                SsAgent.ActiveSheet.Columns[2].HorizontalAlignment = CellHorizontalAlignment.Center;
                SsAgent.ActiveSheet.Columns[2].Locked = false;

                SsAgent.ActiveSheet.Columns[0].Width = 100;
                SsAgent.ActiveSheet.Columns[1].Width = 0;
                SsAgent.ActiveSheet.Columns[2].Width = 40;
                SsAgent.ActiveSheet.Columns[3].Width = 150;
                SsAgent.ActiveSheet.Columns[4].Width = 50;

                for (int i = 0; i < BasicMedcationInfo.Count; i++)
                {
                    if (BasicMedcationInfo[i].Row == 2 || BasicMedcationInfo[i].Row == 3 || BasicMedcationInfo[i].Row == 4)
                    {
                        SsAgent.ActiveSheet.Rows[i].Locked = true;
                    }

                    SsAgent.ActiveSheet.Cells[i, 0].Text = BasicMedcationInfo[i].GroupName;
                    SsAgent.ActiveSheet.Cells[i, 1].Text = BasicMedcationInfo[i].ItemNo;
                    SsAgent.ActiveSheet.Cells[i, 2].Value = ChkSsAgentItem(BasicMedcationInfo[i].Name);
                    SsAgent.ActiveSheet.Cells[i, 3].Text = BasicMedcationInfo[i].Name;
                    SsAgent.ActiveSheet.Cells[i, 4].Text = BasicMedcationInfo[i].Uint;
                }

                PanAgent.Width = (SsAgent.ActiveSheet.Columns[0].Width + SsAgent.ActiveSheet.Columns[1].Width + SsAgent.ActiveSheet.Columns[2].Width + SsAgent.ActiveSheet.Columns[3].Width + SsAgent.ActiveSheet.Columns[4].Width + 70).To<int>(0);
        ...
        private object ChkSsAgentItem(string name)
        {
            bool RtnVal = false;

            for (int i = 0; i < ssView.ActiveSheet.RowCount; i++)
            {
                if (ssView.ActiveSheet.Cells[i, 0].Text.Empty())
                    continue;

                if (ssView.ActiveSheet.Cells[i, 0].Text.Trim().Equals(name.Trim()))
                {
                    RtnVal = true;
                    break;
                }
        ...
        private void InitAnItem()
        {
            //frmEmrAnItem
            Medcations = new List<MedcationInfo>();
            Medcations = BasicMedcationInfo.Where(d => d.USEAgent == 0).ToList();
        ...
        private void btnClearAll_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < ssWrite.ActiveSheet.RowCount; i++)
            {
                ...
                if (ssWrite.ActiveSheet.Cells[i, 0].Text.Equals("EKG") || ssWrite.ActiveSheet.Cells[i, 0].Text.Equals("Warm blanket"))
                {
                    ssWrite.ActiveSheet.Cells[i, 1].Text = "Keep";
                }
        ...
        private void SetBasicMedcationInfo()
        {
            ...
            try
            {
                ...
                {
                    //시작종료
                    new MedcationInfo{Name = "마취", Uint = "", IsView = true, IsWrite = false, ItemNo = "I0000001164" , Row = 2, GroupName = " "},
                    new MedcationInfo{Name = "수술", Uint = "", IsView = true, IsWrite = false, ItemNo = "I0000001422", Row = 3, GroupName = " "},
                    new MedcationInfo{Name = "삽관", Uint = "", IsView = true, IsWrite = false, ItemNo = "I0000012402", Row = 4, GroupName = " "},

                    //V/S
                    new MedcationInfo{Name = "SBP", Uint = "mmHg", ItemNo = "I0000002018", Row = 5, GroupName = "V/S"},
                    new MedcationInfo{Name = "DBP", Uint = "mmHg", ItemNo = "I0000001765", Row = 6, GroupName = "V/S"},
                    new MedcationInfo{Name = "ASBP", Uint = "mmHg", ItemNo = "I0000037255", Row = 7, GroupName = "V/S"},
                    new MedcationInfo{Name = "ADBP", Uint = "mmHg", ItemNo = "I0000037256", Row = 8, GroupName = "V/S"},
                    new MedcationInfo{Name = "HR", Uint = "회/min", ItemNo = "I0000001178", Row = 9, GroupName = "V/S"},
                    new MedcationInfo{Name = "RR", Uint = "회/min", ItemNo = "I0000000574", Row = 10, GroupName = "V/S"},
                    new MedcationInfo{Name = "BT", Uint = "℃", ItemNo = "I0000001811", Row = 11, GroupName = "V/S"},
                
                    //Monitor
                    new MedcationInfo{Name = "SpO2 (%)", Uint = "%", ItemNo = "I0000008708", Row = 12, GroupName = "Monitor"},
                    new MedcationInfo{Name = "ETCO2", Uint = "mmHg", ItemNo = "I0000031627", Row = 13, GroupName = "Monitor"},
                    new MedcationInfo{Name = "EKG", Uint = "", ItemNo = "I0000030240", Row = 14, GroupName = "Monitor"},
                    new MedcationInfo{Name = "BIS", Uint = "", ItemNo = "I0000031829", Row = 15, GroupName = "Monitor"},
                    new MedcationInfo{Name = "PVI", Uint = "", ItemNo = "I0000038223", Row = 16, GroupName = "Monitor"},
                    new MedcationInfo{Name = "PSI", Uint = "", ItemNo = "I0000038222", Row = 17, GroupName = "Monitor"}
                ...
                {
                    BasicMedcationInfo.Add(new MedcationInfo
                    {
                        Name = dt.Rows[i]["BASNAME"].ToString().Trim(),
                        Uint = dt.Rows[i]["BASEXNAME"].ToString().Trim(),
                        ItemNo = dt.Rows[i]["BASCD"].ToString().Trim(),
                        Row = BasicMedcationInfo.Count + 2,
                        GroupName = "Agent",
                        USEAgent = dt.Rows[i]["USECLS"].ToString().Trim().To<int>(0)
                    });
                }
                //Agent
                ...
                //etc
                BasicMedcationInfo.Add(new MedcationInfo { Name = "U/O", Uint = "cc/hr", ItemNo = "I0000034071", Row = BasicMedcationInfo.Count + 2, GroupName = " " });
                BasicMedcationInfo.Add(new MedcationInfo { Name = "Warm blanket", Uint = "", ItemNo = "I0000034072", Row = BasicMedcationInfo.Count + 2, GroupName = " " });
                BasicMedcationInfo.Add(new MedcationInfo { Name = "Tourniquet", Uint = "", ItemNo = "I0000030758", Row = BasicMedcationInfo.Count + 2, GroupName = " " });
        ...
        private void btnSpdMacro_I_Click(object sender, EventArgs e)
        {
            ...
            #endregion
            #region 신규 로직
            using (frmEmrBaseRingerIO frm = new frmEmrBaseRingerIO(AcpEmr, mstrCHARTDATE.IsNullOrEmpty() ? ComQuery.CurrentDateTime(clsDB.DbCon, "D") : mstrCHARTDATE, mstrFormNo, mstrUpdateNo))
            {
        ...
        private int GetSapValue()
        {
            ...
            #region 수술 종료 시간 찾기
            DateTime dtpEndDate = ComQuery.CurrentDateTime(clsDB.DbCon, "S").To<DateTime>();
            string ActEndDate = dtpEndDate.ToString("yyyyMMdd");
            string ActEndTime = dtpEndDate.ToString("HHmmss");

            long dblActSum = 0;
            string Buse = string.Empty;

            for (int i = ssView_Sheet1.ColumnCount - 1; i >= 0; i--)
            {
                if (ssView_Sheet1.Cells[2, i].Text.Equals("X") || ssView_Sheet1.Cells[3, i].Text.Equals("⊙"))
                {
                    ActEndDate = ssView.ActiveSheet.Cells[0, i].Tag.ToString().Replace("-", ""); //날짜
                    ActEndTime = ssView.ActiveSheet.Cells[0, i].Text + ssView.ActiveSheet.Cells[1, i].Text + "00"; //HH + mm (시 + 분)
                    break;
                }
            }
            #endregion
            ...
            {
                #region 쿼리
                SQL = SQL + ComNum.VBLF + "WITH FLUID_LIST AS ";
                SQL = SQL + ComNum.VBLF + "(";
                SQL = SQL + ComNum.VBLF + " SELECT ";
                SQL = SQL + ComNum.VBLF + "           ACTDATE";
                SQL = SQL + ComNum.VBLF + "         , RPAD(ACTTIME, 6, '0') ACTTIME";
                SQL = SQL + ComNum.VBLF + "         , ACTGB";
                ...
                SQL = SQL + ComNum.VBLF + "    AND A.ACTDATE >= '" + dtpNow.Value.ToString("yyyyMMdd") + "' ";
                SQL = SQL + ComNum.VBLF + "    AND A.ACTDATE <= '" + ActEndDate + "'";
                SQL = SQL + ComNum.VBLF + "    AND A.SITEGB IN ('ERD', 'IPD', 'OPR')";
                SQL = SQL + ComNum.VBLF + "    AND A.DCCLS = '0' ";
                SQL = SQL + ComNum.VBLF + "    AND T.BUSECODE = '033103'";
                SQL = SQL + ComNum.VBLF + ")";

                SQL = SQL + ComNum.VBLF + " SELECT ";
                SQL = SQL + ComNum.VBLF + "         SUM(ACTQTY) AS ACTSUM ";
                SQL = SQL + ComNum.VBLF + "       , BUSE";
                SQL = SQL + ComNum.VBLF + "   FROM FLUID_LIST A";
                SQL = SQL + ComNum.VBLF + "  WHERE ACTGB <> '00'";
                SQL = SQL + ComNum.VBLF + "    AND ACTTIME >= '" + dtpNow.Value.ToString("HHmm00") + "'";
                SQL = SQL + ComNum.VBLF + "    AND ACTTIME <= '" + ActEndTime + "'";
                //SQL = SQL + ComNum.VBLF + "  WHERE (ACTDATE || ACTTIME) >= '" + dtpNow.Value.ToString("yyyyMMddHHmm00") + "' ";
                //SQL = SQL + ComNum.VBLF + "    AND (ACTDATE || ACTTIME) <= '" + ActEndDate + ActEndTime + "' ";
                //SQL = SQL + ComNum.VBLF + "    AND ACTGB <> '00'";
                SQL = SQL + ComNum.VBLF + "  GROUP BY BUSE";
                #endregion
                ...
                if (SqlErr != "")
                {
                    clsDB.setRollbackTran(clsDB.DbCon);
                    clsDB.SaveSqlErrLog(SqlErr, SQL, clsDB.DbCon); //에러로그 저장
                    ComFunc.MsgBoxEx(this, "수액기록 IO 계산 도중 오류가 발생했습니다.");
                    Cursor.Current = Cursors.Default;
                    return rtnVal;
                }
                ...
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        dblActSum = reader.GetValue(0).To<long>();
                        Buse = reader.GetValue(1).ToString();
                        ssIn_Sheet1.Cells[0, Buse.Equals("마취과") ? 2 : 1].Text = dblActSum.ToString();
                    }
                }
                ...
                ssIn_Sheet1.Cells[2, 1].Text = (ssIn_Sheet1.Cells[0, 1].Text.To<int>() + ssIn_Sheet1.Cells[1, 1].Text.To<int>()).ToString();
                ssIn_Sheet1.Cells[2, 2].Text = (ssIn_Sheet1.Cells[0, 2].Text.To<int>() + ssIn_Sheet1.Cells[1, 2].Text.To<int>()).ToString();
        ...
        {
            if (btnGraph.BackColor == Color.Pink)
            {
            ...
            {
                btnGraph.BackColor = Color.Pink;

                GetVitalGraph();

                chartVital.Visible = true;
            }
        ...
        private void GetVitalGraph()
        {
            ...
            try
            {
                ...
                for (i = 2; i < ssView.ActiveSheet.ColumnCount; i++)
                {
                    string strDateTime = ssView.ActiveSheet.Cells[0, i].Text + ":" + ssView.ActiveSheet.Cells[1, i].Text;
                    ...
                    for (int h = 5; h < 10; h++)
                    {
                        if (String.IsNullOrWhiteSpace(ssView.ActiveSheet.Cells[h, 0].Text))
                            continue;

                        Val = VB.Val(ssView.ActiveSheet.Cells[h, i].Text.Trim().Replace("/", string.Empty));

                        //new MedcationInfo { Name = "SBP", Uint = "mmHg", ItemNo = "I0000002018", Row = 5 },
                        //new MedcationInfo { Name = "DBP", Uint = "mmHg", ItemNo = "I0000001765", Row = 6 },
                        //new MedcationInfo { Name = "HR", Uint = "회/min", ItemNo = "I0000001178", Row = 7 },
                        //new MedcationInfo { Name = "RR", Uint = "회/min", ItemNo = "I0000000574", Row = 8 },
                        //new MedcationInfo { Name = "BT", Uint = "℃", ItemNo = "I0000001811", Row = 9 },

                        if (ssView.ActiveSheet.Cells[h, 0].Text.Trim() == "SBP")
                        {
                            if (Val > 0)
                            {
                                blnData = true;
                        ...
                        if (ssView.ActiveSheet.Cells[h, 0].Text.Trim() == "DBP")
                        {
                            if (Val > 0)
                            {
                                blnData = true;
                        ...
                        if (ssView.ActiveSheet.Cells[h, 0].Text.Trim() == "ASBP")
                        {
                            if (Val > 0)
                            {
                                blnData = true;
                        ...
                        if (ssView.ActiveSheet.Cells[h, 0].Text.Trim() == "ADBP")
                        {
                            if (Val > 0)
                            {
                                blnData = true;
                        ...
                        if (ssView.ActiveSheet.Cells[h, 0].Text.Trim() == "HR")
                        {
                            if (Val > 0)
                            {
                                blnData = true;
                        ...
                        if (ssView.ActiveSheet.Cells[h, 0].Text.Trim() == "RR")
                        {
                            if (Val > 0)
                            {
                                blnData = true;
                        ...
                        if (ssView.ActiveSheet.Cells[h, 0].Text.Trim() == "BT")
                        {
                            if (Val > 0)
                            {
                                blnData = true;
                ...
                chartVital.Series.Add("ASBP");
                chartVital.Series["ASBP"].ChartType = System.Windows.Forms.DataVisualization.Charting.SeriesChartType.Point;
                chartVital.Series["ASBP"].MarkerImage = clsEmrType.EmrSvrInfo.EmrClient + "\\Icon\\ASBP.png";
                chartVital.Series["ASBP"].IsValueShownAsLabel = false;

                chartVital.Series.Add("ADBP");
                chartVital.Series["ADBP"].ChartType = System.Windows.Forms.DataVisualization.Charting.SeriesChartType.Point;
                chartVital.Series["ADBP"].MarkerImage = clsEmrType.EmrSvrInfo.EmrClient + "\\Icon\\ADBP.png";
                chartVital.Series["ADBP"].IsValueShownAsLabel = false;

                chartVital.Series.Add("HR");
                chartVital.Series["HR"].ChartType = System.Windows.Forms.DataVisualization.Charting.SeriesChartType.Line;
                chartVital.Series["HR"].BorderWidth = 2;
                chartVital.Series["HR"].Color = Color.Blue;
                chartVital.Series["HR"].MarkerStyle = System.Windows.Forms.DataVisualization.Charting.MarkerStyle.Circle;
                chartVital.Series["HR"].MarkerColor = Color.Blue;
                chartVital.Series["HR"].MarkerSize = 6;

                chartVital.Series.Add("RR");
                chartVital.Series["RR"].ChartType = System.Windows.Forms.DataVisualization.Charting.SeriesChartType.Line;
                chartVital.Series["RR"].BorderWidth = 2;
                chartVital.Series["RR"].Color = Color.Green;
                chartVital.Series["RR"].MarkerStyle = System.Windows.Forms.DataVisualization.Charting.MarkerStyle.Circle;
                chartVital.Series["RR"].MarkerColor = Color.Green;
                chartVital.Series["RR"].MarkerSize = 6;

                chartVital.Series.Add("BT");
                chartVital.Series["BT"].ChartType = System.Windows.Forms.DataVisualization.Charting.SeriesChartType.Line;
                chartVital.Series["BT"].BorderWidth = 2;
                chartVital.Series["BT"].Color = Color.Red;
                chartVital.Series["BT"].MarkerStyle = System.Windows.Forms.DataVisualization.Charting.MarkerStyle.Circle;
                chartVital.Series["BT"].MarkerColor = Color.Red;
                chartVital.Series["BT"].MarkerSize = 6;
                ...
                chartVital.ChartAreas["Default"].AxisY.Minimum = 0;

                if (GVLsit.OrderByDescending(d => d.X).ToList()[0].X + 20 > 350)
                {
                    chartVital.ChartAreas["Default"].AxisY.Maximum = 350;
                }
                else
                {
                    chartVital.ChartAreas["Default"].AxisY.Maximum = GVLsit.OrderByDescending(d => d.X).ToList()[0].X + 20;
                }
                ...
                if (chartVital.ChartAreas.Count > 0)
                {
                    ...
                    {
                        if (VB.Split(chartVital.ChartAreas[k].Name, "_")[1] == "HR")
                        {
                        ...
                        else if (VB.Split(chartVital.ChartAreas[k].Name, "_")[1] == "BT")
                        {
                        ...
                        else if (VB.Split(chartVital.ChartAreas[k].Name, "_")[1] == "RR")
                        {
        ...
        private void btnAgent_Click(object sender, EventArgs e)
        {
            ...
            List<MedcationInfo> SetAgentItem = new List<MedcationInfo>();

            int rowidx = 2;

            for (int i = 0; i < SsAgent.RowCount(); i++)
            {
                if (Convert.ToBoolean(SsAgent.ActiveSheet.Cells[i, 2].Value) == true)
                {
                    MedcationInfo item = BasicMedcationInfo.Where(d => d.ItemNo == SsAgent.ActiveSheet.Cells[i, 1].Text).FirstOrDefault();

                    SetAgentItem.Add(new MedcationInfo
                    {
        ...
        private void ssIn_CellClick(object sender, CellClickEventArgs e)
        {

        }


        private void ssIn_EditModeOff(object sender, EventArgs e)
        {
            ssIn_Sheet1.Cells[2, 1].Text = (ssIn_Sheet1.Cells[0, 1].Text.To<int>() + ssIn_Sheet1.Cells[1, 1].Text.To<int>()).ToString();
            ssIn_Sheet1.Cells[2, 2].Text = (ssIn_Sheet1.Cells[0, 2].Text.To<int>() + ssIn_Sheet1.Cells[1, 2].Text.To<int>()).ToString();
        }

        private void btnAnItem_Click(object sender, EventArgs e)
        {
            using(frmAnFormAgentItem frm = new frmAnFormAgentItem())
            {
                frm.StartPosition = FormStartPosition.CenterScreen;
                frm.ShowDialog(this);
            }
