...
{
    public partial class frmNrActingSetWard : Form
    {
        public frmNrActingSetWard()
        {
            InitializeComponent();
        }

        private void frmNrActingSetWard_Load(object sender, EventArgs e)
        {
            cboWard.Items.Clear();
            ssForm_Sheet1.RowCount = 0;
            ssFormWard_Sheet1.RowCount = 0;

            FormSearch("간호활동항목", ssForm_Sheet1);
        ...
        private void FormSearch(string strWard, FarPoint.Win.Spread.SheetView Spd, string strSearch = "")
        {
            ...
            else
            {
                ...
                if (string.IsNullOrWhiteSpace(strSearch) == false)
                {
                    SQL = SQL + ComNum.VBLF + "     AND B.BASNAME LIKE '%" + strSearch.Replace("'", "`") + "%'  ";
                }
        ...
        private bool SaveData(string strItemCD, string strItemNm)
        {
            ...
            try
            {
                ...
                SQL = SQL + ComNum.VBLF + "    AND UNITCLS = '간호활동병동_" + (VB.Right(cboWard.Text, 20)).Trim() + "'"; //간호활동항목
                ...
                SQL = SQL + ComNum.VBLF + "              '간호활동병동_" + (VB.Right(cboWard.Text, 20)).Trim() + "', ";
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            FormSearch("간호활동항목", ssForm_Sheet1, txtSearch.Text.Trim());
        }
