...
{
    public partial class frmNrActingList : Form
    {
        ...
        public frmNrActingList()
        {
            InitializeComponent();
        }

        public frmNrActingList(EmrPatient po)
        {
            InitializeComponent();
        ...
        public frmNrActingList(EmrPatient po, string strEmrNos)
        {
            InitializeComponent();

            AcpEmr = po;
            mstrEmrNos = strEmrNos;
        }

        private void frmNrActingList_Load(object sender, EventArgs e)
        {
        ...
        private void GetData(string strSearch)
        {
            ...
            SQL = "";
            SQL = "SELECT ";
            SQL = SQL + ComNum.VBLF + "    AC.ACTDATE, AC.ACTTIME, AC.EMRNO, AC.NRACTSEQ, AC.ACTGB, AC.ACTGB1, AC.ACTRMK, AC.ACTUSEID, ";
            SQL = SQL + ComNum.VBLF + "    B.BASNAME, U.USENAME ";
            SQL = SQL + ComNum.VBLF + "FROM " + ComNum.DB_EMR + "AEMRNRACT AC";
            SQL = SQL + ComNum.VBLF + "INNER JOIN " + ComNum.DB_EMR + "AEMRBASCD B";
            SQL = SQL + ComNum.VBLF + "    ON AC.ITEMCD = B.BASCD";
            ...
            SQL = SQL + ComNum.VBLF + "    AND AC.ACTDATE >= '" + dtpFrDate.Value.ToString("yyyyMMdd") + "'";
            SQL = SQL + ComNum.VBLF + "    AND AC.ACTDATE <= '" + dtpEndDate.Value.ToString("yyyyMMdd") + "'";
            SQL = SQL + ComNum.VBLF + "    AND AC.DCCLS = '0'";
            if (chkDesc.Checked == true)
            {
                SQL = SQL + ComNum.VBLF + "ORDER BY (AC.ACTDATE || AC.ACTTIME) DESC";
            }
            else
            {
                SQL = SQL + ComNum.VBLF + "ORDER BY (AC.ACTDATE || AC.ACTTIME) ASC";
            }
            ...
            {
                ssView_Sheet1.Cells[i, 0].Text = ComFunc.FormatStrToDate(dt.Rows[i]["ACTDATE"].ToString().Trim(), "D");
                ssView_Sheet1.Cells[i, 1].Text = ComFunc.FormatStrToDate(dt.Rows[i]["ACTTIME"].ToString().Trim(), "M");
                ssView_Sheet1.Cells[i, 2].Text = dt.Rows[i]["BASNAME"].ToString().Trim();
                ssView_Sheet1.Cells[i, 3].Text = dt.Rows[i]["USENAME"].ToString().Trim();

                string strAct = "";

                if (dt.Rows[i]["ACTGB"].ToString().Trim() != "")
                {
                    strAct = dt.Rows[i]["ACTGB"].ToString().Trim();
                    if (dt.Rows[i]["ACTGB1"].ToString().Trim() != "")
                    {
                        strAct = strAct + " / " + dt.Rows[i]["ACTGB1"].ToString().Trim();
                        if (dt.Rows[i]["ACTRMK"].ToString().Trim() != "")
                        {
                            strAct = strAct + " / " + dt.Rows[i]["ACTRMK"].ToString().Trim();
                        }
                    }
                    else
                    {
                ...
                else
                {
                    if (dt.Rows[i]["ACTGB1"].ToString().Trim() != "")
                    {
                        strAct = dt.Rows[i]["ACTGB1"].ToString().Trim();
                        if (dt.Rows[i]["ACTRMK"].ToString().Trim() != "")
                        {
                            strAct = strAct + " / " + dt.Rows[i]["ACTRMK"].ToString().Trim();
                        }
                    ...
                    else
                    {
                        if (dt.Rows[i]["ACTRMK"].ToString().Trim() != "")
                        {
                            strAct = dt.Rows[i]["ACTRMK"].ToString().Trim();
                        }
                    }
                }

                ssView_Sheet1.Cells[i, 4].Text = strAct;
            }
        ...
        private void ssView_CellDoubleClick(object sender, FarPoint.Win.Spread.CellClickEventArgs e)
        {
            ...
            //if (ssView_Sheet1.RowCount != 3)
            //{
            //    strItemCd = ssView_Sheet1.Cells[e.Row, 0].Text.Trim();
            //}
            //else
            //{
            //    strItemCd = "";
