...
namespace ComEmrBase
{
    public partial class frmEmrNrCatheterView : Form
    {
        ...
        private void GetData(FarPoint.Win.Spread.SheetView Spd)
        {
            ...
            SQL.AppendLine("     FROM KOSMOS_EMR.AEMRBASCD I");
            SQL.AppendLine("    WHERE BSNSCLS = '기록지관리'");
            SQL.AppendLine("      AND UNITCLS = '도뇨관관리'");
            ...
            SQL.AppendLine("	  , (SELECT ITEMVALUE FROM KOSMOS_EMR.AEMRCHARTROW WHERE EMRNO = A.EMRNO AND EMRNOHIS = A.EMRNOHIS AND ITEMNO = 'I0000037645') AS INSERTDAY");
            SQL.AppendLine("	  , TO_NUMBER(NVL((SELECT REGEXP_REPLACE(ITEMVALUE, '[^0-9]+') FROM KOSMOS_EMR.AEMRCHARTROW WHERE EMRNO = A.EMRNO AND EMRNOHIS = A.EMRNOHIS AND ITEMNO = 'I0000037742'), 0)) AS USEDAY");
            //SQL.AppendLine("	  , (SELECT ITEMVALUE FROM KOSMOS_EMR.AEMRCHARTROW WHERE EMRNO = A.EMRNO AND EMRNOHIS = A.EMRNOHIS AND ITEMNO = 'I0000037742') AS USEDAY");
            SQL.AppendLine("	  , (  SELECT                                                          ");
            SQL.AppendLine("	   	   MAX(CHARTDATE) ");
            SQL.AppendLine("	       FROM KOSMOS_EMR.AEMRCHARTMST A                                  ");
            SQL.AppendLine("	         INNER JOIN KOSMOS_EMR.AEMRCHARTROW R                          ");
