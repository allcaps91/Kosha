...
namespace ComLibB
{
    /// <summary>
    /// Class Name      : ComLibB
    /// File Name       : frmViewSname.cs
    /// Description     : 수진자명 조회하는 폼
    /// Author          : 안정수
    /// Create Date     : 2017-06
    /// Update History  : try-catch문 수정
    /// <history>       
    /// D:\타병원\PSMHH\basic\busanid\busanid2.frm(FrmViewSname) => frmViewSname.cs 으로 변경함
    /// ComFunc.MidH 사용시 음수 관련 오류 발생 확인필요 -> 기존 Mid로 구현 해놓음
    /// </history>
    /// <seealso>
    /// D:\타병원\PSMHH\basic\busanid\busanid2.frm(FrmViewSname)
    /// </seealso>
    /// <vbp>
    ...
    /// </vbp>
    /// </summary>
    public partial class frmViewSname : Form
    {
        ...
        void frmViewSname_Load(object sender, EventArgs e)
        {
        ...
        void GetData()
        {
            ...
            try
            {
                SQL = "";
                SQL = SQL + ComNum.VBLF + "SELECT ";
                SQL = SQL + ComNum.VBLF + " Sname, Pano, TO_CHAR(Date1, 'YYYY-MM-DD') Day1, CoprName ";
                SQL = SQL + ComNum.VBLF + "FROM " + ComNum.DB_PMPA + "BAS_SANID";
                SQL = SQL + ComNum.VBLF + "WHERE Sname LIKE '%" + txtSname.Text.Trim() + "%'";
                SqlErr = clsDB.GetDataTable(ref dt, SQL, clsDB.DbCon);
                ...
                for (i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    strShowDate = ComFunc.LeftH(dt.Rows[i]["Day1"].ToString().Trim() + VB.Space(10), 10);
                    strShowCopr = ComFunc.LeftH(dt.Rows[i]["CoprName"].ToString().Trim() + VB.Space(16), 16);

                    ssList_Sheet1.Cells[i, 0].Text = strShowSname;
                    ssList_Sheet1.Cells[i, 1].Text = strShowPano;
                    ssList_Sheet1.Cells[i, 2].Text = strShowDate;
                    ssList_Sheet1.Cells[i, 3].Text = strShowCopr;
                }
