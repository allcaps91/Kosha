...
{
    /// <summary>
    /// Class Name : frmOcsMsgPanoLiver
    /// File Name : frmOcsMsgPanoLiver.cs
    /// Title or Description : 간장용제 메시지 처리 - [ 외래 ]
    /// Author : 박창욱
    ...
    /// </summary>
    /// <history>  
    /// VB\BuSuga63.frm(FrmOcsMsgPano_O3_Build) -> frmOcsMsgPanoLiver.cs 으로 변경함
    /// </history>
    /// <seealso> 
    /// VB\basic\busuga\BuSuga63.frm(FrmOcsMsgPano_O3_Build)
    /// </seealso>
    /// <vbp>
    /// default : VB\PSMHH\basic\busuga\\busuga.vbp
    /// </vbp>
    public partial class frmOcsMsgPanoLiver : Form
    {
        ...
        private void OPD_OCS_MSG_Process(string ArgFdate, string ArgTdate, string ArgDay)
        {
            ...
            {
                //간장용제 환자
                if (rdoReservation.Checked == true) //당일 예약환자 기준
                {
                    SQL = "";
                    ...
                    SQL = SQL + ComNum.VBLF + "     AND A.DATE3 < TO_DATE('" + strTDate + "','YYYY-MM-DD') ";   //시간 분까지 데이터 발생
                ...
                for (i = 0; i < nRead; i++)
                {
                    ...
                    SQL = SQL + ComNum.VBLF + "   AND B.SUBCODE IN ( 'CR34','CR35')";
                    ...
                    if (strOK_A == "NO" || strOK_B == "NO")     //OCS 메시지 처리
                    {
        ...
        private void btnMsg_Click(object sender, EventArgs e)
        {
            ...
            for (i = 1; i < ssView_Sheet1.RowCount; i++)
            {
                if (Convert.ToBoolean(ssView_Sheet1.Cells[i - 1, 0].Value) == true)
                {
                    ...
                    strMsg = "";
                    strMsg = "          <<< 간장용제 관련 정보 >>>              " + ComNum.VBLF;
                    strMsg = strMsg + "-----------------------------------------------   " + ComNum.VBLF;
                    strMsg = strMsg + "  검사종류            검사일자     결  과  " + ComNum.VBLF;
                    strMsg = strMsg + "-----------------------------------------------   " + ComNum.VBLF;
                    strMsg = strMsg + " G O T        " + strExamDate_A + VB.Space(5) + strResult_A + ComNum.VBLF;
                    strMsg = strMsg + " [참고치: 7 ~ 38 ]                              " + ComNum.VBLF;
                    strMsg = strMsg + "-----------------------------------------------   " + ComNum.VBLF;
                    strMsg = strMsg + " G P T         " + strExamDate_B + VB.Space(5) + strResult_B + ComNum.VBLF;
                    strMsg = strMsg + " [참고치: 4 ~ 43 ]                              " + ComNum.VBLF;
                    strMsg = strMsg + "-----------------------------------------------  " + ComNum.VBLF + ComNum.VBLF;
                    strMsg = strMsg + " ◈ 간장용제 오더 전송시 본인 부담 100% 로   " + ComNum.VBLF;
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            strHead1 = "/c/f1" + "간 장 용 제 메세지 대상자" + "/n";
        ...
        private void frmOcsMsgPanoLiver_Load(object sender, EventArgs e)
        {
