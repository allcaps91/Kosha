...
namespace ComLibB
{
    ...
    public partial class frmGyellist : Form, MainFormMessage
    {
        ...
        private void GetData(string TabTag)
        {
            ...
            {
                SQL = "";
                SQL = "SELECT A.ACTDATE, A.ENTDATE, A.PANO, A.SNAME, A.EDUCATOR, A.ILLCODE, A.GUNGU, A.M2_DISREG9,(SELECT SEX FROM ADMIN.BAS_PATIENT WHERE PANO = A.PANO) AS SEX, ADMIN.FC_GET_AGE2(A.PANO,TRUNC(SYSDATE)) AS AGE, A.ROWID ";
                if(TabTag == "0")
                {
