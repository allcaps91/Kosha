...
namespace ComLibB
{
    ...
    public partial class frmComConPatInfo_SCH : Form
    {
        ...
        List<string> sel_PATIENT_SCHDUL_TIME(PsmhDb pDbCon, string strPANO)
        {
            ...
            SQL += " 	   FROM ADMIN.OPD_RESERVED_NEW                                                        \r\n";
            SQL += " 	  WHERE 1 = 1                                                                               \r\n";
            SQL += " 	   AND Pano     = " + ComFunc.covSqlstr(strPANO, false);
        ...
        DataSet sel_PATIENT_SCHDUL_Detail(PsmhDb pDbCon, string strPANO, string strBDATE)
        {
            ...
            SQL += "    , DEPTCODE                                                                                                          \r\n";
            SQL += "    , DRCODE                                                                                                            \r\n";
            SQL += "    , ADMIN.FC_BAS_DOCTOR_DRNAME(DRCODE) 						FC_DRNAME                                           \r\n";
            SQL += "     FROM ADMIN.OPD_MASTER                                                                                        \r\n";
            SQL += "    WHERE 1 = 1                                                                                                         \r\n";
            SQL += "     AND PANO   = " + ComFunc.covSqlstr(strPANO, false);
            ...
            SQL += "    	'3' 															SORT                                            \r\n";
            SQL += "    ,	'수술예약' 														GBN                                             \r\n";
            SQL += "    ,	TRIM(OPBUN) || '.' || ADMIN.FC_BAS_BCODE_NAMEREAD('ORAN_수술구분',TRIM(OPBUN)) GBN2                        \r\n";
            SQL += "    ,	OPTITLE 														GBN3                                            \r\n";
            ...
            SQL += "     	'4' 																					SORT                    \r\n";
            SQL += "    ,	'내시경' 																				GBN                     \r\n";
            SQL += "    ,	TRIM(GBJOB) || '.' || ADMIN.FC_BAS_BCODE_NAMEREAD('ENDO_내시경분류',TRIM(GBJOB)) 	GBN2                    \r\n";
            SQL += "    ,	ADMIN.FC_OCS_ORDERCODE_NAME(ORDERCODE) 											GBN3                    \r\n";
            ...
            SQL += "      '5' SORT                                                                                                          \r\n";
            SQL += "    ,'기능검사' GBN                                                                                                     \r\n";
            SQL += "    , TRIM(GUBUN) || '.' || ADMIN.FC_BAS_BCODE_NAMEREAD('C#_EKG검사종류',TRIM(GUBUN)) 	GBN2                        \r\n";
            SQL += "    ,	ADMIN.FC_OCS_ORDERCODE_NAME(ORDERCODE) 											GBN3                    \r\n";
            SQL += "    , DECODE(GBJOB,'1','','Y')																CHK_ACT                     \r\n";
            SQL += "    ,	TO_CHAR(RDATE,'YYYY-MM-DD HH24:MI') 													RDATE                   \r\n";
            SQL += "    ,	DEPTCODE                                                                                                        \r\n";
            SQL += "    ,	DRCODE                                                                                                          \r\n";
            SQL += "    ,	ADMIN.FC_BAS_DOCTOR_DRNAME(TRIM(DRCODE)) 											AS FC_DRNAME            \r\n";
            SQL += "     FROM ADMIN.ETC_JUPMST                                                                                         \r\n";
            SQL += "    WHERE 1 = 1                                                                                                         \r\n";
            SQL += "     AND PTNO  = " + ComFunc.covSqlstr(strPANO, false);
            SQL += "     AND GBJOB IN ('1','2','3')                                                                                         \r\n";
            ...
            SQL += "   SELECT                                                                                                               \r\n";
            SQL += "    	'6' SORT,'영상검사' 																	GBN                     \r\n";
            SQL += "    ,	TRIM(XJONG) || '.' || ADMIN.FC_BAS_BCODE_NAMEREAD('XRAY_방사선종류',TRIM(XJONG)) 	GBN2                    \r\n";
            SQL += "    ,	ADMIN.FC_OCS_ORDERCODE_NAME(ORDERCODE) 											GBN3                    \r\n";
            ...
            SQL += "          '7' 																			SORT                            \r\n";
            SQL += "    	  , '진단검사' 																		GBN                         \r\n";
            SQL += "    	  , ADMIN.FC_OCS_ORDERCODE_SLIPNM(TRIM(A.MASTERCODE)) 							AS GBN2                     \r\n";
            SQL += "    	  , (SELECT MAX(ORDERNAME) FROM ADMIN.OCS_ORDERCODE WHERE TRIM(ITEMCD) = A.MASTERCODE)																		AS GBN3                     \r\n";
            SQL += "     	  , DECODE(NVL(A.SPECNO,'^*'),'^*','','Y')											AS CHK_ACT                  \r\n";
            SQL += "     	  , CASE WHEN (A.RDATE IS NULL OR A.RDATE = '')  THEN TO_CHAR(A.BDATE, 'YYYY-MM-DD')                            \r\n";
            SQL += "    		    		 ELSE TO_CHAR(A.RDATE, 'YYYY-MM-DD') END 							AS RDATE                    \r\n";
            SQL += "        , DEPTCODE                                                                                                      \r\n";
            SQL += "        , DRCODE                                                                                                        \r\n";
            SQL += "        , ADMIN.FC_BAS_DOCTOR_DRNAME(TRIM(DRCODE)) FC_DRNAME                                                       \r\n";
            SQL += "    	  FROM ADMIN.EXAM_ORDER   A                                                                                \r\n";
            SQL += "    	  WHERE 1=1                                                                                                     \r\n";
            SQL += "    	    AND A.PANO = " + ComFunc.covSqlstr(strPANO, false);
            SQL += "    	    AND (                                                                                                       \r\n";
