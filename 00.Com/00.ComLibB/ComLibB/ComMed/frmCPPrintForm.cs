...
namespace ComLibB
{
    public partial class frmCPPrintForm : Form
    {
        ...
        private void Set_CP()
        {
            ...
            mParameter.AppendSql("  FROM KOSMOS_OCS.OCS_CP_MAIN A                                                                                                                                                                                               ");
            mParameter.AppendSql(" INNER JOIN KOSMOS_OCS.OCS_CP_SUB B                                                                                                                                                                                           ");
            ...
            mParameter.AppendSql("   AND A.SDATE = (SELECT MAX(SDATE) FROM KOSMOS_OCS.OCS_CP_MAIN WHERE CPCODE = A.CPCODE)                                                                                                                                      ");
            ...
            mParameter.AppendSql(" 	,	CASE WHEN GUBUN = '01' AND ROW_NUM = 0 THEN (SELECT REPLACE(WM_CONCAT(SUB.NAME), ',', '\r\n') FROM KOSMOS_OCS.OCS_CP_SUB SUB WHERE SUB.CPCODE = A.CPCODE AND SUB.SDATE = A.SDATE AND SUB.GUBUN = '07') END AGREERECORD  ");
            mParameter.AppendSql(" 	,	CASE WHEN GUBUN = '01' AND ROW_NUM = 0 THEN (SELECT BASNAME FROM KOSMOS_PMPA.BAS_BASCD SUB WHERE SUB.GRPCDB = 'CP관리' AND SUB.GRPCD  = 'CP코드관리' AND SUB.BASCD = A.CPCODE) END CPNAME                                 ");
