...
namespace ComHpcLibB
{
    public partial class frmHcSchoolChargeExpenses : Form
    {
            ...
            else if (sender == btnSearch)
            {
                ...
                for (int i = 0; i < nRead; i++)
                {
                    ...
                    if (strOK == "OK")
                    {
                        ...
                        SS1.ActiveSheet.Cells[i, 2].Text = list[i].MINDATE;
                        SS1.ActiveSheet.Cells[i, 3].Text = list[i].MAXDATE;
                        SS1.ActiveSheet.Cells[i, 4].Text = list[i].CNT.To<string>();
                        SS1.ActiveSheet.Cells[i, 5].Text = list[i].CLASS.To<string>();
                        SS1.ActiveSheet.Cells[i, 6].Text = list[i].LTDCODE.To<string>();
                    }
                    else if (strOK2 == "OK")
                    {
                        SS1.ActiveSheet.Cells[i, 1].Text = hb.READ_Ltd_Name(list[i].LTDCODE.To<string>());
        ...
        void fn_Screen_Display(string argDate1, string argDate2, string argClass, string argBan, string argLtdCode)
        {
            ...
            for (int i = 1; i <= 8; i++)
            {
                nPrice[i] = VB.Pstr(strData, ",", i).To<long>();
                if (nPrice[i] == 0)
                {
            ...
            for (int i = 15; i <= 22; i++)
            {
                SS2.ActiveSheet.Cells[i - 1, 5].Text = nMan[i - 14].To<string>();
                nSum[2] += nMan[i - 14];
                SS2.ActiveSheet.Cells[i - 1, 6].Text = nWoMan[i - 14].To<string>();
                nSum[3] += nWoMan[i - 14];
                SS2.ActiveSheet.Cells[i - 1, 7].Text = (nMan[i - 14] + nWoMan[i - 14]).To<string>();
