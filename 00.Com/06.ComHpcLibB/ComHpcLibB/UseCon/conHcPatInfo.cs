    ...
    public partial class conHcPatInfo : UserControl
    {
        ...
        private DataTable SelDB(string gbIo, string bDate, string Ptno, string DeptCode, string GjJong)
        {
            ...
            try
            {
                ...
                //SQL = SQL + ComNum.VBLF + "    , ADMIN.FC_GET_AGE(ADMIN.FC_BAS_PATIENT_BIRTH(PAT.PTNO), TRUNC(SYSDATE))          AS AGE      -- 02 나이";
                ...
                SQL = SQL + ComNum.VBLF + "    , HIC.GJJONG || '.' || ADMIN.FC_HIC_GJJONG_NAME(HIC.GJJONG, HIC.UCODES)               AS GJJONG   -- 07 검진종류";
                SQL = SQL + ComNum.VBLF + "    , ADMIN.FC_HIC_JEPSUJONG(HIC.PTNO, TO_DATE(HIC.JEPDATE))                              AS GBGJYN   -- 08 검진여부";       // TODO : 공백처리
                SQL = SQL + ComNum.VBLF + "    , ADMIN.FC_HIC_LTDNAME(HIC.LTDCODE)                                                   AS LTDNAME  -- 09 사업장명칭";     
                ...
                SQL = SQL + ComNum.VBLF + "    , ADMIN.FC_BAS_VIPINFO(HIC.PTNO) || '/' || PAT.VIPREMARK                              AS VIP      -- 15 VIP ";
                SQL = SQL + ComNum.VBLF + "    , PAT.GBPRIVACY                                                                             AS PRIVACY  -- 16 개인정보 ";
                SQL = SQL + ComNum.VBLF + "      , ''                                                                                      AS CVR      -- 17 CVR ";           // TODO : CVR
                SQL = SQL + ComNum.VBLF + "    , ADMIN.FC_DRUG_ADR_CHK(PAT.PTNO)                                                      AS ADR      -- 18 ADR";
                SQL = SQL + ComNum.VBLF + "    , ADMIN.FC_ETC_ALLERGY_MST_CHK(PAT.PTNO)                                               AS ALLEGY   -- 19 알러지";
                SQL = SQL + ComNum.VBLF + "    , ADMIN.FC_NUR_FALL_REPORT_CHK(PAT.PTNO, TRUNC(SYSDATE))                               AS FALL     -- 20 낙상";
                //SQL = SQL + ComNum.VBLF + "    , ADMIN.FC_NUR_MASTER_FIRE(IPD.IPDNO)                                                  AS FIRE     -- 21 화재 ";
                ...
                SQL = SQL + ComNum.VBLF + "    , ADMIN.FC_EXAM_INFECT_MASTER_IMG_EX(HIC.PTNO, TRUNC(HIC.JEPDATE))                     AS INFE_IMG -- 25 감염이미지";
                SQL = SQL + ComNum.VBLF + "  FROM ADMIN.HIC_PATIENT PAT ";
                SQL = SQL + ComNum.VBLF + "       LEFT OUTER JOIN ADMIN.HIC_JEPSU HIC ";
                ...
                SQL = SQL + ComNum.VBLF + "     , HEA.GJJONG || '.' || ADMIN.FC_HEA_GJJONG_NAME(HEA.GJJONG)                          AS GJJONG   -- 07 검진종류";       
                SQL = SQL + ComNum.VBLF + "     , ADMIN.FC_HIC_JEPSUJONG(HEA.PTNO, TO_DATE(HEA.SDATE))                               AS GBGJYN   -- 08 검진여부";       // TODO : 공백처리
                SQL = SQL + ComNum.VBLF + "     , ADMIN.FC_HIC_LTDNAME(HEA.LTDCODE)                                                  AS LTDNAME  -- 09 사업장명칭";
                //SQL = SQL + ComNum.VBLF + "     , ''                                                                                       AS WEIGHT   -- 10 신장";          
                //SQL = SQL + ComNum.VBLF + "     , ''                                                                                       AS HEIGHT   -- 11 몸무게";        
                //SQL = SQL + ComNum.VBLF + "     , ''                                                                                       AS BPRESS   -- 12 혈압";          
                //SQL = SQL + ComNum.VBLF + "     , ''                                                                                       AS BPULSE   -- 13 맥박";          
                SQL = SQL + ComNum.VBLF + "     , ''                                                                                       AS UCODENM  -- 14 유해인자명";     // TODO : 공백처리
                SQL = SQL + ComNum.VBLF + "     , ADMIN.FC_BAS_VIPINFO(HEA.PTNO) || '/' || PAT.VIPREMARK                             AS VIP      -- 15 VIP ";
                SQL = SQL + ComNum.VBLF + "     , PAT.GBPRIVACY                                                                            AS PRIVACY  -- 16 개인정보 ";
                SQL = SQL + ComNum.VBLF + "       , ''                                                                                     AS CVR      -- 17 CVR ";           // TODO : CVR
                SQL = SQL + ComNum.VBLF + "     , ADMIN.FC_DRUG_ADR_CHK(PAT.PTNO)                                                     AS ADR      -- 18 ADR";
                SQL = SQL + ComNum.VBLF + "     , ADMIN.FC_ETC_ALLERGY_MST_CHK(PAT.PTNO)                                              AS ALLEGY   -- 19 알러지";
                ...
                SQL = SQL + ComNum.VBLF + "     , ''                                                                                       AS PASTILL  -- 24 과거력 ";
                SQL = SQL + ComNum.VBLF + "    , ADMIN.FC_EXAM_INFECT_MASTER_IMG_EX(HEA.PTNO, TRUNC(HEA.SDATE))                       AS INFE_IMG -- 25 감염이미지";
                SQL = SQL + ComNum.VBLF + "  FROM ADMIN.HEA_JEPSU    HEA                                                                                              ";
                SQL = SQL + ComNum.VBLF + "     , ADMIN.HIC_PATIENT  PAT                                                                                              ";
                SQL = SQL + ComNum.VBLF + " WHERE 1    = 1                                                                                                                       ";
